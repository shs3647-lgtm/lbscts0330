/**
 * @file seed-sod-m066.mjs
 * @description SOD 기준표에 m066 (12inch Au Bump) 반도체 공정 사례 추가
 * 실행: node scripts/seed-sod-m066.mjs
 */
import pg from 'pg';
const { Client } = pg;
...
async function main() {
  ...
  console.log('✅ Severity: 10건 m066 사례 추가');

  // ── Occurrence: description에 m066 사례 병기 ──
  const occEx = [
    [1,  '[m066] Poka-Yoke: Wafer 방향 센서 → 역삽입 물리적 차단'],
    [2,  '[m066] Sputter Target kWh 자동교체 (고장이력 0건)'],
    [3,  '[m066] Photo CD SPC (Cpk≥1.67), 간헐적 편차만'],
    [4,  '[m066] Plating 전류밀도 SPC (Cpk≥1.33)'],
    [5,  '[m066] 도금액 4hr ICP 분석, 500 wf당 1건 편차'],
    [6,  '[m066] Etch Bath ±1°C 관리, 200 wf당 1건 이탈'],
    [7,  '[m066] Strip 시간 기본관리, 100 wf당 1건 잔류물'],
    [8,  '[m066] 진공도 기본PM만, 50 wf당 1건 접착력 불량'],
    [9,  '[m066] Chamber 오염 미흡, 20 wf당 1건 파티클'],
    [10, '[m066] 신규 도금액, 기준 없음, 10 wf당 1건+ 불량'],
  ];
  for (const [r, ex] of occEx) {
    // 기존 description 뒤에 m066 사례 추가 (이미 있으면 스킵)
    const cur = await c.query(
      `SELECT description FROM public.pfmea_occurrence_criteria WHERE rating = $1`, [r]
    );
    const curDesc = cur.rows[0]?.description || '';
    if (!curDesc.includes('[m066]')) {
      await c.query(
  ...
  console.log('✅ Occurrence: 10건 m066 사례 추가');

  // ── Detection: detection 필드에 m066 사례 병기 ──
  const detEx = [
    [1,  '[m066] Wafer 방향 센서 자동정지 (물리적 차단)'],
    [2,  '[m066] 막두께 In-line 전수 9-point + 자동 Reject'],
    [3,  '[m066] AOI 전수검사 + 자동정지 + SEM 확인'],
    [4,  '[m066] CD-SEM 자동측정 (매 lot 5-point 전수)'],
    [5,  '[m066] X-ray Void 검사 (lot당 3매 샘플링)'],
    [6,  '[m066] SPC 모니터링 + 수동 현미경 (Shift별 1회)'],
    [7,  '[m066] 육안 + 수동 치수 (작업자 숙련도 의존)'],
    [8,  '[m066] 이중 육안검사 (QA+공정, 미세결함 한계)'],
    [9,  '[m066] 최종 외관검사만 (공정 중 검사 없음)'],
    [10, '[m066] 검사 미수립 (신규 결함, 기준/방법 없음)'],
  ];
  ...
  console.log('✅ Detection: 10건 m066 사례 추가');
