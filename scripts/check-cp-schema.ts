...
async function main() {
  // 1. public schema CP
  const cp = await p.controlPlan.findFirst({ where: { cpNo: 'cp26-m066' } });
  ...
  try {
    const result: any = await p.$queryRawUnsafe("SELECT count(*)::int as cnt FROM pfmea_pfm26_m066.control_plan_items");
  ...
  try {
    const result2: any = await p.$queryRawUnsafe("SELECT count(*)::int as cnt FROM pfmea_pfm26_m066.control_plans");
