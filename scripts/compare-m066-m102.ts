/**
 * m002 vs m102 Atomic DB 1:1 비교 — 차이점 출력
 */
...
async function main() {
  ...
  const p066 = getPrismaForSchema('pfmea_pfm26_m002');
  ...
  console.log('=== m002 vs m102 Atomic DB Comparison ===\n');
  console.log('| Entity | m002 | m102 | Diff | Status |');
  console.log('|--------|------|------|------|--------|');

  for (const [table, label] of tables) {
    const c066 = await (p066 as any)[table].count({ where: { fmeaId: 'pfm26-m002' } }).catch(() => 0);
  ...
  // 고장사슬 FK 비교
  const fl066 = await p066.failureLink.findMany({ where: { fmeaId: 'pfm26-m002', deletedAt: null } });
  ...
  console.log(`\n=== FE text comparison ===`);
  console.log(`m002 unique effect texts: ${feText066.size}`);
  console.log(`m102 unique effect texts: ${feText102.size}`);

  // FC 비교 
  const fc066 = await p066.failureCause.findMany({ where: { fmeaId: 'pfm26-m002' } });
  const fc102 = await p102.failureCause.findMany({ where: { fmeaId: 'pfm26-m102' } });
  console.log(`\n=== FC count by L3 (processNo) ===`);
  console.log(`m002: ${fc066.length} causes across ${new Set(fc066.map(f => f.l3StructId)).size} L3s`);
