...
export default function DataSelectModal({
  ...
  // ✅ 2026-01-19: currentValues가 변경되면 (다른 셀 클릭) initialized 리셋
  useEffect(() => {
    const prevStr = prevCurrentValuesRef.current.sort().join('|');
    const currStr = [...currentValues].sort().join('|');
    
    if (prevStr !== currStr) {
      console.log('[DataSelectModal] 대상 변경 감지 - 선택 상태 리셋', { prev: prevCurrentValuesRef.current, curr: currentValues });
