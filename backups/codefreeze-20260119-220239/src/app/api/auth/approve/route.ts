...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(`✅ 사용자 승인 완료: ${updatedUser.name} (${updatedUser.email})`);

    // 사용자에게 승인 이메일 발송
    try {
      await sendApprovalEmail(updatedUser);
      console.log(`📧 승인 이메일 발송 완료: ${updatedUser.email}`);
...
async function sendApprovalEmail(user: { name: string; email: string | null; phone: string | null }) {
  if (!user.email) {
    console.warn('⚠️ 사용자 이메일이 없어 발송할 수 없습니다.');
    return;
  }
  ...
  if (!smtpUser || !smtpPass) {
    console.warn('⚠️ SMTP 설정이 없어 이메일을 발송할 수 없습니다.');
    return;
  }
  ...
  await transporter.sendMail({
    from: smtpUser,
    to: user.email,
    subject: `[FMEA] 가입 승인 완료 - ${user.name}님`,
    html: htmlContent,
  });
}
