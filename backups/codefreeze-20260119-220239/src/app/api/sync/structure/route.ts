  ...
  try {
    ...
    if (cpNo) {
      ...
      if (cpRegistration?.fmeaId) {
        ...
        console.log(`[SYNC] ★★★ CP(${cpNo})에 등록된 FMEA ID 사용: ${actualFmeaId} (요청된 fmeaId: ${fmeaId}) ★★★`);
      } else {
        console.log(`[SYNC] CP(${cpNo})에 등록된 FMEA ID 없음, 요청된 fmeaId 사용: ${fmeaId}`);
      }
    }
    ...
    try {
      ...
      if (fmeaProject?.parentFmeaId && fmeaProject.parentFmeaId !== fmeaIdLower) {
        ...
        console.log(`[SYNC] 상위 FMEA 발견: ${parentFmeaId} (현재 FMEA: ${fmeaIdLower}, 유형: ${fmeaProject.fmeaType})`);
      }
    } catch (e) {
      console.log(`[SYNC] FmeaProject 조회 실패 (무시): ${e}`);
    }
    ...
    console.log(`[SYNC-SOURCE] 현재 FMEA(${fmeaIdLower}) L2Structure 조회: ${l2Structures?.length || 0}개`);
    if (l2Structures && l2Structures.length > 0) {
      console.log(`[SYNC-SOURCE] 현재 FMEA 데이터 공정명: [${l2Structures.map((l2: any) => l2.name).join(', ')}]`);
    }

    // ★ 상위 FMEA 데이터 병합 (현재 FMEA에 없는 공정은 상위에서 가져옴)
    if (parentFmeaId && (!l2Structures || l2Structures.length === 0)) {
      console.log(`[SYNC-SOURCE] 현재 FMEA에 L2 데이터 없음, 상위 FMEA(${parentFmeaId}) 데이터 조회`);
      ...
      if (parentL2 && parentL2.length > 0) {
        ...
        console.log(`[SYNC-SOURCE] ★★★ 상위 FMEA(${parentFmeaId})에서 ${parentL2.length}개 공정 로드 ★★★`);
        console.log(`[SYNC-SOURCE] 상위 FMEA 데이터 공정명: [${parentL2.map((l2: any) => l2.name).join(', ')}]`);
    ...
    if (!l2Structures || l2Structures.length === 0) {
      console.log(`[SYNC-SOURCE] ❌ L2Structure 없음. 현재 FMEA(${fmeaId}) 및 상위 FMEA(${parentFmeaId || '없음'}) 모두 데이터 없음`);
      console.log(`[SYNC-SOURCE] ❌ 레거시 데이터 조회 비활성화 - 데이터 누락으로 처리`);
    ...
    for (const l2 of l2Structures) {
      ...
      console.log(`[SYNC-RISK] L2=${l2.name}, S=${maxSeverity}, O=${maxOccurrence}, D=${maxDetection}, AP=${highestAp}`);
      ...
      console.log(`[SYNC-FUNC] L2=${l2.name}, 메인공정기능(D열): [${processDesc}]`);
      ...
      console.log(`[SYNC-WORK] L2=${l2.name}, 작업요소(E열): [${workElement}]`);
    ...
    console.log(`[SYNC-SOURCE] 최종 데이터 출처: ${dataSource} (FMEA ID: ${dataSourceFmeaId})`);
    console.log(`[SYNC-SOURCE] 요청된 FMEA: ${fmeaId}, 실제 사용된 FMEA: ${fmeaIdLower}`);
