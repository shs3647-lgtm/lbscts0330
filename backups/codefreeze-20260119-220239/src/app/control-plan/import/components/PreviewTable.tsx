    ...
    <table className="border-collapse border-spacing-0 w-full table-fixed m-0 p-0 border-0">
      ...
      <thead className="sticky top-0 z-[10]">
        ...
        <tr className="h-[22px]">
          ...
          <th className={`${tw.headerCell} bg-[#0d9488] sticky top-[18px]`}>작업</th>
          {PREVIEW_COLUMNS.map(col => {
        ...
        {processNos.length === 0 ? (
          Array.from({ length: 500 }).map((_, i) => (
            <tr key={`empty-${i}`} className={`h-[18px] ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
              ...
              <td className="border border-gray-300 px-[1px] py-[1px] text-[8px] text-center"></td>
              {PREVIEW_COLUMNS.map(col => <td key={col.key} className="border border-gray-300 px-[1px] py-[1px] text-[8px]"></td>)}
            </tr>
          ))
        ) : (
          processNos.map((processNo, i) => {
            ...
            return (
              <tr key={`row-${processNo}-${i}`} className={`min-h-[18px] ${selectedRows.has(processNo) ? 'bg-blue-50' : i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                ...
                {PREVIEW_COLUMNS.map(col => {
                  // 공정설명(processDesc)만 좌측정렬, 나머지는 가로/세로 중앙정렬
                  const isProcessDesc = col.key === 'processDesc';
                  const alignClass = isProcessDesc
                    ? 'text-left align-top'
                    : 'text-center align-middle';

                  return (
                    <td key={col.key} className={`border border-gray-300 px-[1px] py-[1px] text-[8px] whitespace-pre-wrap break-words ${alignClass} ${selectedColumn === col.key ? 'bg-yellow-100' : ''}`}>
                      {isEditing ? (
                        <input
                          type="text"
