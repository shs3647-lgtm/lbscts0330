 ...
 *
 * 연동 필드:
 * - 공정번호/공정명: PFMEA L2.no/name ↔ CP processNo/processName
 * - 공정설명(D열): PFMEA L2 메인공정 기능 ↔ CP processDesc
 * - 작업요소: PFMEA L3 [4M + 작업요소명] ↔ CP workElement
 * - 제품특성: PFMEA L2.functions.productChars ↔ CP productChar
 * - 공정특성: PFMEA L3.functions.processChars ↔ CP processChar
...
export function syncPfmeaToCP(state: WorksheetState): { cpRows: CPRow[]; result: SyncResult } {
  ...
  // 🔍 DEBUG: state.l2 전체 구조 확인
  console.log('═══════════════════════════════════════════════════════');
  console.log('[syncPfmeaToCP] 📊 state.l2 요약:');
  (state.l2 || []).forEach((proc: any, idx: number) => {
    console.log(`  [${idx}] "${proc.name}" - functions: ${(proc.functions || []).length}개, l3: ${(proc.l3 || []).length}개`);
  ...
  // 기존 CP 행들 (ID 기준 맵) - PFMEA 연동 필드는 항상 새로 계산하므로 참고용
  ...
  (state.l2 || []).forEach((proc: any) => {
    ...
    // ✅ 공정설명(D열) = L2 메인공정 기능 (functions)
    const rawFunctions = proc.functions || [];
    ...
    // 🔍 DEBUG: 최종 processDesc 확인
    if (rawFunctions.length > 0 || processDesc) {
      console.log(`[syncPfmeaToCP] ✅ "${proc.name}" → processDesc: "${processDesc}" (functions ${rawFunctions.length}개)`);
    } else {
      console.warn(`[syncPfmeaToCP] ⚠️ "${proc.name}" → functions 비어있음! processDesc가 빈값이 됩니다.`);
    }
    ...
    (proc.l3 || []).forEach((we: any) => {
      ...
      // ✅ MN 제외 - MC, IM, EN만 CP 연동
      if (m4 && !CP_SYNC_4M_CODES.includes(m4)) return;
      const workElementName = we.name;

      // ✅ 작업요소 = [4M] + 작업요소명
      const workElement = m4 ? `[${m4}] ${workElementName}` : workElementName;
      ...
      cpRow.processDesc = processDesc;           // ✅ L2 메인공정 기능
      cpRow.workElement = workElement;           // ✅ [4M] 작업요소명
...
export function checkSyncStatus(state: WorksheetState, cpRows: CPRow[]): {
  ...
  // PFMEA 행 수 계산 (MC, IM, EN만 카운트 - MN 제외)
  let pfmeaCount = 0;
  (state.l2 || []).forEach((proc: any) => {
    if (!proc.name || proc.name.includes('클릭')) return;
    (proc.l3 || []).forEach((we: any) => {
