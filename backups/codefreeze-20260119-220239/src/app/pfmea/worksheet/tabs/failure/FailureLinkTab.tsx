...
export default function FailureLinkTab({ state, setState, setStateSynced, setDirty, saveToLocalStorage, saveToLocalStorageOnly, saveAtomicDB }: FailureTabProps) {
  ...
  // ========== ✅ 고아(Orphan) 감지 - 읽기 전용 계산 (Rule 9 준수) ==========
  // ⚠️ 절대 linkedFEs/linkedFCs를 수정하지 않음!
  const orphanIds = useMemo(() => {
    ...
    // 고아 ID Set들 (savedLinks에 있지만 현재 데이터에 없는 것)
    const orphanFmIds = new Set<string>();
    const orphanFeIds = new Set<string>();
    const orphanFcIds = new Set<string>();
    
    savedLinks.forEach(link => {
      // FM 고아 감지: savedLinks에 있는 fmId가 현재 fmData에 없으면 고아
      if (link.fmId && !validFmIds.has(link.fmId)) {
        orphanFmIds.add(link.fmId);
      }
      // FE 고아 감지
      if (link.feId && link.feId.trim() !== '' && !validFeIds.has(link.feId)) {
        orphanFeIds.add(link.feId);
      }
      // FC 고아 감지
      if (link.fcId && link.fcId.trim() !== '' && !validFcIds.has(link.fcId)) {
        orphanFcIds.add(link.fcId);
      }
    });
    
    // 고아 로그 출력
    if (orphanFmIds.size > 0 || orphanFeIds.size > 0 || orphanFcIds.size > 0) {
      console.log('[고아 감지] FM:', orphanFmIds.size, '개, FE:', orphanFeIds.size, '개, FC:', orphanFcIds.size, '개');
    }
  ...
  const fmDataWithOrphans = useMemo(() => {
    ...
    // 고아 FM들을 savedLinks에서 추출하여 fmData 형태로 변환
    ...
    savedLinks.forEach(link => {
      if (orphanIds.orphanFmIds.has(link.fmId) && !addedIds.has(link.fmId)) {
        orphanFMs.push({
          id: link.fmId,
          fmNo: `🗑️${link.fmNo || 'M?'}`,  // 🗑️ prefix로 고아 표시
    ...
    console.log('[고아 FM 추가] 고아:', orphanFMs.length, '개 추가됨');
    return [...fmData, ...orphanFMs];
  }, [fmData, orphanIds.orphanFmIds, savedLinks]);

  // ========== 고아를 제외한 savedLinks (ALL 화면용) ==========
  const savedLinksWithoutOrphans = useMemo(() => {
    ...
    return savedLinks.filter(link => {
      // FM이 유효한지 확인 (고아 FM 제외)
      if (!validFmIds.has(link.fmId)) return false;
      // FE/FC가 빈 값이면 허용, 있으면 유효한지 확인
      if (link.feId && link.feId.trim() !== '' && !validFeIds.has(link.feId)) return false;
      if (link.fcId && link.fcId.trim() !== '' && !validFcIds.has(link.fcId)) return false;
      return true;
    });
  }, [savedLinks, fmData, feData, fcData]);
  ...
  const handleDeleteOrphanFM = useCallback((fmId: string, fmText: string) => {
    ...
    console.log('[고아 FM 삭제 확인]', {
      삭제할FM_ID: fmId,
      삭제할FM_텍스트: fmText,
      삭제될_링크수: linksToDelete.length,
      다른고아FM: uniqueOtherOrphans,
    });
    ...
    if (!window.confirm(
      ...
      `연결된 모든 FE/FC 연결도 삭제됩니다.`
    )) {
      return;
    }
    
    // savedLinks에서 해당 fmId를 가진 모든 링크만 삭제
    setSavedLinks(prev => {
      const filtered = prev.filter(link => link.fmId !== fmId);
      const deletedCount = prev.length - filtered.length;
      console.log('[고아 FM 삭제 완료]', fmId, '→', deletedCount, '개 링크 삭제됨');
      
      // 검증: 다른 고아 FM 링크는 남아있어야 함
      const remainingOrphans = filtered.filter(link => orphanIds.orphanFmIds.has(link.fmId));
      console.log('[검증] 남은 고아 FM 링크:', remainingOrphans.length);
      
      return filtered;
    });
  }, [savedLinks, orphanIds.orphanFmIds]);

  // ========== 고아 FM 복구 핸들러 ==========
  const handleRestoreOrphanFM = useCallback((fmId: string, fmText: string, fmProcess: string) => {
    if (!window.confirm(`고아 FM "${fmText}"를 복구하시겠습니까?\n(${fmProcess} 공정의 고장형태로 복구됩니다)`)) {
      return;
    }
    
    // savedLinks에서 해당 FM 정보 찾기
    ...
    const updateFn = (prev: any) => {
      const newL2 = (prev.l2 || []).map((proc: any) => {
        ...
        if (existingFm) {
          console.log('[복구] FM이 이미 존재함:', fmId);
          return proc;
        }
        ...
        console.log('[복구] FM 추가:', newFm, '→ 공정:', fmProcess);
  ...
  const handleConfirmAll = useCallback(() => {
    ...
    // ✅ 2026-01-19: 변경 히스토리 기록 + 자동 백업 (고장연결 확정)
    ...
    try {
      if (fmeaId) {
        ...
        console.log('[변경 히스토리] 고장연결 확정 기록됨:', fmeaId);
        ...
        setTimeout(async () => {
          try {
            ...
            if (savedData) {
              ...
              console.log('[자동 백업] 고장연결 확정 백업 완료:', version);
            }
          } catch (backupErr) {
      ...
      <div style={rightPanelStyle}>
        ...
        <div className="flex-1 overflow-auto" style={{ paddingBottom: '50px' }}>
          ...
          {viewMode === 'result' && (
            ...
            <FailureLinkResult savedLinks={savedLinksWithoutOrphans} fmData={fmData} />
          )}
        </div>
      </div>
    </div>
