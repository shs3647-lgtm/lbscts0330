...
export default function OptTabConfirmable({ 
  ...
  useEffect(() => {
    ...
    if (optDataRef.current && dataKey !== optDataRef.current) {
      ...
      saveTimerRef.current = setTimeout(() => {
        saveAtomicDB?.();
        console.log('[OptTab] DB 자동 저장 완료');
      }, 500);
    }
  ...
  const failureCauses = React.useMemo(() => 
    state.l2.flatMap(proc => (proc as any).failureCauses || [])
  , [state.l2]);
  ...
  const handleConfirm = useCallback(() => {
    ...
    try {
      if (fmeaId) {
        ...
        console.log('[변경 히스토리] 6ST 확정 기록됨:', fmeaId);
      }
    } catch (e) {
      ...
      try {
        ...
        if (fmeaId) {
          ...
          if (savedData) {
            ...
            console.log('[자동 백업] 6ST 확정 백업 완료:', version);
          }
        }
      } catch (error) {
