...
export default function FailureChainPanel({ state }: FailureChainPanelProps) {
  ...
  const fmList = useMemo(() => {
    ...
    failureLinks.forEach((link: any) => {
      if (!link.fmId || !link.fmText) return;
      if (fmMap.has(link.fmId)) return;
  ...
  React.useEffect(() => {
    ...
    } else if (selectedFmId && !fmList.find(fm => fm.id === selectedFmId)) {
      // 선택된 FM이 목록에 없으면 첫 번째로 변경
      setSelectedFmId(fmList[0]?.id || null);
    }
  }, [fmList, selectedFmId]);
        ...
        <div className="flex gap-2 min-h-full">
          ...
          <div className="flex-1 flex flex-col gap-2">
            ...
            {linkedData.fes.map(fe => (
              <div
                key={fe.id}
                className="p-2 border rounded text-xs bg-blue-50 border-blue-200"
              >
          ...
          <div className="flex-1 flex flex-col gap-2">
            ...
            {linkedData.fcs.map(fc => (
              <div
                key={fc.id}
                className="p-2 border rounded text-xs bg-green-50 border-green-200"
              >
