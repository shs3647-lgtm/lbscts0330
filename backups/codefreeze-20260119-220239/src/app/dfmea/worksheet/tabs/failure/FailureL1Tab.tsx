...
export default function FailureL1Tab({ state, setState, setStateSynced, setDirty, saveToLocalStorage, saveAtomicDB }: FailureTabProps) {
  ...
  const handleDoubleClickEdit = useCallback((effectId: string, newValue: string) => {
    ...
    setTimeout(async () => {
      saveToLocalStorage?.();
      if (saveAtomicDB) {
        try { await saveAtomicDB(); } catch (e) { console.error('[FailureL1Tab DFMEA] 수정 후 DB 저장 오류:', e); }
      }
    }, 100);
  ...
  const updateSeverity = useCallback((effectId: string, severity: number | undefined) => {
    const updateFn = (prev: any) => {
      ...
      if (autoLinkedCount > 0) {
        console.log(`[FailureL1Tab DFMEA] 심각도 자동연결: "${effectName}" → ${autoLinkedCount}건에 심각도 ${severity} 적용`);
      }
    ...
    setTimeout(async () => {
      saveToLocalStorage?.();
      if (saveAtomicDB) {
        try { await saveAtomicDB(); } catch (e) { console.error('[FailureL1Tab DFMEA] 심각도 DB 저장 오류:', e); }
      }
    }, 100);
  }, [setState, setStateSynced, setDirty, saveToLocalStorage, saveAtomicDB]);
  ...
  const deleteRow = useCallback((effectId: string) => {
    ...
    setTimeout(async () => {
      saveToLocalStorage?.();
      if (saveAtomicDB) {
        try { await saveAtomicDB(); console.log('[FailureL1Tab DFMEA] 삭제 후 DB 저장 완료'); } catch (e) { console.error('[FailureL1Tab DFMEA] 삭제 후 DB 저장 오류:', e); }
      }
    }, 200);
  }, [setState, setStateSynced, setDirty, saveToLocalStorage, saveAtomicDB]);
