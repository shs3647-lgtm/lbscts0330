...
export default function FunctionL1Tab({ state, setState, setStateSynced, setDirty, saveToLocalStorage, saveAtomicDB }: FunctionTabProps) {
  ...
  const handleInlineEditRequirement = useCallback((typeId: string, funcId: string, reqId: string, newValue: string) => {
    ...
    setTimeout(async () => {
      saveToLocalStorage?.();
      if (saveAtomicDB) {
        try { await saveAtomicDB(); } catch (e) { console.error('[FunctionL1Tab DFMEA] 요구사항 편집 DB 저장 오류:', e); }
      }
    }, 100);
  ...
  const handleInlineEditFunction = useCallback((typeId: string, funcId: string, newValue: string) => {
    ...
    setTimeout(async () => {
      saveToLocalStorage?.();
      if (saveAtomicDB) {
        try { await saveAtomicDB(); } catch (e) { console.error('[FunctionL1Tab DFMEA] 기능 편집 DB 저장 오류:', e); }
      }
    }, 100);
  }, [setState, setStateSynced, setDirty, saveToLocalStorage, saveAtomicDB]);
  ...
  const handleDelete = useCallback((deletedValues: string[]) => {
    console.log('[FunctionL1Tab DFMEA] handleDelete 호출됨');
    console.log('[FunctionL1Tab DFMEA] deletedValues:', deletedValues);
    console.log('[FunctionL1Tab DFMEA] modal:', modal);
    
    if (!modal) {
      console.error('[FunctionL1Tab DFMEA] modal이 없음!');
      return;
    }
    
    const { type, id } = modal;
    const deletedSet = new Set(deletedValues);
    console.log('[FunctionL1Tab DFMEA] type:', type, 'id:', id);
    ...
    const updateFn = (prev: any) => {
      ...
      if (type === 'l1Type') {
        ...
        console.log('[FunctionL1Tab DFMEA] l1Type 삭제, 이전 types:', newState.l1.types.map((t: any) => t.name));
        newState.l1.types = newState.l1.types.filter((t: any) => !deletedSet.has(t.name));
        console.log('[FunctionL1Tab DFMEA] l1Type 삭제 후 types:', newState.l1.types.map((t: any) => t.name));
      } 
      else if (type === 'l1Function') {
        // 제품 기능 삭제 - 모든 타입에서 삭제 (id가 비어있을 수 있음) (DFMEA)
        console.log('[FunctionL1Tab DFMEA] l1Function 삭제');
        newState.l1.types = newState.l1.types.map((t: any) => {
          ...
          console.log(`[FunctionL1Tab DFMEA] type ${t.name}: ${beforeCount} -> ${newFunctions.length} functions`);
          return { ...t, functions: newFunctions };
        });
      }
      else if (type === 'l1Requirement') {
        // 요구사항 삭제
        console.log('[FunctionL1Tab DFMEA] l1Requirement 삭제');
        newState.l1.types = newState.l1.types.map((t: any) => ({
          ...t,
          functions: t.functions.map((f: any) => {
            ...
            console.log(`[FunctionL1Tab DFMEA] function ${f.name}: ${beforeCount} -> ${newReqs.length} requirements`);
            return { ...f, requirements: newReqs };
          })
        }));
      }
      
      console.log('[FunctionL1Tab DFMEA] 새 상태 반환');
      return newState;
    };
    ...
    console.log('[FunctionL1Tab DFMEA] 저장 실행');
    setTimeout(async () => {
      saveToLocalStorage?.();
      if (saveAtomicDB) {
        try {
          await saveAtomicDB();
          console.log('[FunctionL1Tab DFMEA] 삭제 후 DB 저장 완료');
        } catch (e) {
          console.error('[FunctionL1Tab DFMEA] 삭제 후 DB 저장 오류:', e);
        }
      }
    }, 200);
