 ...
 * dfm26-M001 → M, dfm26-F001 → F, dfm26-P001 → P
 */
function extractFmeaType(id: string): { code: string; label: string; color: string } {
  const match = id.match(/dfm\d{2}-([MFP])/i);
 ...
 * 예시: dfm26-M001 (Master), dfm26-F001 (Family), dfm26-P001 (Part)
 */
...
export default function FMEAListPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (result.success && result.projects.length > 0) {
        ...
        const storedPfmea = localStorage.getItem('dfmea-projects');
  ...
  const handleDeleteSelected = () => {
    ...
    localStorage.setItem('dfmea-projects', JSON.stringify(remaining.filter(p => p.id.includes('DFMEA'))));
    localStorage.setItem('fmea-projects', JSON.stringify(remaining.filter(p => !p.id.includes('DFMEA'))));
  ...
  const handleEditSelected = () => {
    ...
    window.location.href = `/dfmea/register?id=${selectedId}`;
  };

  // 등록화면으로 이동 (CFT 섹션으로 스크롤)
  const handleOpenRegisterPage = (fmeaId: string, section?: 'cft') => {
    const url = section === 'cft' 
      ? `/dfmea/register?id=${fmeaId}#cft-section`
      : `/dfmea/register?id=${fmeaId}`;
    ...
    <>
      ...
      <div className="min-h-screen bg-[#f0f0f0] px-3 py-3 pt-9 font-[Malgun_Gothic]">
        ...
        <div className="flex items-center gap-2 mb-4">
          ...
          <h1 className="text-base font-bold text-gray-800">D-FMEA 리스트</h1>
          {isLoading ? (
            ...
            {filteredProjects.map((p, index) => (
              ...
              >
                ...
                <td className="border border-gray-400 px-2 py-1 text-center align-middle font-semibold text-blue-600">
                  <a href={`/dfmea/register?id=${p.id.toLowerCase()}`} className="hover:underline">
                ...
                <td className="border border-gray-400 px-2 py-1 text-center align-middle">
                  {p.parentFmeaId ? (
                    <a 
                      href={`/dfmea/register?id=${p.parentFmeaId.toLowerCase()}`} 
                ...
                <td className="border border-gray-400 px-2 py-1 text-left align-middle">
                  {(() => {
                    ...
                    return (
                      <a 
                        href={`/dfmea/register?id=${p.id.toLowerCase()}`} 
