    ...
    <div className="min-h-screen bg-gray-100 py-8 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-[#00587a] text-white rounded-t-lg p-4 flex justify-between items-center">
          <div>
            <h1 className="text-xl font-bold">⚙️ 결재 시스템 환경설정</h1>
            <p className="text-sm text-gray-200">관리자 전용 - SMTP, 결재자 지정, 알림 설정</p>
          </div>
          <button
        ...
        <div className="bg-white border-b flex">
          {[
            { id: 'smtp', label: '📧 SMTP 설정', desc: '이메일 서버' },
            { id: 'autoAssign', label: '👥 결재자 지정', desc: '자동 지정 규칙' },
            { id: 'notification', label: '🔔 알림 설정', desc: '알림 방식' },
            { id: 'groupware', label: '🏢 그룹웨어 연동', desc: '외부 시스템' },
          ].map(tab => (
            <button
              key={tab.id}
        ...
        <div className="bg-white shadow-lg p-6 rounded-b-lg">
          ...
          {activeTab === 'smtp' && (
            <div className="space-y-4">
              <div className="flex items-center justify-between pb-4 border-b">
                <div>
                  <h2 className="font-bold text-lg">SMTP 이메일 설정</h2>
                  <p className="text-sm text-gray-500">결재 요청 이메일 발송을 위한 SMTP 서버 설정</p>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-semibold mb-1">SMTP 호스트</label>
                  <select
                  ...
                  {settings.smtp.host && !['smtp.gmail.com', 'smtp.naver.com', 'smtp.kakao.com', 'smtp.office365.com'].includes(settings.smtp.host) && (
                    <input
                      type="text"
                      value={settings.smtp.host}
                ...
                <div>
                  <label className="block text-sm font-semibold mb-1">포트</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-sm font-semibold mb-1">사용자 (이메일)</label>
                  <input
                    type="email"
                ...
                <div>
                  <label className="block text-sm font-semibold mb-1">비밀번호 (앱 비밀번호)</label>
                  <input
                    type="password"
                ...
                <div>
                  <label className="block text-sm font-semibold mb-1">발신자 이름</label>
                  <input
                    type="text"
          ...
          {activeTab === 'autoAssign' && (
            <div className="space-y-4">
              <div className="flex items-center justify-between pb-4 border-b">
                <div>
                  <h2 className="font-bold text-lg">결재자 자동 지정 규칙</h2>
                  <p className="text-sm text-gray-500">FMEA 등록정보에서 결재자를 자동으로 가져옵니다</p>
                </div>
                <label className="flex items-center gap-2">
                  <input
                    type="checkbox"
              ...
              <div className="space-y-4">
                <div className="p-4 bg-blue-50 rounded-lg">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <div className="flex-1">
                      <label className="block text-sm font-semibold">작성자 역할</label>
                      <input
                        type="text"
                ...
                <div className="p-4 bg-yellow-50 rounded-lg">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <div className="flex-1">
                      <label className="block text-sm font-semibold">검토자 역할</label>
                      <input
                        type="text"
                ...
                <div className="p-4 bg-green-50 rounded-lg">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <div className="flex-1">
                      <label className="block text-sm font-semibold">승인자 역할</label>
                      <input
                        type="text"
              ...
              <div className="p-4 bg-gray-100 rounded-lg">
                <h3 className="font-semibold mb-2">📋 결재 흐름</h3>
                <div className="flex items-center justify-center gap-4 text-sm">
                  <span className="px-3 py-1 bg-blue-200 rounded">작성자 승인</span>
                  <span>→</span>
                  <span className="px-3 py-1 bg-yellow-200 rounded">검토자 승인</span>
                  <span>→</span>
                  <span className="px-3 py-1 bg-green-200 rounded">승인자 최종승인</span>
                </div>
              </div>
            </div>
          )}
          ...
          {activeTab === 'notification' && (
            <div className="space-y-4">
              <div className="pb-4 border-b">
                ...
                <p className="text-sm text-gray-500">결재 요청/결과 알림 방식을 설정합니다</p>
              </div>
              
              <div className="space-y-4">
                <label className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                  <div>
                    <span className="font-semibold">📧 이메일 알림</span>
                    <p className="text-sm text-gray-500">결재 요청/승인/반려 시 이메일 발송</p>
                  </div>
                  <input
                    type="checkbox"
                ...
                <label className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                  <div>
                    <span className="font-semibold">🔔 시스템 내 알림</span>
                    <p className="text-sm text-gray-500">FMEA 시스템 내 알림 표시</p>
                  </div>
                  <input
                    type="checkbox"
                ...
                <div className="p-4 bg-gray-50 rounded-lg">
                  <label className="block font-semibold mb-2">⏰ 리마인더 발송</label>
          ...
          {activeTab === 'groupware' && (
            <div className="space-y-4">
              <div className="pb-4 border-b">
                <h2 className="font-bold text-lg">그룹웨어 연동</h2>
                <p className="text-sm text-gray-500">기업 그룹웨어와 결재 시스템을 연동합니다 (선택사항)</p>
              </div>
              
              <div>
                <label className="block text-sm font-semibold mb-2">그룹웨어 유형</label>
                <select
                  value={settings.groupware.type}
                  onChange={(e) => updateGroupware('type', e.target.value)}
                  className="w-full p-2 border rounded"
                >
                  <option value="none">사용 안함 (이메일만 사용)</option>
                  ...
                  <option value="custom">직접 설정 (API 연동)</option>
                </select>
              </div>
        ...
        <div className="mt-4 p-4 bg-blue-50 rounded-lg text-sm">
          <h3 className="font-bold mb-2">💡 설정 가이드</h3>
