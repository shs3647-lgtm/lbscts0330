...
| 2026-03-22 | - | **Import 미리보기 타입 정리**: `TemplatePreviewContent`의 `previewLevel`에서 미사용 `'FC'` 제거 — 실제 상태는 `TemplateGeneratorPanel`의 `'L1'|'L2'|'L3'`만 사용, 고장사슬은 `activeStep === 'FC'`. `npx tsc --noEmit` 0건 복구 | Claude |
...
| 2026-03-23 | - | **파이프라인·FK CLI 검증**: `scripts/verify-location-fk-baseline.ts`, `npm run verify:pipeline-baseline` / `:strict` (`GET` pipeline-verify, `--baseline` 시 m002 골든). 문서: `docs/LOCATION_FK_SNAPSHOT_AND_FREEZE.md` §3, `docs/MAINTENANCE_MANUAL.md` §2.5 | Claude |
...
| 2026-03-22 | - | **출시 Forge 감사**: `docs/FMEA_RELEASE_FORGE_SIDEBAR_AUDIT_REPORT.md` — FMEA Core 사이드바 전 메뉴·연동 경로·`Sidebar` 이중 정의 리스크. `npm run audit:sidebar-routes` (`scripts/audit-fmea-sidebar-routes.ts`) | Claude |
| 2026-03-22 | - | **사이드바 SSoT 순차 개선**: `fmea-core-sidebar-menu.tsx`, `release:audit`, `tests/e2e/sidebar-fmea-core-routes-smoke.spec.ts`, 감사 스크립트가 SSoT에서 href 수집 | Claude |
...
| 03-24 | - | Import/연결표 | **UI 연결표(Linkage) vs FC_고장사슬 불일치**: `findFCSheet`가 `연결표` 시트명을 인식 못 해 FC 파싱 0건 → 메인시트 도출 체인과 불일치. **해결**: `excel-parser-fc`에 10열 연결표 레이아웃 감지·`parseLinkageExportSheet`, `findFCSheet`에 linkage 시트 패턴. **보내기**: `excel-export-linkage` Process 열에 `fmProcessNo`·공정명 보강. 스크립트 `mergeWithLinkageTable` Process carry. 테스트: `linkage-sheet-parse.test.ts` |
| 03-24 | - | Import/FA검증바 | **M1/M8 명세-수량 완화**: VERIFY수식 체인건수 vs 파싱 `chainCount`가 통합시트·`supplementChainsFromFlatData` 등에서 1:1이 아닐 수 있음 → **기대>0이고 파싱>0이면 행#1·#8은 FAIL 배너·NG 제외** (`faVerificationSpecRelax.ts`, `FAVerificationBar`). 연결 품질은 행5~7·미매칭 패널. 테스트: `fa-verification-spec-relax.test.ts` |
