...
- 산업 DB(`KrIndustry*`), LLD(`LLDFilterCode`), m066 시딩과의 **우선순위 조합**
- 누락 시 경고 코드(`M066_MISSING`) 및 UI 정책
...
| 순위 | 소스 | 설명 | 비고 |
|------|------|------|------|
| 기준 시딩 | **m066 Atomic DB** | 91 WE, 104 FC, 26 FM, 20 FE, 103 L3Function 등 **골든 레퍼런스** 규모로 Master 행을 초기 적재 | 프로젝트 ID 예: `pfm26-m066` |
...
| 필드 | 타입 | 설명 |
|------|------|------|
| `sourceProject` | `String` | 유래 프로젝트 ID (기본 예: `pfm26-m066`) |
| `sourceType` | `String` | `m066` \| `import` \| `manual` \| `lld` \| `industry` 등 |
...
DB 조회 (프로젝트 Atomic → Master → 산업 → LLD)
        ↓ 실패
경고 코드: M066_MISSING (또는 범용 MISSING_MASTER)
        ↓
...
- **트리거**: `M066_MISSING`에 대해 사용자가 삭제를 선택하거나, 타임아웃·배치 정리 정책으로 무응답이 확정된 경우.
...
| 코드 | 의미 | 권장 HTTP/UI |
|------|------|----------------|
| `M066_MISSING` | 골든/ Master / 허용 소스에서 필수 필드를 채울 수 없음 | 404 또는 200 + `warnings[]` |
...
- **목표**: m066 기준 시딩 및 동기화 완료 후 **100%** 커버리지 (골든 베이스라인과 일치)

### 9.2 누락 경고 리포트

- 기간별 `M066_MISSING` (또는 통합 MISSING) 발생 건수
...
2. m066 Atomic 스냅샷에서 배치 시딩 스크립트 실행 → 골든 레퍼런스 적재.
...
| **Golden Reference** | m066 등 검증된 기준 데이터 집합 |
...
```json
{
  "ok": false,
  "matchTier": null,
  "warnings": [{ "code": "M066_MISSING", "message": "No master row for given keys" }]
}
```
...
{
  "activeRows": 1200,
  "bySourceType": { "m066": 400, "import": 500, "manual": 200, "lld": 80, "industry": 20 },
  "coverage": { "projectId": "pfm26-m066", "matchedWe": 91, "totalWe": 91, "rate": 1.0 }
}
```
