/**
 * @file help/content/pfd-register.tsx
 * @description PFD(공정흐름도) 등록화면 도움말 콘텐츠
 * @module help
 */
...
export const PFD_REGISTER_META: HelpManualMeta = {
  title: 'PFD 등록화면 도움말',
  module: 'PFD Register',
  version: 'v1.0',
...
function OverviewSection() {
  return (
    <div className="space-y-2">
      <p className="font-bold text-yellow-800">PFD(Process Flow Diagram) 등록화면은 공정흐름도의 기본 정보를 등록하고 관리하는 화면입니다.</p>
      <div className="grid grid-cols-2 gap-2">
        <div className="bg-white rounded p-2 border border-yellow-200">
          ...
          <ul className="space-y-0.5 text-[10px]">
            <li>- PFD 프로젝트 신규 등록 / 수정</li>
            <li>- PFD 유형(M/F/P) 선택 및 ID 자동 부여</li>
            <li>- PFMEA / CP / APQP 문서 연동</li>
        ...
        <div className="bg-white rounded p-2 border border-yellow-200">
          ...
          <ul className="space-y-0.5 text-[10px]">
            <li><span className="font-bold text-green-600">새로 작성</span> — 새 PFD 등록 (Triplet 세트 자동 생성)</li>
            <li><span className="font-bold text-yellow-600">편집</span> — 기존 PFD 불러오기 / 수정모드</li>
...
function FieldsSection() {
  return (
    <div className="space-y-2">
      <p className="font-bold text-yellow-800">PFD 등록 필드 설명</p>
      <table className="w-full text-[10px] border-collapse">
        ...
        <tbody>
          {[
            ['PFD 유형', 'M(Master), F(Family), P(Part) 중 선택. Triplet 유형과 동일합니다.', 'Y'],
            ['PFD명(Subject)', '공정흐름도 명칭. Triplet 생성 시 자동 입력됩니다.', 'Y'],
            ['PFD ID', '자동 생성된 고유 식별자 (예: pfd26-m001).', '-'],
            ['연동 PFMEA', 'Triplet으로 연결된 PFMEA. 배지 클릭 시 이동합니다.', '-'],
            ['연동 CP', 'Triplet으로 연결된 CP. 배지 클릭 시 이동합니다.', '-'],
    ...
    <div className="space-y-2">
      ...
      <div className="bg-white rounded p-2 border border-yellow-200">
        <div className="flex items-center gap-2 text-[10px] mb-2">
          <span className="px-2 py-1 bg-purple-100 text-purple-700 rounded font-bold">Master PFD</span>
          <span className="text-gray-400">&rarr;</span>
          <span className="px-2 py-1 bg-blue-100 text-blue-700 rounded font-bold">Family PFD</span>
          <span className="text-gray-400">&rarr;</span>
          <span className="px-2 py-1 bg-green-100 text-green-700 rounded font-bold">Part PFD</span>
        </div>
        <table className="w-full text-[10px] border-collapse">
          <tbody>
            {([
              ['M', 'bg-purple-50', 'text-purple-700', '표준 공정흐름도 (최상위 기준, FMEA Master와 연동)'],
              ['F', 'bg-blue-50', 'text-blue-700', '제품군 단위 공정흐름도 (FMEA Family와 연동)'],
              ['P', 'bg-green-50', 'text-green-700', '개별 부품 공정흐름도 (FMEA Part와 연동, 양산용)'],
      ...
      <div className="bg-blue-50 rounded p-2 border border-blue-200">
        <p className="font-bold text-[10px] text-blue-700 mb-1">PFD ID 형식</p>
        <table className="w-full text-[9px] border-collapse bg-white">
          ...
          <tbody>
            <tr className="bg-purple-50">
              ...
              <td className="border border-blue-200 px-1.5 py-0.5 font-mono">pfd{'{yy}'}-m{'{NNN}'}</td>
              <td className="border border-blue-200 px-1.5 py-0.5 font-mono text-purple-700">pfd26-m001</td>
            </tr>
            <tr className="bg-blue-50">
              <td className="border border-blue-200 px-1.5 py-0.5 font-bold">Family</td>
              <td className="border border-blue-200 px-1.5 py-0.5 font-mono">pfd{'{yy}'}-f{'{NNN}'}</td>
              <td className="border border-blue-200 px-1.5 py-0.5 font-mono text-blue-700">pfd26-f001</td>
            </tr>
            <tr className="bg-green-50">
              <td className="border border-blue-200 px-1.5 py-0.5 font-bold">Part</td>
              <td className="border border-blue-200 px-1.5 py-0.5 font-mono">pfd{'{yy}'}-p{'{NNN}'}-i{'{GG}'}</td>
              <td className="border border-blue-200 px-1.5 py-0.5 font-mono text-green-700">pfd26-p001-i01</td>
            </tr>
          </tbody>
        </table>
      </div>
...
function LinkageSection() {
  return (
    <div className="space-y-2">
      ...
      <p className="text-[10px] text-gray-600">PFD는 PFMEA, CP와 <strong>Triplet(3중 세트)</strong>로 자동 연동됩니다.</p>
      ...
      <div className="bg-indigo-50 rounded p-2 border border-indigo-200">
        ...
        <p className="text-[9px] text-indigo-600 mt-1">* PFD 화면에서 &quot;새로 작성&quot; → FMEA+CP도 함께 생성됩니다.</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <div className="bg-white rounded p-2 border border-yellow-200">
          <p className="font-bold text-[10px] text-teal-700 mb-1">연동 CP</p>
          <ul className="space-y-0.5 text-[10px]">
            <li>- Triplet으로 자동 연결된 CP 표시</li>
            <li>- 배지 클릭 시 CP 등록화면으로 이동</li>
...
function WorkflowSection() {
  return (
    <div className="space-y-2">
      <p className="font-bold text-yellow-800">PFD 등록 작업 흐름</p>
      <div className="bg-white rounded p-2 border border-yellow-200">
        <div className="space-y-2 text-[10px]">
          {[
            { step: '1', title: '신규 등록 (Triplet)', desc: '새로 작성 클릭 → M/F/P 유형 선택 → FMEA+CP+PFD 세트 자동 생성 → PFD 등록화면 이동', color: 'bg-green-100 text-green-700' },
            { step: '2', title: '기본정보 입력', desc: 'PFD명, 담당자, 고객명, 회사명, 품명/품번, 일자 등 입력', color: 'bg-blue-100 text-blue-700' },
            { step: '3', title: '연동 확인', desc: 'Triplet으로 자동 생성된 PFMEA/CP 확인', color: 'bg-purple-100 text-purple-700' },
            { step: '4', title: 'CFT 구성', desc: 'CFT 멤버 등록 — 역할 지정 및 사용자 검색', color: 'bg-orange-100 text-orange-700' },
            { step: '5', title: '저장', desc: '저장 클릭 → DB에 PFD 정보 저장', color: 'bg-indigo-100 text-indigo-700' },
            { step: '6', title: '공정 흐름 작성', desc: 'PFD 워크시트에서 공정 흐름도 작성', color: 'bg-amber-100 text-amber-700' },
          ].map(item => (
      ...
      <div className="bg-red-50 rounded p-2 border border-red-200">
        ...
        <ul className="space-y-0.5 text-[10px] text-red-600">
          ...
          <li>- PFD 유형 변경 시 ID가 재생성되므로 주의하세요</li>
          <li>- Triplet 생성 시 PFMEA/CP가 함께 자동 생성됩니다</li>
        </ul>
      </div>
    </div>
  );
...
export const PFD_REGISTER_SECTIONS: HelpSectionDef[] = [
