...
const _AP_TABLE: { sMin: number; sMax: number; oMin: number; oMax: number; d: APLevel[] }[] = [
...
export const calculateAP5 = (
  ...
  const dIdx = detection >= 7 ? 0 : detection >= 5 ? 1 : detection >= 2 ? 2 : 3;
  for (const row of _AP_TABLE) {
    if (severity >= row.sMin && severity <= row.sMax && occurrence >= row.oMin && occurrence <= row.oMax) {
      return row.d[dIdx];
    }
  }
  return 'L';
