 ...
 */
import { WorksheetState } from '@/app/(fmea-core)/pfmea/worksheet/constants';
...
export async function triggerAutoBackup(
  ...
  try {
    ...
    // 3. DB 스냅샷 백업 (확정 시 자동)
    await createSnapshot(fmeaId, state, 'AUTO_CONFIRM', `${fmeaName} 확정 자동백업`).catch(
      (e: Error) => console.error('[백업] 스냅샷 생성 실패:', e.message)
    );
