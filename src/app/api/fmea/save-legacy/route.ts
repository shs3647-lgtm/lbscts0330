...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.error('❌ 레거시 저장 오류:', error instanceof Error ? error.message : error);
    return NextResponse.json({
      success: false,
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    console.error('❌ 레거시 조회 오류:', error instanceof Error ? error.message : error);
    return NextResponse.json({
      success: false,
      error: safeErrorMessage(error)
    }, { status: 500 });
