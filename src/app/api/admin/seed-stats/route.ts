...
export async function POST(req: NextRequest) {
    try {
        ...
        const specialCharacteristics = [
            { code: 'CC', name: 'Critical Characteristic', symbol: '◆', description: '규정된 차량안전, 배기가스관련 또는 사고예방에 관련된 특성', customer: null, color: '#FF0000', sortOrder: 1 },
            { code: 'SC', name: 'Significant Characteristic', symbol: '★', description: '기능, 성능, 내구성 또는 외관과 관련하여 고객에게 중대한 불만족을 야기할 수 있는 특성', customer: null, color: '#FF8C00', sortOrder: 2 },
            { code: 'IC', name: 'Important Characteristic', symbol: '●', description: '고객 요구사항, 피팅, 기능, 제조, 조립 또는 서비스에 중요한 특성', customer: null, color: '#0066CC', sortOrder: 3 },
            { code: 'HMC_CC', name: '중요특성 (현대차)', symbol: '♦', description: '현대자동차 중요특성 - 안전/배기 관련', customer: '현대자동차', color: '#FF0000', sortOrder: 10 },
            { code: 'HMC_SC', name: '주요특성 (현대차)', symbol: '☆', description: '현대자동차 주요특성 - 기능/성능 관련', customer: '현대자동차', color: '#FF8C00', sortOrder: 11 },
        ];
        ...
        results['specialCharacteristicCriteria'] = specialCharacteristics.length;
        ...
        results['evalMethodLibrary'] = evalMethods.length;
        ...
        results['controlMethodLibrary'] = controlMethods.length;
        ...
        results['reactionPlanTemplate'] = reactionPlans.length;

        return NextResponse.json({
            success: true,
            message: '통계/마스터 테이블 시드 완료',
            results,
        });
    } catch (error: any) {
...
export async function GET() {
    return NextResponse.json({
        message: 'POST 요청으로 시드를 실행하세요',
        endpoints: {
            'POST /api/admin/seed-stats': '통계/마스터 테이블 시드 실행',
        },
    });
}
