...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const isMaster = searchParams.get('isMaster'); // 'true' | 'false' | null
    const search = searchParams.get('search'); // 검색어 (장치명, 공정명)

    const where: any = { isActive: true };

    // 마스터/CP별 구분 (cpNo가 'M' 또는 'MASTER' 또는 null이면 마스터)
    const MASTER_MARKERS = ['M', 'MASTER', null];
    
    if (isMaster === 'true') {
      // 마스터만 조회: cpNo가 'M', 'MASTER', 또는 null
      where.OR = [
        { cpNo: null },
        { cpNo: 'M' },
        { cpNo: 'MASTER' },
      ...
      // isMaster 미지정: cpNo/fmeaId로 필터 + 마스터 포함
      if (cpNo || fmeaId) {
        const orConditions: any[] = [
          { cpNo: null },     // 마스터 (null)
          { cpNo: 'M' },      // 마스터 ('M')
          { cpNo: 'MASTER' }, // 마스터 ('MASTER')
        ];
        if (cpNo) orConditions.push({ cpNo });
        if (fmeaId) orConditions.push({ fmeaId });
    ...
    // 마스터/CP별 구분 추가 (cpNo가 null, 'M', 'MASTER'이면 마스터)
    const isMasterDevice = (cpNo: string | null) => 
      cpNo === null || cpNo === 'M' || cpNo === 'MASTER';
    
    const devicesWithMaster = devices.map((d: any) => ({
      ...d,
      isMaster: isMasterDevice(d.cpNo),
    }));

    return NextResponse.json({
      success: true,
      data: devicesWithMaster,
      count: devices.length,
      masterCount: devices.filter((d: any) => isMasterDevice(d.cpNo)).length,
      cpCount: devices.filter((d: any) => !isMasterDevice(d.cpNo)).length,
    });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { devices, cpNo, fmeaId, replaceAll, isMaster = false } = body;
    ...
    if (replaceAll) {
      ...
      if (isMaster) {
        ...
        else if (fmeaId) deleteWhere.fmeaId = fmeaId;
      }
    ...
    // 새 데이터 저장 (마스터는 cpNo='M'으로 저장)
    const createdDevices = await prisma.epDevice.createMany({
      data: validDevices.map((d: any, idx: number) => ({
        cpNo: isMaster ? 'M' : (cpNo || d.cpNo || null),
        fmeaId: isMaster ? null : (fmeaId || d.fmeaId || null),
    ...
    // 저장된 데이터 다시 조회
    const queryWhere: any = { isActive: true };
    if (isMaster) {
      queryWhere.OR = [
        { cpNo: null },
        { cpNo: 'M' },
      ...
      if (cpNo) queryWhere.cpNo = cpNo;
      if (fmeaId) queryWhere.fmeaId = fmeaId;
    }
    ...
    // isMaster 필드 추가
    const isMasterDevice = (deviceCpNo: string | null) => 
      deviceCpNo === null || deviceCpNo === 'M' || deviceCpNo === 'MASTER';
    
    const result = savedDevices.map((d: any) => ({
      ...d,
      isMaster: isMasterDevice(d.cpNo),
    }));
