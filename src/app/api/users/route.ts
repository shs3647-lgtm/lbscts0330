  ...
  try {
    if (userId) {
      ...
      const user = await prisma.user.findUnique({
        where: { id: userId },
        select: {
          id: true, factory: true, department: true, name: true, position: true,
          phone: true, email: true, photoUrl: true, remark: true, role: true,
          permPfmea: true, permDfmea: true, permCp: true, permPfd: true,
      ...
      // ★ 사용자 목록은 자주 변하지 않는 참조 데이터 → 1분 캐시
      return NextResponse.json({
        success: true,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { factory, department, name, position, phone, email, photoUrl, remark, role, password, permPfmea, permDfmea, permCp, permPfd } = body;
...
export async function PUT(request: NextRequest) {
  ...
  try {
    const body = await request.json();
    const { id, factory, department, name, position, phone, email, photoUrl, remark, role, isActive, password, permPfmea, permDfmea, permCp, permPfd } = body;
    ...
    // ✅ 2026-01-19: 비밀번호 해시 (변경 시에만, bcrypt 사용)
