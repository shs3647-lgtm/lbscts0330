 ...
 * ██  PFD 연동 근본 수정 완료. 프로젝트 스키마에 pfd_registration 동기화 ██
 * ██  + pfdItem을 projectPrisma에 저장하는 핵심 로직 보호.              ██
 * ██  수정 시 반드시 사용자 승인 필수.                                   ██
 * ██████████████████████████████████████████████████████████████████████████
 *
...
export async function POST(request: NextRequest) {
    try {
                    ...
                    for (const l2Fn of l2Functions) {
                        ...
                        const savedItem = await tx.pfdItem.create({
                            data: {
                                ...
                                processName: l2.name || '',
                                processLevel,
                                processDesc: l2Fn.functionName || l2Fn.productChar || l2.name || '',
                                partName: '',
                                workElement: '',
                        ...
                        for (const l3Fn of l3Functions) {
                            ...
                            const savedItem = await tx.pfdItem.create({
                                data: {
                                    ...
                                    processName: l2.name || '',
                                    processLevel,
                                    processDesc: l3Fn.functionName || l3Fn.processChar || l3.name || l2.name || '',
                                    partName: l3.name || '',
                                    workElement: l3.name || '',
                                    equipment: equipmentStr,
                                    equipmentM4: equipmentM4Str,
        ...
        try {
            const fmeaLinkage = await (prisma as any).projectLinkage.findFirst({
                where: {
                    OR: [
                        { pfmeaId: { equals: fmeaId.toLowerCase(), mode: 'insensitive' } },
                        { dfmeaId: { equals: fmeaId.toLowerCase(), mode: 'insensitive' } },
                    ],
                    status: 'active',
                },
            });
            if (fmeaLinkage) {
                ...
                await (prisma as any).projectLinkage.create({
                    data: {
                        ...(isPfmea ? { pfmeaId: fmeaId.toLowerCase() } : { dfmeaId: fmeaId.toLowerCase() }),
                        pfdNo: targetPfdNo,
                        linkType: 'auto',
                        status: 'active',
                    },
            ...
            console.error('[sync-from-fmea] ProjectLinkage 업데이트 실패:', linkErr);
        }
