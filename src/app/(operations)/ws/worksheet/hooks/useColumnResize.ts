...
import { PFD_COLUMNS, RESIZE_CONFIG } from '../pfdConstants';
...
function loadSavedWidths(): ColumnWidths | null {
    ...
    try {
        ...
        console.error('[WS 컬럼 폭 불러오기] 오류:', e);
    }
    return null;
}
...
function getColumnResizeBounds(colId: number) {
    const col = PFD_COLUMNS.find(c => c.id === colId);
...
export function useColumnResize() {
    ...
    const startResize = useCallback((colId: number, clientX: number) => {
        const currentWidth = columnWidths[colId] || PFD_COLUMNS.find(c => c.id === colId)?.width || 80;
        setResizeState({
            isResizing: true,
            colId,
            startX: clientX,
    ...
    const endResize = useCallback(() => {
        ...
        if (typeof window !== 'undefined') {
            ...
            } catch (e) { console.error('[WS 컬럼 폭 저장] 오류:', e); }
        }
        setResizeState({
            isResizing: false,
            colId: null,
    ...
    const getColumnWidth = useCallback((colId: number): number => {
        return columnWidths[colId] || PFD_COLUMNS.find(c => c.id === colId)?.width || 80;
    }, [columnWidths]);
