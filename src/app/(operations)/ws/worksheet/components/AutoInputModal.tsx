 ...
 * @description PFD 워크시트 자동 입력 모달 (CP 수평전개) - 플로팅 윈도우
 */
...
function AutoInputModalInner({
  ...
  const getTypeLabel = () => {
    switch (modal.type) {
      ...
      case 'work': return '🔧 작업요소 기준';
      case 'char': return '📊 특성 기준';
    }
  };
  ...
  const getParentFields = () => {
    switch (modal.type) {
      ...
      case 'char': return ' 공정번호, 공정명, 레벨, 공정설명, 작업요소';
    }
  };
  ...
  return typeof document === 'undefined' ? null : createPortal(
    <div className="fixed z-[9999] bg-white rounded-lg shadow-2xl flex flex-col select-none border border-gray-300"
      ...
      <div className="flex items-center gap-2 p-4 pb-0 cursor-move" onMouseDown={onDragStart}>
        ...
        <h3 className="text-lg font-bold text-gray-800 flex-1">자동 행 추가</h3>
      ...
      <div className="flex-1 overflow-auto p-4">
        ...
        <div className="mb-4">
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`flex-1 py-2 px-3 rounded text-sm font-bold transition-all ${modal.position === 'above'
            ...
            <button
              ...
              className={`flex-1 py-2 px-3 rounded text-sm font-bold transition-all ${modal.position === 'below'
                  ? 'bg-[#1e3a5f] text-white'
                  : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                }`}
            >
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={onInsert}
            className="flex-1 py-2 px-4 rounded bg-[#1e3a5f] text-white hover:bg-[#162d4a] text-sm font-bold"
          >
            ✅ 행 추가
          </button>
        </div>
...
export const AutoInputModal = React.memo(AutoInputModalInner);
