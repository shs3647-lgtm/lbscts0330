...
const createInitialCFTMembers = (): CFTMember[] => [
    { id: '', role: 'Champion', name: '', department: '', position: '', task: '', phone: '', email: '', remark: '' },
    { id: '', role: 'Technical Leader', name: '', department: '', position: '', task: '', phone: '', email: '', remark: '' },
    { id: '', role: 'Leader', name: '', department: '', position: '', task: '', phone: '', email: '', remark: '' },
    { id: '', role: 'PM', name: '', department: '', position: '', task: '', phone: '', email: '', remark: '' },
    { id: '', role: 'Moderator', name: '', department: '', position: '', task: '', phone: '', email: '', remark: '' },
    ...Array(5).fill(null).map(() => ({ id: '', role: 'CFT 팀원', name: '', department: '', position: '', task: '', phone: '', email: '', remark: '' })),
];
...
function ensureRequiredCFTRoles(members: CFTMember[]): CFTMember[] {
    ...
    for (const role of REQUIRED_ROLES) {
        if (!result.some(m => m.role === role)) {
            result.push({ id: '', role, name: '', department: '', position: '', task: '', phone: '', email: '', remark: '' });
        }
    }
    if (!result.some(m => m.role === 'CFT 팀원')) {
        result.push({ id: '', role: 'CFT 팀원', name: '', department: '', position: '', task: '', phone: '', email: '', remark: '' });
    }
...
export function usePfdRegister(
    ...
    const loadAccessLogs = useCallback(async () => {
        ...
        } catch (e) { console.error('[WS 접속로그 로드] 오류:', e); setAccessLogs([]); }
    }, [pfdId, editId]);
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            if (!project) {
                ...
                } catch (e) { console.error('[WS localStorage 파싱] 오류:', e); }
            }
