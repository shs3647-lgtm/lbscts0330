/**
 * @file pfdRegister.ts
 * @description PM 등록 타입 정의 (PFD 등록과 동등 수준)
 * @updated 2026-02-10
 */
...
export interface PFDInfo {
    ...
    modelYear: string;
    subject: string;
    pfdStartDate: string;   // pmStartDate로 매핑
    pfdRevisionDate: string; // pmRevisionDate로 매핑
    pfdId: string;           // pmNo로 매핑
    pfdType: PFDType;
    processResponsibility: string;
    confidentialityLevel: string;
    securityLevel: string;
    pfdResponsibleName: string; // pmResponsibleName으로 매핑
    linkedCpNo: string;
    createdAt: string;
    updatedAt: string;
    partName?: string;
