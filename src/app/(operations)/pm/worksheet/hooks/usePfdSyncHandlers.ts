...
export function usePfdSyncHandlers(options: UsePfdSyncHandlersOptions): UsePfdSyncHandlersReturn {
    ...
    const handleCpSync = useCallback(async () => {
        ...
        const confirmed = confirm(
            `PFD 데이터를 CP로 연동하시겠습니까?\n\n` +
            `• 대상 CP: ${targetCpNo}\n` +
            `• 연동 항목: ${validItems.length}건`
        );
    ...
    const handleFmeaSync = useCallback(async () => {
        ...
        const confirmed = confirm(
            `PFD 데이터로 PFMEA를 ${fmeaId ? '동기화' : '생성'}하시겠습니까?\n\n` +
            `• 대상 FMEA: ${targetFmeaId}\n` +
    ...
    const handleCpToPfd = useCallback(async () => {
        ...
        const confirmed = confirm(
            `CP 데이터를 PFD로 가져오시겠습니까?\n\n` +
            `• 연결된 CP: ${cpNo}\n` +
            `• 현재 PFD 항목: ${items.length}건\n\n` +
            `⚠️ PFD의 일부 데이터가 CP 데이터로 덮어쓰여질 수 있습니다.`
        );
    ...
    const handleFmeaToPfd = useCallback(async () => {
        ...
        const confirmed = confirm(
            `PFMEA 데이터를 PFD로 가져오시겠습니까?\n\n` +
            `• 연결된 FMEA: ${fmeaId}\n` +
            `• 현재 PFD 항목: ${items.length}건\n\n` +
            `⚠️ PFD의 일부 데이터가 FMEA 데이터로 덮어쓰여질 수 있습니다.`
        );
