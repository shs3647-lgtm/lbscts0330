    ...
    const loadPfdData = useCallback(async (targetPfdNo: string) => {
        try {
            ...
            if (pfdRes.ok) {
                ...
                if (pfdData.success) {
                    // ★ 연동 CP에서 partNameMode 상속
                    let partNameMode: 'A' | 'B' = 'A';
                    // cpNo 우선순위: cpNo 필드 → linkedCpNos 첫번째 → 빈 문자열
                    const linkedCpNos: string[] = Array.isArray(pfdData.data?.linkedCpNos) ? pfdData.data.linkedCpNos : [];
                    const cpNo = pfdData.data?.cpNo || linkedCpNos[0] || '';
                    if (cpNo) {
                        try {
                            const cpRes = await fetch(`/api/control-plan/${cpNo}/items`);
                            if (cpRes.ok) {
                                const cpData = await cpRes.json();
                                if (cpData.success && cpData.partNameMode) {
                                    partNameMode = cpData.partNameMode as 'A' | 'B';
                                }
                            }
                        } catch {
                            // CP partNameMode 로드 실패 — 기본값(A) 유지
                        }
                    }

                    // ★ 잘못된 PFD ID로 접근했으면 교정된 ID로 리다이렉트
                    const actualPfdNo = pfdData.data?.correctedPfdNo || targetPfdNo;
                    if (pfdData.data?.correctedPfdNo && pfdData.data.correctedPfdNo !== targetPfdNo) {
                        router.replace(`/pfd/worksheet?pfdNo=${pfdData.data.correctedPfdNo}`);
                    }
                    ...
                    setState(prev => ({
                        ...
                        dirty: false,
                        equipmentTools: pfdData.data?.equipmentTools || [],
                        partItems: pfdData.data?.partItems || [],
                        partNameMode,  // ★ CP에서 상속
                    }));
