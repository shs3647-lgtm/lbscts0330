 ...
 * 유효: pfd26-m001-L01, pfd26-f002-S
 * 레거시: pfd26-m001, pfdl26-f001
 */
export function isValidPfdNo(pfdNo: string): boolean {
    ...
    // 새 형식: pfd26-m001-i01 or pfd26-m001-s (레거시 L도 인식) + 선택적 -r00
    if (/^pfd\d{2}-[mfp]\d{3}-[LI]\d{2}(-r\d{2})?$/i.test(pfdNo)) return true;
    if (/^pfd\d{2}-[mfp]\d{3}-S(-r\d{2})?$/i.test(pfdNo)) return true;
    // 레거시 형식: pfd26-m001 or pfdl26-m001
    return /^pfdl?\d{2}-[mfp]\d{3}(-r\d{2})?$/i.test(pfdNo);
}
...
/**
 * PFD ID 생성 (v2.1 - L{NN}/S 접미사 방식)
 * 형식: pfd{YY}-{t}{NNN}-{L{NN}|S}
 * 예시: pfd26-m001-L01 (Master, 연동그룹01)
    ...
    if (typeof window !== 'undefined') {
        try {
            ...
            if (stored) {
                ...
                if (ids.length > 0) {
                    return `pfd${year}-${typeChar}${(Math.max(...ids) + 1).toString().padStart(3, '0')}-${linkSuffix}-r00`;
                }
            }
        } catch (e) { console.error('[PFD ID 생성] localStorage 오류:', e); }
    }
    return `pfd${year}-${typeChar}001-${linkSuffix}-r00`;
}
...
export function isLinked(pfdNo: string): boolean {
    return /-[LI]\d{2}(-r\d{2})?$/i.test(pfdNo);
}
...
export function getLinkGroupNo(pfdNo: string): number {
    const match = pfdNo.match(/-[LI](\d{2})(-r\d{2})?$/i);
...
export function setLinked(pfdNo: string, linkGroupNo: number = 1): string {
    ...
    // -r00 개정 접미사 보존
    const revMatch = pfdNo.match(/(-r\d{2})$/i);
    const revSuffix = revMatch ? revMatch[1] : '';
    const base = revSuffix ? pfdNo.slice(0, -revSuffix.length) : pfdNo;
