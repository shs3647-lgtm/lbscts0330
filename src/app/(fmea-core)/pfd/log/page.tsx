/**
 * @file page.tsx
 * @description PFD 접속 로그 페이지 - API 연동
 * @version 3.0.0
...
export default function PFDAccessLogPage() {
  ...
  const loadLogs = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/auth/access-log?module=PFD&limit=100');
      if (res.ok) {
          ...
          <div className="max-w-[1400px] mx-auto">
            ...
            <div className="flex items-center justify-between mb-4 mt-2">
              <div className="flex items-center gap-3">
                <h1 className="text-xl font-bold text-violet-700">PFD 접속 로그(Access Log)</h1>
                <button
                  onClick={loadLogs}
                  disabled={loading}
              ...
              <div className="flex gap-2">
                ...
                >
                  <option value="all">전체 기간(All)</option>
                  <option value="today">오늘(Today)</option>
                  <option value="week">최근 7일(7 Days)</option>
                  <option value="month">최근 30일(30 Days)</option>
                </select>
                <input
                  type="text"
                  value={searchQuery}
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-violet-600 text-white">
                    <tr>
                      <th className="px-4 py-3 text-left text-xs font-semibold">시간(Time)</th>
                      <th className="px-4 py-3 text-left text-xs font-semibold">사용자(User)</th>
                      <th className="px-4 py-3 text-left text-xs font-semibold">액션(Action)</th>
                      <th className="px-4 py-3 text-left text-xs font-semibold">대상(Target)</th>
                      <th className="px-4 py-3 text-left text-xs font-semibold">대상 ID(Target ID)</th>
                    </tr>
                  </thead>
                  <tbody>
                    {loading ? (
                      <tr>
                        <td colSpan={5} className="px-4 py-12 text-center text-gray-500">
                          로그를 불러오는 중...(Loading logs)
                        </td>
                      </tr>
                    ) : filteredLogs.length === 0 ? (
                      <tr>
                        <td colSpan={5} className="px-4 py-12 text-center text-gray-500">
                          접속 로그가 없습니다.(No access logs)
                        </td>
                      </tr>
                    ) : (
            ...
            <div className="mt-4 text-sm text-gray-500">
              총(Total) {filteredLogs.length}개 로그(logs)
            </div>
          </div>
        </div>
      </div>
