...
const createInitialCFTMembers = (): CFTMember[] => [
    { id: '', projectId: '', factory: '', role: 'Champion', name: '', department: '', position: '', responsibility: '', phone: '', email: '', remark: '' },
    { id: '', projectId: '', factory: '', role: 'Technical Leader', name: '', department: '', position: '', responsibility: '', phone: '', email: '', remark: '' },
    { id: '', projectId: '', factory: '', role: 'Leader', name: '', department: '', position: '', responsibility: '', phone: '', email: '', remark: '' },
    { id: '', projectId: '', factory: '', role: 'PM', name: '', department: '', position: '', responsibility: '', phone: '', email: '', remark: '' },
    { id: '', projectId: '', factory: '', role: 'Moderator', name: '', department: '', position: '', responsibility: '', phone: '', email: '', remark: '' },
    ...Array(5).fill(null).map(() => ({ id: '', projectId: '', factory: '', role: 'CFT 팀원', name: '', department: '', position: '', responsibility: '', phone: '', email: '', remark: '' })),
];

/** DB 로드 후 필수 역할이 누락되어 있으면 자동 추가 */
function ensureRequiredCFTRoles(members: CFTMember[]): CFTMember[] {
    const result = [...members];
    for (const role of REQUIRED_ROLES) {
        if (!result.some(m => m.role === role)) {
            result.push({ id: '', projectId: '', factory: '', role, name: '', department: '', position: '', responsibility: '', phone: '', email: '', remark: '' });
        }
    }
    if (!result.some(m => m.role === 'CFT 팀원')) {
        result.push({ id: '', projectId: '', factory: '', role: 'CFT 팀원', name: '', department: '', position: '', responsibility: '', phone: '', email: '', remark: '' });
    }
    const sorted: CFTMember[] = [];
    for (const role of REQUIRED_ROLES) {
        const m = result.find(x => x.role === role);
        if (m) sorted.push(m);
    }
...
export function usePfdRegister(
    ...
    const loadAccessLogs = useCallback(async () => {
        ...
        } catch (e) { console.error('[PFD 접속로그 로드] 오류:', e); setAccessLogs([]); }
    }, [pfdId, editId]);
        ...
        const loadData = async () => {
            ...
            if (project) {
                ...
                const loadedInfo: PFDInfo = {
                    ...
                    securityLevel: project.securityLevel || project.pfdInfo?.securityLevel || '',
                    ...
                    createdAt: project.createdAt || project.pfdInfo?.createdAt || '',
