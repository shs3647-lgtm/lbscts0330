...
export function DocHeader() {
  return (
    <>
      <tr>
        <th colSpan={5} style={{ ...mainHdr, width: '100%' }}><BiHeader ko="문서화 (7단계)" en="Documentation (Step 7)" /></th>
      </tr>
      <tr>
        ...
        {rows.length === 0 ? (
          <tr>
            <td colSpan={5} className="text-center py-8 text-gray-400 text-sm">
              문서관리 데이터가 없습니다. 구조분석을 먼저 완료하세요.(No documentation data. Complete Structure Analysis first.)
            </td>
          </tr>
        ) : rows.map((row, idx) => (
