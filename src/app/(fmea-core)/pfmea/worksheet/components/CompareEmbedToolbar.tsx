...
export default function CompareEmbedToolbar({
  ...
  if (readOnly) {
    return (
      <div
        className="flex shrink-0 items-center gap-1 border-b border-slate-200 bg-slate-100 px-2 text-[9px] text-slate-600"
        style={{ height: 22 }}
        data-testid="compare-embed-toolbar"
      >
        <span className="font-semibold text-slate-800">Master 참조</span>
        <span className="text-slate-400">읽기 전용</span>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div className="flex min-w-0 items-center gap-1 overflow-hidden">
        ...
        {dirty ? (
          <span className="shrink-0 rounded bg-amber-200 px-1 font-semibold text-amber-900">수정됨</span>
        ) : (
          <span className="shrink-0 text-slate-500">저장됨</span>
        )}
      </div>
      <button
        type="button"
