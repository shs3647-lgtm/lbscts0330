...
/** FailureLink 타입 for normalization */
...
export const normalizeFailureLinks = (links: FailureLinkRecord[], stateSnapshot: WorksheetState): FailureLinkRecord[] => {
  ...
  (stateSnapshot.l2 || []).forEach((proc: AnyRecord) => {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ((stateSnapshot.l1 as any)?.failureScopes || []).forEach((fe: AnyRecord) => {
  ...
  return links.map((link: FailureLinkRecord) => {
