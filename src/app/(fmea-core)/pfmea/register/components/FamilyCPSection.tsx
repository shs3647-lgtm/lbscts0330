    ...
    <div className="mt-3 rounded-lg overflow-hidden border border-gray-400 bg-white">
      ...
      <div className="flex items-center justify-between px-3 py-1.5 bg-[#00587a] text-white">
        <div className="flex items-center gap-2">
          <span className="text-xs font-bold" title="Family Control Plan Management">Family CP 관리(Mgmt)</span>
        ...
        <div className="flex items-center gap-1.5">
          ...
          >
            {creating ? '생성중...(Creating)' : '일괄생성(Batch Create)'}
          </button>
          <button
      ...
      <div className="px-2 py-1.5">
        {loading ? (
          <div className="text-center py-3 text-[10px] text-gray-400">로딩 중...(Loading)</div>
        ) : cpList.length === 0 ? (
          <div className="text-center py-4 text-[10px] text-gray-400 bg-gray-50 rounded border border-dashed border-gray-200">
            <p className="mb-1">등록된 관리계획서가 없습니다(No CP registered)</p>
            <p className="text-[9px] text-gray-300">CP개수를 지정하고 [일괄생성(Batch Create)]을 클릭하세요</p>
          </div>
        ) : (
          <table className="w-full text-[10px] border-collapse">
            <thead>
              <tr className="bg-gray-50 text-gray-500">
                <th className="px-2 py-1 text-left w-8 border border-gray-200" title="Number">No</th>
                <th className="px-2 py-1 text-left w-28 border border-gray-200" title="Control Plan Identifier">CP ID</th>
                <th className="px-2 py-1 text-center w-14 border border-gray-200" title="Classification">구분(Class)</th>
                <th className="px-2 py-1 text-left w-20 border border-gray-200" title="Label">라벨(Label)</th>
                <th className="px-2 py-1 text-left border border-gray-200" title="Subject / Title">제목(Title)</th>
                <th className="px-2 py-1 text-center w-12 border border-gray-200" title="Status">상태(Status)</th>
                <th className="px-2 py-1 text-center w-24 border border-gray-200" title="Action">동작(Action)</th>
              </tr>
            </thead>
            <tbody>
              {cpList.map((cp, idx) => (
                <tr key={cp.cpNo} className="hover:bg-blue-50">
                  ...
                  <td className="px-2 py-1 text-center border border-gray-200">
                    <span className={`px-1 py-0 text-[9px] rounded ${
                      ...
                      {cp.status === 'approved' ? '승인(Approved)' : cp.status === 'draft' ? '초안(Draft)' : cp.status || '초안(Draft)'}
                    </span>
                  </td>
                  <td className="px-2 py-1 text-center border border-gray-200">
