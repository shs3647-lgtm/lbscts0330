    ...
    <div className="rounded-lg overflow-hidden border border-gray-400 bg-white">
      ...
      <div className="flex items-center justify-between px-3 py-1.5 bg-[#00587a] text-white">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => alert(
              '📋 결재 진행 도움말 (Approval Guide)\n\n' +
              '【 결재 흐름 】\n' +
              '1. 작성(Author) → 상신 버튼 클릭\n' +
              '2. 검토(Review) → 검토 승인 또는 반려\n' +
              '3. 승인(Approve) → 최종 승인 또는 반려\n\n' +
              '【 회수 】\n' +
              '• 상신 후 검토자가 처리하기 전에 회수 가능\n' +
              '• 검토 승인 후 승인자가 처리하기 전에 회수 가능\n\n' +
              '【 반려 시 】\n' +
              '• 검토/승인 반려 → 작성자가 수정 후 재상신\n\n' +
              '【 결재 완료 후 】\n' +
              '• 이력추가: 현재 개정에 변경 이력 추가\n' +
              '• REV N+1: 새로운 개정 버전 시작\n\n' +
              '【 담당자 지정 】\n' +
              '• 각 단계의 이름을 클릭하여 담당자 변경 가능\n' +
              '• 직급(Position)도 클릭하여 변경 가능'
            )}
            className="px-1.5 py-0.5 text-[9px] font-bold bg-white/20 text-white rounded hover:bg-white/30 cursor-pointer"
            title="결재 진행 도움말 (Approval Help)"
          >
            ?
          </button>
        </div>
