...
export function useRevisionData(idFromUrl: string): UseRevisionDataReturn {
    ...
    const loadProjects = useCallback(async () => {
        const isPfmeaProject = (id: string) => {
            ...
            return lowerId.startsWith('pfm') || lowerId.startsWith('pfmea');
        };
        ...
        try {
            ...
            if (result.success && result.projects.length > 0) {
                ...
                setProjectList(pfmeaProjects);

                if (!selectedProjectId && pfmeaProjects.length > 0) {
                    const urlProject = pfmeaProjects.find((p: FMEAProject) => p.id === idFromUrl);
                    setSelectedProjectId(urlProject?.id || pfmeaProjects[0].id);
                }
                return;
            }
        } catch (error) {
