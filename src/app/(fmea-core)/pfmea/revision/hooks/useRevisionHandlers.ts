...
export function useRevisionHandlers({
    ...
    const handleAddRevision = useCallback(() => {
        ...
        const newRevision: RevisionRecord = {
            ...
            createPosition: fmeaInfo?.fmeaResponsiblePosition || '리더',
            createName: fmeaInfo?.fmeaResponsibleName || '',
            createDate: today,
            createStatus: '진행',
            // 검토자: CFT Technical Leader
            reviewPosition: fmeaInfo?.reviewResponsiblePosition || 'te',
            reviewName: fmeaInfo?.reviewResponsibleName || '',
            reviewDate: '',
            reviewStatus: '',
            // 승인자: CFT Champion
            approvePosition: fmeaInfo?.approvalResponsiblePosition || 'champion',
