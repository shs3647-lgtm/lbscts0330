...
const SHEET_DEFINITIONS = [
  ...
  { name: 'L2 통합(A1-A6)', headers: ['공정번호', '공정명(A1)', '공정기능(A2)', '제품특성(A3)', '특별특성(A4)', '고장형태(A5)', '검출관리(A6)'], color: HEADER_COLOR, required: [true, true, false, false, false, false, false], legacyName: 'L2_UNIFIED', guide: '공정번호+공정명 필수 | 제품특성·고장형태가 여러 건이면 공정번호·공정명 중복 기입 (Ctrl+D)' },
...
const SAMPLE_DATA: Record<string, string[][]> = {
  ...
  // L2 통합: 공정번호, 공정명(A1), 공정기능(A2), 제품특성(A3), 특별특성(A4), 고장형태(A5), 검출관리(A6)
  // L3 통합: 공정번호, 4M, 작업요소(B1), 요소기능(B2), 공정특성(B3), 특별특성, 고장원인(B4), 예방관리(B5)
  // ═══════════════════════════════════════════════════════════════

  'L1 통합(C1-C4)': [
    // 구분(C1) | 제품기능(C2) | 요구사항(C3) | 고장영향(C4)
    ['YP', 'Bump 전기적 연결 확보', 'Bump Height 10±1μm', '전기적 Open/Short'],
    ['YP', 'Bump 전기적 연결 확보', 'Bump Pitch 40μm 이하', 'Bump간 Bridge'],
    ['YP', 'Bump 전기적 연결 확보', 'UBM 접착력 규격', 'Bump Lift-off'],
    ['SP', '패키지 신뢰성 확보', 'TCT 1000cycle 이상', '필드 불량, 반품'],
    ['SP', '패키지 신뢰성 확보', 'HTS 1000hr 이상', 'IMC 과성장에 의한 접합부 열화'],
    ['USER', '고객 수율 요구 충족', '수율 99.5% 이상', '고객 라인 정지, 클레임'],
    ['USER', '고객 수율 요구 충족', 'DPPM 100 이하', '고객 신뢰도 하락'],
  ],
  ...
  'L3 통합(B1-B5)': [
    ...
    ['10', 'MN', '검사원', '검사원이 IQA 기준에 따라 Wafer를 판정한다', '', '', '판정 기준 오적용', '검사원 정기 교육'],
    ['10', 'IM', 'Wafer', 'Wafer가 후속 공정의 기판 역할을 한다', '', '', '자재 규격 미달', '수입검사 강화, IQC COA 확인'],
    ...
    ['150', 'MN', '검사원', '검사원이 최종 판정 기준에 따라 합부를 결정한다', '', '', '판정 기준 오적용', '검사원 정기 교육, 한도 견본 비치'],
    ...
    ['200', 'MN', '포장 작업자', '포장 작업자가 체크리스트에 따라 출하 포장을 완료한다', '', '', '체크리스트 누락', '출하 체크리스트 교육'],
  ],
};
