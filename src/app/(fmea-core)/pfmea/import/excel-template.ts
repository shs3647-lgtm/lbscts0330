...
const SHEET_DEFINITIONS = [
  ...
  // ★★★ FC 고장사슬 + FA 통합분석 ★★★
  { name: 'FC 고장사슬', headers: ['FE구분', 'FE(고장영향)', 'L2-1.공정번호', 'FM(고장형태)', '4M', 'WE(작업요소)', 'FC(고장원인)', 'B5.예방관리(발생 전 방지)', 'A6.검출관리(발생 후 검출)', 'O', 'D', 'AP'], color: 'B91C1C', required: [false, false, true, true, false, false, false, false, false, false, false, false], legacyName: 'FC', guide: '매 행에 FE구분·FE·공정번호·FM을 반복 기입(개별셀). B5/A6/O·D·AP | 앱 미리보기는 병합 표시', legacy: false },
...
const DFMEA_SHEET_DEFINITIONS = [
  // L1: 구분(법규/기본/보조/관능) — 문서 매핑표 F1~F4
  { name: 'L1 통합(C1-C4)', headers: ['구분(C1)', '제품기능(C2)', '요구사항(C3)', '고장영향(C4)'], color: DFMEA_HEADER_COLOR, guide: '구분: 법규/기본/보조/관능 | 제품기능·요구사항·고장영향을 한 행에 기입 | 1:N 관계는 상위값을 중복 기입 (Ctrl+D)' },
  // L2: 번호(D1) + 초점요소(D2) + 초점요소 기능(D3) + 제품특성(D4) — 문서 매핑표 기준
  { name: 'L2 통합(A1-A6)', headers: ['A1.번호', 'A2.초점요소', 'A3.초점요소 기능', 'A4.제품특성', '특별특성', 'A5.고장형태', 'A6.검출관리'], color: DFMEA_HEADER_COLOR, guide: '번호+초점요소 필수 | 제품특성·고장형태가 여러 건이면 번호·초점요소 중복 기입 (Ctrl+D)' },
  // L3: 타입(E1) + 부품 또는 특성(E1) + 부품 기능 또는 특성(E2) + 부품 요구사항(E3) — 문서 매핑표 기준
  { name: 'L3 통합(B1-B5)', headers: ['번호', '타입', '부품 또는 특성(B1)', '부품 기능 또는 특성(B2)', '부품 요구사항(B3)', '특별특성', '고장원인(B4)', '예방관리(B5)'], color: DFMEA_HEADER_COLOR, guide: '타입: 부품/재료/인터페이스/소프트웨어 | 부품별 기능·부품 요구사항·고장원인·예방관리를 한 행에 기입 | 1:N은 상위값 중복 기입' },
  // FC: 번호 + 타입 + 부품 또는 특성
  { name: 'FC 고장사슬', headers: ['FE구분', 'FE(고장영향)', 'L2-1.번호', 'FM(고장형태)', '타입', '부품 또는 특성', 'FC(고장원인)', 'B5.예방관리(발생 전 방지)', 'A6.검출관리(발생 후 검출)', 'O', 'D', 'AP'], color: 'B91C1C', guide: '매 행에 FE구분·FE·번호·FM을 반복 기입(개별셀). B5/A6/O·D·AP | 앱 미리보기는 병합 표시' },
  // FA: DFMEA 용어 — 문서 매핑표 기준
  { name: 'FA 통합분석', headers: ['구분(C1)', '제품기능(C2)', '요구사항(C3)', '번호(A1)', '초점요소(A2)', '초점요소 기능(A3)', '제품특성(A4)', '특별특성(A4)', '타입', '부품 또는 특성(B1)', '부품 기능 또는 특성(B2)', '부품 요구사항(B3)', '특별특성(B3)', '고장영향(C4)', '고장형태(A5)', '고장원인(B4)', 'S', 'O', 'D', 'AP', 'DC추천1', 'DC추천2', 'PC추천1', 'PC추천2', 'O추천', 'D추천'], color: '1E40AF', guide: '' },
];
...
export async function downloadDfmeaEmptyTemplate(customFileName?: string) {
  ...
  const emptyWidths: Record<string, number> = {
    'A1.번호': 16, 'A2.초점요소': 22, 'A3.초점요소 기능': 40, 'A4.제품특성': 22,
    '특별특성': 10, 'A5.고장형태': 25, 'A6.검출관리': 40,
    '구분(C1)': 14, '제품기능(C2)': 40, '요구사항(C3)': 25, '고장영향(C4)': 30,
    '부품 또는 특성(B1)': 28, '부품 기능 또는 특성(B2)': 35, '부품 요구사항(B3)': 22, '고장원인(B4)': 30,
    '예방관리(B5)': 40, '타입': 12,
    'DC추천1': 25, 'DC추천2': 25, 'PC추천1': 25, 'PC추천2': 25, 'O추천': 8, 'D추천': 8,
  };
