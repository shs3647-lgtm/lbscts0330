...
import { inferMissingPCDC, inferC2C3, inferChar, getDefaultRuleSet } from './pc-dc-inference';
...
export function buildImportData(rows: StepBRawRow[], warn: WarningCollector): StepBBuildData {
  ...
  // ★ PC/DC 빈 값 자동추론 (업종별 DB 기반)
  // fcChains에서 빈 PC/DC를 키워드 매칭으로 자동 채움
  inferMissingPCDC(fcChains, warn);
  ...
  for (const pno of sortedProcNos) {
    ...
    for (const item of items) {
      ...
      flatData.push({
        ...
        belongsTo: item.we, parentItemId: b1IdMap.get(weKey),
        createdAt: now,
      });
    }
  }
  ...
  for (const pno of sortedProcNos) {
    ...
    for (const item of items) {
      flatData.push({
        ...
        value: item.fc, m4: item.m4, createdAt: now,
      });
    }
  }
