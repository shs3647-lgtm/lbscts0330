...
/**
 * @file types.ts
 * @description DFMEA Import 타입 정의 (3단계 프로세스)
 * @author AI Assistant
 * @created 2025-12-26
 ...
 * 
 * [L2/L3 레벨 - 카테고리 A/B]
 *   - processNo = 초점요소번호 (예: '10', '20', '30', ...)
 *   - 특수값 '00' = 공통 하위요소 (모든 초점요소에 적용)
 *   - A1~A6: 초점요소번호 기준으로 그룹핑
 *   - B1~B5: 초점요소번호 기준으로 그룹핑 (00은 공통)
 *
 * [L1 레벨 - 카테고리 C]
 *   - processNo = 구분 (C1 값: '법규', '기본', 'USER', '관능')
...
export interface ImportedFlatData {
  ...
  /**
   * 구분자 (레벨별 다름)
   * - L2/L3 (A/B): 초점요소번호 (예: '10', '20', '00'=공통)
   * - L1 (C): 구분값 (예: '법규', '기본', 'USER', '관능')
   */
  processNo: string;
  category: 'A' | 'B' | 'C';
   ...
   */
  value: string;
  m4?: string;  // 타입 분류 (B1 하위요소 전용): PT(부품), AY(A'SSY), PC/ME/ET/DE/HM(I/F)
  specialChar?: string;  // ★ 특별특성 기호 (A4/B3 시트): ◇, ◆, △, ○ 등
  belongsTo?: string;  // ★ B3 전용: 소속 하위요소 이름 (complex template에서 파싱)
...
export const ITEM_CODE_LABELS: Record<string, string> = {
  // L2 레벨 (초점요소) - 내부 코드 A1~A5 (v3.0: A6 검출관리 IMPORT 제외 → 리스크 탭)
  A1: '초점요소번호',
  A2: '초점요소명',
  A3: '초점기능(설명)',
  A4: '초점요소 요구사항',
  A5: '고장형태',
  A6: '검출관리',
  // L3 레벨 (하위요소) - 내부 코드 B1~B5
  B1: '하위요소',
  B2: '하위기능',
  B3: '하위기능 요구사항',
  B4: '고장원인',
  B5: '예방관리',
  // L1 레벨 (상위수준) - 내부 코드 C1~C4
  C1: '구분',  // 법규, 기본, 보조, 관능
  C2: '상위기능',
  C3: '상위요구사항',
  C4: '고장영향',
...
/** C1 구분 값 (고장영향 수준) — DFMEA: 법규/기본/보조/관능 */
export const C1_CATEGORIES = ['법규', '기본', '보조', '관능'] as const;
export type C1CategoryType = typeof C1_CATEGORIES[number];

/** 레벨별 아이템 코드 매핑 (v3.0: A6/B5 IMPORT 제외) */
export const LEVEL_ITEM_CODES = {
  L1: ['C1', 'C2', 'C3', 'C4'],  // 상위수준 레벨 (L1-1 ~ L1-4)
  L2: ['A1', 'A2', 'A3', 'A4', 'A5'],  // 초점요소 레벨 (L2-1 ~ L2-5) — A6 검출관리 → 리스크 탭
  L3: ['B1', 'B2', 'B3', 'B4'],  // 하위요소 레벨 (L3-1 ~ L3-4) — B5 예방관리 → 리스크 탭
} as const;

/** 신규 시트명 → 내부 코드 매핑 (v3.0: A6/B5 제거) */
...
/** 내부 코드 → 신규 시트명 매핑 (v3.0: A6/B5 제거) */
export const CODE_TO_SHEET_MAP: Record<string, string> = {
  'A1': 'L2-1', 'A2': 'L2-2', 'A3': 'L2-3', 'A4': 'L2-4', 'A5': 'L2-5',
  'B1': 'L3-1', 'B2': 'L3-2', 'B3': 'L3-3', 'B4': 'L3-4',
