    ...
    return (
      <button
        onClick={(e) => {
          e.stopPropagation();
          if (fmeaId) {
            window.location.href = `/dfmea/worksheet?id=${fmeaId}`;
          }
        }}
...
const TH = 'bg-[#00587a] text-white border border-gray-400 px-1 py-0.5 font-bold text-center whitespace-nowrap h-6 text-[10px] cursor-pointer hover:bg-[#006d99] select-none';
        ...
        <table className="w-full border-collapse table-fixed">
          ...
          <thead className="sticky top-0 z-10">
            <tr>
              ...
              <th className={TH} onClick={() => handleSort('fmeaType')} title="FMEA 유형: Master / Family / Part">유형{sortArrow('fmeaType')}</th>
              <th className={TH} onClick={() => handleSort('companyName')} title="FMEA 등록 회사명">회사명{sortArrow('companyName')}</th>
              <th className={TH} onClick={() => handleSort('customerName')} title="고객사 (납품처)">고객{sortArrow('customerName')}</th>
              <th className={TH} onClick={() => handleSort('bdId')} title="Basic Data 고유 ID (자동 생성)">BD ID{sortArrow('bdId')}</th>
              <th className={TH} onClick={() => handleSort('createdAt')} title="Basic Data 최초 생성일 (Import 시점)">BD생성일{sortArrow('createdAt')}</th>
              <th className={TH} onClick={() => handleSort('bdVersion')} title="Basic Data 버전 (저장할 때마다 증가)">BD Rev{sortArrow('bdVersion')}</th>
              <th className={TH} onClick={() => handleSort('fmeaId')} title="FMEA 프로젝트 고유 ID">FMEA ID{sortArrow('fmeaId')}</th>
              <th className={TH} onClick={() => handleSort('fmeaName')} title="FMEA 프로젝트명 (제목)">FMEA명{sortArrow('fmeaName')}</th>
              <th className={TH} onClick={() => handleSort('startDate')} title="FMEA 프로젝트 작성일">작성일{sortArrow('startDate')}</th>
              <th className={TH} onClick={() => handleSort('revisionNo')} title="개정번호 (Revision)">Rev{sortArrow('revisionNo')}</th>
              <th className={TH} onClick={() => handleSort('processCount')} title="공정 수 (FA 검증 기준)">공정{sortArrow('processCount')}</th>
              <th className={TH} onClick={() => handleSort('fmCount' as SortKey)} title="FM(고장형태) 고유 수">FM</th>
              <th className={TH} onClick={() => handleSort('fcCount' as SortKey)} title="FC(고장원인) 고유 수">FC</th>
              <th className={TH} onClick={() => handleSort('dataCount')} title="기초정보 데이터 건수 (값이 있는 항목 수)">데이터{sortArrow('dataCount')}</th>
              <th className={TH} onClick={() => handleSort('judge')} title="판정: 공정·FM·FC 모두 있으면 적합">판정{sortArrow('judge')}</th>
            </tr>
            {/* 삭제됨 선택 버튼 (관리자 모드) */}
            {adminMode && deletedCount > 0 && (
              <tr>
