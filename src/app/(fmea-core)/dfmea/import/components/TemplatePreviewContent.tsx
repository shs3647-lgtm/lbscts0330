...
export function TemplatePreviewContent(props: TemplatePreviewContentProps) {
  const {
    ...
    stepState, setActiveStep, canSA, canFC, canFA, confirmSA, confirmFC, confirmFA, resetToSA, resetToFC,
  ...
  const isSAActive = !isDownload || stepState.activeStep === 'SA';
  ...
  const handleScrollToUnmatched = useCallback((type: 'FE' | 'FM' | 'FC', text: string) => {
    ...
    if (isDownload && stepState.activeStep !== 'SA') setActiveStep('SA');
    setPreviewLevel(level);
    setScrollTarget({ type, text });
  }, [isDownload, stepState.activeStep, setActiveStep, setPreviewLevel]);
  ...
  return (
    <div className={`flex-1 min-w-0 ${isFullscreen ? '' : 'border-l border-blue-200 pl-3'}`}>
      {/* ─── 기존데이터 서브탭 (SA/FC/FA) — 3단계 확정 배지 포함 ─── */}
      {isDownload && flatData.length > 0 && (
      ...
      <div>
        <div className="flex flex-wrap items-center gap-1 mb-1">
          ...
          {/* ─── 확정 버튼 그룹 (download 모드 — 항상 표시, 활성화/비활성화) ─── */}
          {isDownload && (<>
            {/* SA: 미확정→확정 / 확정됨→되돌리기 */}
            ...
            {isAnalysisComplete && (
              <span className="text-[10px] text-green-600 font-bold">워크시트로 이동합니다...</span>
            )}
          </>)}
      ...
      {/* ─── 변환결과 통계표 — 모든 단계에서 표시 ─── */}
      {isDownload && effectiveStatistics && (
      ...
      {/* ─── FC 콘텐츠: 고장사슬 미리보기 + 비교 결과 ─── */}
      {isDownload && stepState.activeStep === 'FC' && (<>
      ...
      {/* ─── FA 콘텐츠: 통합분석 미리보기 ─── */}
      {isDownload && stepState.activeStep === 'FA' && (<>
