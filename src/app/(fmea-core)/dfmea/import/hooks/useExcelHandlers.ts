...
export function useExcelHandlers({
  ...
  const downloadFmeaSample = async (fmeaId: string, level: 'L0' | 'L1' | 'L2' | 'L3') => {
    ...
    const wsData = localStorage.getItem(`dfmea-worksheet-${fmeaId}`);
    if (!wsData) {
  ...
  const handleDownloadPreview = async () => {
    ...
    a.href = url;
    a.download = `Import_${selectedLabel}_${new Date().toISOString().slice(0, 10)}.xlsx`;
    a.click();
    URL.revokeObjectURL(url);
  };
      ...
      for (let i = 2; i <= sheet.rowCount; i++) {
        ...
        for (let col = 2; col <= colMapping.length; col++) {
          ...
          if (value && itemCode) {
            ...
            const c1Value = itemCode === 'C1' ? value : (String(row.getCell(2).value || '').trim() || 'YP');
            newData.push({
