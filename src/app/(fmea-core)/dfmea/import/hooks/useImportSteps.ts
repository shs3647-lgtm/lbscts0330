  ...
  const confirmSA = useCallback((): BuildResult | null => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const comparison = compareFCChains(failureChains, derived);
          ...
          try {
            ...
            if (vcRes.ok) {
              ...
              if (vcJson.success) {
                setStepState(prev => ({
                  ...prev,
                  // ★ SA확정: import 건수만 표시, db(atomic 테이블)는 FA확정 전이므로 빈 객체
                  // → DB저장/I→D 컬럼에 "-" 표시 (이전 FA확정의 오래된 데이터 혼동 방지)
                  dbVerifyCounts: { import: vcJson.import, db: {} },
                }));
              }
            }
          } catch (err) {
  ...
  const confirmFA = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      if (!fcSheetChains && fmeaId) {
        try {
          const dbRes = await fetch(`/api/dfmea/master?fmeaId=${encodeURIComponent(fmeaId)}&includeItems=true`);
          if (dbRes.ok) {
        ...
        try {
          ...
          if (usedChains.length > 0) {
            const patchResp = await fetch(
              '/api/dfmea/master?fmeaId=' + encodeURIComponent(fmeaId),
      ...
      if (wsResult.success) {
        ...
        // ★★★ 2026-03-01: FA 확정 후 verify-counts API 호출 → 실제 DB 건수 확인 ★★★
        ...
        try {
          ...
          console.error('[FA 확정] verify-counts 조회 실패:', vcErr);
        }
        ...
        // ★ DB 불일치 경고 (2026-03-03 수정: DB < Import만 경고 — DB > Import는 분배 알고리즘 정상 동작)
        let dbMismatchWarning = '';
        if (verifyCounts) {
          const mismatchItems: string[] = [];
          const itemCodes = ['A2', 'B1', 'C1', 'C2', 'C3', 'A3', 'A4', 'B2', 'B3', 'C4', 'A5', 'B4'];
          const TOLERANCE = 2; // 1-2건 차이는 DISTINCT/분배 기준 차이로 허용
          for (const code of itemCodes) {
            const imp = verifyCounts.import[code] ?? 0;
            const db = verifyCounts.db[code] ?? 0;
            // DB < Import - tolerance: 데이터 손실 가능성 → 경고
            // DB >= Import: 분배 알고리즘(B3/B4 등)으로 복제 생성 → 정상
            if (imp > 0 && db < imp - TOLERANCE) {
              mismatchItems.push(`${code}: Import=${imp}, DB=${db} (${imp - db}건 손실)`);
            }
          }
          if (mismatchItems.length > 0) {
            dbMismatchWarning = `\n\n⚠️ Import→DB 데이터 손실 감지:\n${mismatchItems.join('\n')}`;
          }
        }
