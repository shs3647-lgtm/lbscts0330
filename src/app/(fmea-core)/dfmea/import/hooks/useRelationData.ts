...
export function useRelationData(flatData: ImportedFlatData[], relationTab: 'A' | 'B' | 'C') {
  ...
  const getAllProcessNos = (tab: 'A' | 'B' | 'C'): { processNo: string; displayNo: string }[] => {
    ...
    // ★ 공통공정(0, 00, 공통공정, 공통) 제외 — 워크시트에서 수동 매칭
    // (이전: processNo '0'을 '공통공정'으로 추가했으나 제거)

    // 2. 해당 탭의 모든 데이터에서 processNo 추가 수집 (A1 없어도 표시되도록)
    // ✅ 유효한 값이 있는 데이터만 수집
    // ★★★ L1(C 카테고리)은 별도 구분자(YP/SP/USER)를 사용하므로 제외 (2026-02-02) ★★★
    const tabPrefix = tab; // 'A', 'B', 'C'
    flatData.filter(d =>
      d.itemCode?.startsWith(tabPrefix) &&
      d.processNo &&
      d.category !== 'C' &&  // L1(C 카테고리)은 별도 그룹
    ...
    // ✅ 빈 displayNo 필터링 (단, "0" 또는 "00"은 "공통공정"으로 허용)
    // ★ 공통공정(0, 00, 공통공정, 공통) 제외
    const isCommon = (pNo: string) => {
