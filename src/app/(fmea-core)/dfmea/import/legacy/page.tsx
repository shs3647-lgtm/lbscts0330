...
export default function LegacyImportPage() {
  ...
  const handleWorksheetSaved = useCallback(() => {
    if (selectedFmeaId) {
      window.location.href = `/dfmea/worksheet?id=${selectedFmeaId}&tab=structure`;
    }
  }, [selectedFmeaId]);
  ...
  const l1Name = selectedFmea?.fmeaInfo?.subject || selectedFmea?.project?.productName || '';
