...
export function compareFCChains(
  ...
  for (const e of existing) {
    ...
    // 완화 맵: 같은 processNo+FC에 여러 FM이 있으면 첫 번째만
    const rKey = chainKeyRelaxed(e);
    if (!existingRelaxedMap.has(rKey)) {
      existingRelaxedMap.set(rKey, e);
    }
  }
  ...
  for (const d of derived) {
    ...
    // 2단계: 완화 매칭 (processNo + FC — SA 자동도출의 FM 할당 오류 대응)
    if (!e) {
      e = existingRelaxedMap.get(relaxedKey);
    }
