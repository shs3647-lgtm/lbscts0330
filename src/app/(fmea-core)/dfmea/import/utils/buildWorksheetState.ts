...
function buildL2Structure(
  ...
  for (const [pNo, items] of byProcess) {
    ...
    // A1/A2가 있는 경우에만 공정으로 인정 (C 카테고리 processNo=YP/SP/USER 제외)
...
function fillL2Data(process: Process, items: ImportedFlatData[]): void {
  ...
  // ★ A5: 고장형태 → A4 제품특성에 균등 배분 (productCharId FK)
  const allProductChars = process.functions.flatMap(f => f.productChars || []);

  if (allProductChars.length > 0 && a5Items.length > 0) {
    // ★ 균등 배분: distribute(A5, PC개수) → 각 PC에 할당된 FM만 연결
    const a5Dist = distribute(a5Items, allProductChars.length);
    const modes: L2FailureMode[] = [];
    allProductChars.forEach((pc, i) => {
      a5Dist[i].forEach(a5 => {
        modes.push({
          id: uid(),
          name: a5.value,
  ...
  // ★★★ 근본 방어: productChar에 연결된 FM이 없으면 placeholder 자동생성 ★★★
  // 원인: B4/A5 부분 존재 시 import-builder 폴백이 스킵되어 orphan PC 발생
  // 증상: "고장형태 선택" (FM 누락) 반복 발생
