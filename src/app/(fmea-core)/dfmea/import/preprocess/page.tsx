...
export default function PreprocessPage() {
  ...
  // 워크시트 이동
  const goToWorksheet = useCallback(() => {
    router.push(`/dfmea/worksheet?id=${encodeURIComponent(fmeaId)}`);
  }, [router, fmeaId]);
...
function DoneSection({
  ...
  return (
    <div className="bg-white rounded border border-green-200 p-6 text-center space-y-3">
      <div className="text-[16px] font-bold text-green-700">DB 저장 완료</div>
      {statistics && (
      ...
      <div className="flex items-center justify-center gap-2">
        <button
          onClick={onReset}
          className="px-4 py-2 text-[11px] rounded border border-gray-300 text-gray-600 hover:bg-gray-50"
        >
          다른 파일 전처리
        </button>
        <button
          onClick={onGoWorksheet}
          className="px-6 py-2 bg-blue-600 text-white text-[11px] font-bold rounded hover:bg-blue-700"
        >
          워크시트로 이동
        </button>
      </div>
