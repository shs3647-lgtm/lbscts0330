 ...
 * @author AI Assistant
 * @created 2025-12-26
 * @updated 2025-12-26 - 다운로드/Import 탭 추가
 * 
 * L2 고장형태, L3 고장원인, L1 고장영향, 다운로드, Import 탭
...
export default function RelationPreview({ data, onDataChange }: RelationPreviewProps) {
  ...
  // 공정번호 목록 추출 (A1 기초정보에 있는 것만)
  const processNos = useMemo(() => {
    // A1 기초정보에서만 공정번호 추출 (value를 표시용으로 사용)
    const a1Items = data.filter(d => d.itemCode === 'A1');
    const processNoMap = new Map<string, string>(); // processNo → value (표시용)
    ...
    // value 기준으로 정렬 (숫자 우선)
    return Array.from(processNoMap.entries())
      .map(([processNo, displayNo]) => displayNo)
      .sort((a, b) => {
  ...
  const handleSave = async () => {
    ...
    try {
      // localStorage에 저장 (키: pfmea-relation-data)
      const saveData = {
        data,
      ...
      localStorage.setItem('pfmea-relation-data', JSON.stringify(saveData));

      // 저장 완료 표시를 위한 약간의 딜레이
            ...
            ['C1', 'C2', 'C3', 'C4'].forEach((code, idx) => {
              ...
              if (value) {
                ...
                newData.push({
                  ...
                  processNo: productName,  // C1값(구분: YP/SP/USER)이 processNo
                  category: 'C',
                  itemCode: code,
                  value,
                  createdAt: new Date(),
    ...
    <div className="bg-white rounded-lg shadow-md">
      ...
      {activeTab === 'import' && (
        <div className="p-8 text-center">
          ...
          <h3 className="text-lg font-bold text-[#00587a] mb-2">관계형 DATA Import</h3>
          <p className="text-sm text-gray-600 mb-6">
