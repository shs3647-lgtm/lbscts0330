/**
 * @file page.tsx
 * @description DFMEA 리스트 페이지 - 모듈화 적용 + @tanstack/react-virtual 가상화
 * @version 3.2.0
 * @updated 2026-03-08 @tanstack/react-virtual 가상화 적용 (대량 프로젝트 성능 최적화)
 *
      ...
      try {
        ...
        if (linkageData.success && linkageData.data?.length > 0) {
          ...
          for (const link of linkageData.data) {
            if (link.dfmeaId) {
              const existing = linkageMap.get(link.dfmeaId);
              linkageMap.set(link.dfmeaId, {
  ...
  return (
    ...
    >
      <div className="font-[Malgun_Gothic]" style={{ padding: '16px 16px 16px 5px' }}>
            ...
            <thead className="sticky top-0 z-10">
              <tr className="bg-[#00587a] text-white" style={{ height: '28px' }}>
                ...
                {[
                  ...
                  { label: 'Type', field: 'fmeaType' },
                  { label: 'ID', field: 'id' },
                  { label: 'Rev', field: 'revisionNo' },
                  { label: '단계(Step)', field: 'step' },
                  { label: '공장(Plant)', field: 'engineeringLocation' },
                  { label: 'FMEA명(Name)', field: 'subject' },
                  { label: '고객사(Customer)', field: 'customerName' },
                  { label: '담당자(Resp.)', field: 'fmeaResponsibleName' },
                  { label: 'DFMEA', field: 'linkedDfmeaNo' },
                  { label: 'PFD', field: 'linkedPfdNo' },
                  { label: 'CP', field: 'linkedCpNo' },
                  { label: '현황(Status)', field: 'status' },
                  { label: '시작일(Start)', field: 'fmeaStartDate' },
                  { label: '목표완료일(Target)', field: 'fmeaRevisionDate' },
                ].map((col, i) => (
                  <th
                    key={i}
        ...
        {/* 상태바 */}
        <ListStatusBar filteredCount={filteredProjects.length} totalCount={projects.length} moduleName={CONFIG.moduleName} version="v3.2.0" />
      </div>
