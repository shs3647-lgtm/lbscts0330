...
export default function ProcessSelectModal({
  ...
  const clearAndSave = () => {
    ...
    if (selectedIds.size > 0) {
      ...
      if (totalL3ToDelete > 0) parts.push(`• 하위 부품(컴포넌트): ${totalL3ToDelete}개도 함께 삭제(${totalL3ToDelete} sub-elements will also be deleted)`);
    ...
    const message = `⚠️ 워크시트의 모든 공정을 삭제하시겠습니까?\n(Delete all processes from worksheet?)\n\n` +
      ...
      `• 하위 부품(컴포넌트)(Sub-elements): ${totalL3Count}개\n` +
      ...
      >
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-[11px] font-bold">서브시스템명 선택(Select Process)</h2>
          </div>
        ...
        <div className="px-3 py-1 border-b bg-gradient-to-r from-green-50 to-emerald-50 flex items-center justify-between">
          <span className="text-[10px] font-bold text-green-700">▼ 하위항목(Child): 서브시스템명(Main Process)</span>
          <div className="flex items-center gap-2">
