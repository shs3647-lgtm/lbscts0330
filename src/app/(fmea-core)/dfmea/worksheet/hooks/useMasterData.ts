 ...
 * @description DFMEA 마스터 데이터 접근 훅 - 모든 모달/화면에서 사용
 * @version 1.0.0
 * @created 2026-01-21
 */
...
export function useMasterData() {
  ...
  const getProductFunctions = (): SelectOption[] => 
    PRODUCT_FUNCTIONS.map((f, i) => ({ id: `pf-${i}`, name: f }));
  const getProductRequirements = (): SelectOption[] =>
    PRODUCT_REQUIREMENTS.map((r, i) => ({ id: `pr-${i}`, name: r }));

  // 2. 초점요소 (ASSY) 목록
  const getFocusElements = (): SelectOption[] =>
    FOCUS_ELEMENTS.map(fe => ({ id: fe.id, name: fe.name }));
  
  const getFocusElementFunctions = (feId: string): SelectOption[] => {
    const fe = FOCUS_ELEMENTS.find(f => f.id === feId);
  ...
  const getParts = (focusElementName?: string): SelectOption[] => {
    if (focusElementName) {
      const fe = FOCUS_ELEMENTS.find(f => f.name === focusElementName);
      if (fe) {
  ...
  const getPartFunctions = (partId: string): SelectOption[] => {
    ...
    return part ? part.functions.map((f, i) => ({ id: `${partId}-f${i}`, name: f })) : [];
  };
  
  const getPartRequirements = (partId: string): SelectOption[] => {
    const part = PARTS.find(p => p.id === partId);
    return part ? part.requirements.map((r, i) => ({ id: `${partId}-r${i}`, name: r })) : [];
  };

  // 4. 고장 분석 데이터
  const getFailureEffects = (): SelectOption[] =>
    FAILURE_EFFECTS.map(fe => ({ id: fe.id, name: fe.effect }));
  
  const getFailureModes = (category?: string): SelectOption[] => {
    const modes = category ? getFailureModesByCategory(category) : FAILURE_MODES;
    return modes.map(fm => ({ id: fm.id, name: fm.mode, category: fm.category }));
  };
  ...
  const getFailureCauses = (category?: string): SelectOption[] => {
    ...
    return causes.map(fc => ({ id: fc.id, name: fc.cause, category: fc.category }));
  };
  ...
  const getPreventionControls = (category?: string): SelectOption[] => {
    ...
    return controls.map(pc => ({ id: pc.id, name: pc.control, category: pc.category }));
  };
  ...
  const getDetectionControls = (category?: string): SelectOption[] => {
    ...
    return controls.map(dc => ({ id: dc.id, name: dc.control, category: dc.category }));
  };

  // 6. 전체 카테고리 목록
  const getCategories = () => {
    const cats = new Set<string>();
    FAILURE_MODES.forEach(fm => cats.add(fm.category));
