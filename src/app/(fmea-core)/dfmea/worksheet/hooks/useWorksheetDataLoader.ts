...
export function useWorksheetDataLoader({
  ...
  useEffect(() => {
    ...
    if (false /* localStorage.getItem(migrationKey) - DB Only 정책 */) {
      ...
      const keys = ['dfmea_worksheet_', 'dfmea_tab_', 'dfmea_riskData_', 'dfmea_atomic_'];
      keys.forEach(prefix => {
    ...
    (async () => {
        ...
        if (projectRes.ok) {
          ...
          if (projectData.success && projectData.projects?.length > 0) {
            ...
            if (currentProject) {
              ...
              projectL1Name = isDefault ? '' : `${baseName}+생산공정`;
            }
          }
        }
      } catch (e) {
      ...
      let savedTab = '';
      try {
        const storedTab = localStorage.getItem(`dfmea_tab_${normalizedFmeaId}`);
        if (storedTab) {
          savedTab = storedTab;
        }
      } catch (e) { /* ignore */ }
      ...
      const ensureL1Types = (l1: any) => {
        ...
        const finalL1Name = isPlaceholder && projectL1Name ? projectL1Name : currentName;
      ...
      // 레거시 데이터 로드 시도
      const keys = [`dfmea_worksheet_${normalizedFmeaId}`, `fmea-worksheet-${normalizedFmeaId}`];
      let savedData = null;
        ...
        try {
          ...
          if (typeof window !== 'undefined') {
            ...
            if (urlParams.get('reset-fm') === 'true') {
              ...
              localStorage.setItem(`dfmea_worksheet_${normalizedFmeaId}`, JSON.stringify(parsed));
          ...
          if (parsed.l1 && parsed.l2 && (hasValidL1 || hasValidL2)) {
            ...
            if (!migratedL1.name || migratedL1.name.trim() === '') {
              const atomicKey = `dfmea_atomic_${normalizedFmeaId}`;
