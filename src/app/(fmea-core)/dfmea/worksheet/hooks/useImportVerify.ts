...
export interface ImportCounts {
  processCount: number;
  l3Count: number;          // B1 부품(컴포넌트)
  l1FuncCount: number;      // C2 1L기능
  l2FuncCount: number;      // A3 2L기능
  productCharCount: number; // A4 설계특성
  l3FuncCount: number;      // B2 3L기능
  processCharCount: number; // B3 설계파라미터
...
function computeImportCounts(flatItems: FlatItem[]): ImportCounts {
  ...
  const l3s = new Set<string>();         // B1 부품(컴포넌트)
  const l1Funcs = new Set<string>();     // C2 1L기능
  const l2Funcs = new Set<string>();     // A3 2L기능
  const productChars = new Set<string>();// A4 설계특성
...
export interface ImportChain {
  ...
  feValue: string;
  feScope?: string;
  pcValue?: string;   // B5 설계검증 예방 (FC시트)
  dcValue?: string;   // A6 설계검증 검출 (FC시트)
}
    ...
    (async () => {
      try {
        ...
        const rawChains: ImportChain[] = (json.dataset?.failureChains || []).map((c: Record<string, unknown>) => ({
          ...
          pcValue: (c.pcValue as string) || '',   // B5 설계검증 예방
          dcValue: (c.dcValue as string) || '',   // A6 설계검증 검출
        }));
...
export function buildVerifyItems(
  ...
  add('설계특성', importCounts.productCharCount, currentCounts.productCharCount);
  add('FM(고장형태)', importCounts.fmCount, currentCounts.fmCount);
  add('설계파라미터', importCounts.processCharCount, currentCounts.processCharCount);
