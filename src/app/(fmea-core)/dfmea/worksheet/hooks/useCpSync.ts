...
export function useCpSync(selectedFmeaId: string | null): UseCpSyncReturn {
  ...
  useEffect(() => {
    ...
    const fetchLinkedCpAndPfd = async () => {
      ...
      try {
        ...
        const regRes = await fetch(`/api/dfmea/${selectedFmeaId}`);
        let cpFound = false;
        let pfdFound = false;
  ...
  const handleCreateCp = useCallback(async (state: any) => {
    ...
    try {
      ...
      const res = await fetch('/api/dfmea/create-cp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          fmeaId: selectedFmeaId,
  ...
  const handleCreatePfd = useCallback(async (state: any) => {
    ...
    try {
      ...
      const res = await fetch('/api/dfmea/create-pfd', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          fmeaId: selectedFmeaId,
  ...
  const handleCpToFmea = useCallback(async (setState: React.Dispatch<any>) => {
    ...
    try {
      ...
      const res = await fetch('/api/dfmea/sync-from-cp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          cpNo: linkedCpNo,
  ...
  const handlePfdToFmea = useCallback(async (setState: React.Dispatch<any>) => {
    ...
    try {
      ...
      const res = await fetch('/api/dfmea/sync-from-pfd', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          pfdNo: linkedPfdNo,
  ...
  const executeNextStep = useCallback(async () => {
    ...
    try {
      ...
      switch (currentStep) {
        ...
        case 2:
          // 2단계: 구조 연동 API 호출
          const structRes = await fetch('/api/dfmea/sync-to-cp/structure', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        case 3:
          // 3단계: 제품특성 연동 API 호출
          const prodRes = await fetch('/api/dfmea/sync-to-cp/product-char', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ fmeaId, cpNo, l2Data: state.l2 }),
          });
        ...
        case 4:
          // 4단계: 공정특성 연동 API 호출
          const procRes = await fetch('/api/dfmea/sync-to-cp/process-char', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ fmeaId, cpNo, l2Data: state.l2 }),
          });
        ...
        case 5:
          // 5단계: 특별특성 연동 API 호출
          const specRes = await fetch('/api/dfmea/sync-to-cp/special-char', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
  ...
  const executeAllSteps = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch('/api/dfmea/sync-to-cp/all', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          fmeaId,
