...
export function useCpSync(selectedFmeaId: string | null): UseCpSyncReturn {
  ...
  const handleCreateCp = useCallback(async (state: any) => {
    ...
    const confirmed = window.confirm(
      ...
      `• 부품(컴포넌트) 수: ${state.l2.reduce((sum: number, p: any) => sum + (p.l3?.length || 0), 0)}개\n\n` +
  ...
  const handleCpToFmea = useCallback(async (setState: React.Dispatch<any>) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert(`✅ CP → FMEA 동기화 완료!\n\n• ${data.data?.count || 0}건의 데이터가 반영되었습니다.\n• 서브시스템기능: ${data.data?.l2Structures?.reduce((sum: number, l2: any) => sum + (l2.functions?.length || 0), 0) || 0}건\n• 화면이 자동으로 업데이트되었습니다.`);
      } else {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert(
          ...
          `• L3 부품(컴포넌트): ${data.data?.l3Count || 0}개\n` +
          `• 화면이 자동으로 업데이트되었습니다.`
        );
      } else {
        throw new Error(data.error || '동기화 실패');
  ...
  const startSyncWizard = useCallback(async (state: any) => {
    ...
    const steps: SyncStep[] = [
      ...
      { step: 3, name: '설계특성 연동', description: '설계특성 행 추가 (상위 병합 확장)', status: 'pending' },
      { step: 4, name: '설계파라미터 연동', description: '설계파라미터 매핑', status: 'pending' },
    ...
    (state.l2 || []).forEach((l2: any) => {
      ...
      // ★ L3.functions[].processChars 경로에서 설계파라미터 카운트 (신규 구조)
