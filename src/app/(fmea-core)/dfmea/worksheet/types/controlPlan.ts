 ...
 * │                    공정현황                    │        관리항목        │  관리기준  │ 조치기준 │
 * ├───────┬──────┬────┬────────┬──────┬──┬────┬──┬──────┬──────┬────┬──────┬────┬────┬────┬──┬──┬────┤
 * │공정번호│공정명│형태│공정설명│부품(컴포넌트)│EP│자동│NO│설계특성│설계파라미터│특별│규격/공차│측정│샘플│주기│관리│생산│품질│조치│
 * └───────┴──────┴────┴────────┴──────┴──┴────┴──┴──────┴──────┴────┴──────┴────┴────┴────┴──┴──┴────┘
 *
 * PFMEA 연동 필드:
 * - 공정번호/공정명 ↔ PFMEA L2.no/name (processNo, processName)
 * - 공정설명(D열) ↔ PFMEA L2 서브시스템 기능 (processDesc)
 * - 부품(컴포넌트) ↔ PFMEA L3 [4M + 부품(컴포넌트)명] (workElement)
 * - 설계특성 ↔ PFMEA L2.functions.productChars (productChar)
 * - 설계파라미터 ↔ PFMEA L3.functions.processChars (processChar)
 * - 특별특성 ↔ PFMEA specialChar (specialChar)
 * - 관리방법 ↔ PFMEA 설계검증 예방/설계검증 검출 (controlMethod)
 */
...
export interface CPRow {
  ...
  processDesc: string;      // 공정설명(D열) ↔ PFMEA L2 서브시스템 기능
  workElement: string;      // 부품(컴포넌트) ↔ PFMEA L3 [4M + 부품(컴포넌트)명]
  ...
  productChar: string;      // 설계특성 ↔ PFMEA 설계특성
  processChar: string;      // 설계파라미터 ↔ PFMEA 설계파라미터
  ...
  controlMethod: string;    // 관리방법 ↔ PFMEA 설계검증 예방/설계검증 검출
  ...
  pfmeaWorkElemId?: string; // 연결된 PFMEA 부품(컴포넌트) ID
...
export const CP_COLUMNS = [
  ...
  { key: 'workElement', label: '부품(컴포넌트)', width: 100, group: '공정현황', pfmeaSync: true },
  
  // 관리항목 (7열)
  { key: 'ep', label: 'EP', width: 50, group: '관리항목' },
  { key: 'autoInspect', label: '자동검사', width: 70, group: '관리항목' },
  { key: 'itemNo', label: 'NO', width: 40, group: '관리항목' },
  { key: 'productChar', label: '설계특성', width: 100, group: '관리항목', pfmeaSync: true },
  { key: 'processChar', label: '설계파라미터', width: 50, group: '관리항목', pfmeaSync: true },
