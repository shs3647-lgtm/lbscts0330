 ...
 * 
 * 4판 양식 열 구조:
 * NO+공정명 | 설계기능 | 고장형태(FM) | 특별특성 | 고장영향(FE) | 심각도 | 
 * 특별특성 | 고장원인(FC) | 설계검증 예방(PC) | 발생도 | 설계검증 검출(DC) | 검출도 | RPN |
 * 설계검증 예방개선 | 설계검증 검출개선 | 담당자 | 완료일 | 심각도 | 발생도 | 검출도 | RPN | 비고
 */
...
export interface Fmea4Row {
  ...
  // 3. 현재 관리
  preventionControl: string;  // 설계검증 예방 (PC)
  occurrence: number;         // 발생도 (O) 1-10
  detectionControl: string;   // 설계검증 검출 (DC)
  detection: number;          // 검출도 (D) 1-10
  rpn: number;                // RPN = S × O × D
  
  // 4. 개선 조치
  preventionImprove: string;  // 설계검증 예방 개선
  detectionImprove: string;   // 설계검증 검출 개선
...
export const FMEA4_COLUMNS = [
  ...
  { key: 'preventionControl', label: '설계검증 예방(PC)', width: 100 },
  { key: 'occurrence', label: '발생도', width: 45 },
  { key: 'detectionControl', label: '설계검증 검출(DC)', width: 100 },
  { key: 'detection', label: '검출도', width: 45 },
  { key: 'rpn', label: 'RPN', width: 50 },
  { key: 'preventionImprove', label: '설계검증 예방개선', width: 100 },
  { key: 'detectionImprove', label: '설계검증 검출개선', width: 100 },
