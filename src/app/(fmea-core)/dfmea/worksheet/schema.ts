...
// ============ 설계특성 (A4) - ProcessProductChar ============

/**
 * 설계특성 (ProcessProductChar - A4)
 * - 상위: L2Structure (서브시스템) - FK: l2StructId
...
export interface FailureCause extends AtomicRecord {
  ...
  processCharId?: string; // ✅ FK: 설계파라미터 ID (FailureMode의 productCharId와 동일 패턴)
...
export interface FailureAnalysis extends AtomicRecord {
  ...
  fcWorkElementName: string; // 부품(컴포넌트)명 (L3 Structure.name)
  ...
  l2ProductChar: string;  // 설계특성
  l2SpecialChar?: string; // 특별특성
  
  // L3 (부품(컴포넌트) 기능)
  l3FuncId: string;       // FK: L3Function.id
  l3FuncName: string;     // 부품(컴포넌트) 기능명
  l3ProcessChar: string;  // 설계파라미터
...
// P7+: canonical source re-export (crypto.getRandomValues 기반)
export { uid } from './schema/utils/uid';
 ...
 * - L3Structure: 각 공정에 최소 1개 부품(컴포넌트), 부품(컴포넌트)명 필수
 */
export function validateStructure(db: FMEAWorksheetDB): ValidationResult {
  ...
  if (db.l2Structures.length === 0) {
    errors.push({ level: 'error', field: 'l2Structures', message: '최소 1개의 서브시스템이 필요합니다.' });
    missingCount++;
  } else {
  ...
  db.l3Structures.forEach((l3, idx) => {
    ...
    if (!l3.name || l3.name.trim() === '' || l3.name.includes('클릭') || l3.name.includes('추가')) {
      errors.push({ level: 'error', field: `l3Structures[${idx}].name`, message: `부품(컴포넌트)명이 누락되었습니다.`, itemId: l3.id });
      missingCount++;
    }
    if (!l3.l2Id) {
      warnings.push({ level: 'warning', field: `l3Structures[${idx}].l2Id`, message: `부품(컴포넌트)의 상위 공정 FK가 누락되었습니다.`, itemId: l3.id });
    }
  });
 ...
 * - L2Function: 기능명, 설계특성 필수, l2StructId FK 필수
 * - L3Function: 기능명, 설계파라미터 필수, l3StructId FK 필수
 */
export function validateFunction(db: FMEAWorksheetDB): ValidationResult {
  ...
  db.l2Functions.forEach((f, idx) => {
    totalCount++;
    if (!f.functionName || f.functionName.trim() === '') {
      errors.push({ level: 'error', field: `l2Functions[${idx}].functionName`, message: '서브시스템 기능명이 누락되었습니다.', itemId: f.id });
      missingCount++;
    }
    if (!f.productChar || f.productChar.trim() === '') {
      errors.push({ level: 'error', field: `l2Functions[${idx}].productChar`, message: '설계특성이 누락되었습니다.', itemId: f.id });
      missingCount++;
    }
    if (!f.l2StructId) {
  ...
  db.l3Functions.forEach((f, idx) => {
    totalCount++;
    if (!f.functionName || f.functionName.trim() === '') {
      errors.push({ level: 'error', field: `l3Functions[${idx}].functionName`, message: '부품(컴포넌트) 기능명이 누락되었습니다.', itemId: f.id });
      missingCount++;
    }
    if (!f.processChar || f.processChar.trim() === '') {
      errors.push({ level: 'error', field: `l3Functions[${idx}].processChar`, message: '설계파라미터이 누락되었습니다.', itemId: f.id });
      missingCount++;
    }
    if (!f.l3StructId) {
      warnings.push({ level: 'warning', field: `l3Functions[${idx}].l3StructId`, message: '상위 구조분석(부품(컴포넌트)) FK가 누락되었습니다.', itemId: f.id });
    }
  });
