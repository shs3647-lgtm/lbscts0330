...
/** Row data with process display info */
interface RowWithProcessInfo {
 ...
 * @returns 행 데이터 배열
 */
export function generateRowsFromProcessGroups<T extends RowWithProcessInfo>(
  ...
  processGroups.forEach((pg) => {
    ...
    if (pg.startIdx >= 0 && allRows[pg.startIdx]) {
      ...
      allRows[pg.startIdx].showProcess = true;
    }
  });
