...
const SC_MASTER_KEY = 'dfmea_special_char_master';
 ...
 * 특별특성 마스터 통계
 */
export function getSCMasterStats(): { total: number; productSC: number; processSC: number; dfmeaLinked: number } {
  const masterData = getSpecialCharMaster();
  return {
    ...
    dfmeaLinked: masterData.filter(m => m.linkPFMEA).length,
  };
}
