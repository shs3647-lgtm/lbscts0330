 ...
 * @description DFMEA 워크시트 헤더 정보 컴포넌트
 * @version 1.0.0
 * @created 2025-12-26
 */
...
import { type DFMEAHeader } from './types';

interface WorksheetHeaderProps {
  header: DFMEAHeader;
  onChange?: (header: DFMEAHeader) => void;
  readOnly?: boolean;
}

/** 워크시트 헤더 정보 */
export function WorksheetHeader({ header, onChange, readOnly = false }: WorksheetHeaderProps) {
  const handleChange = (field: keyof DFMEAHeader, value: string | number) => {
    if (onChange) {
