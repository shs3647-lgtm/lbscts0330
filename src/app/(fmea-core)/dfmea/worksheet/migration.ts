...
interface OldWorksheetData {
  ...
  failureLinks?: Array<{
    ...
    fmProcessFunction?: string;  // ★ 설계기능 (역전개)
    fmProductChar?: string;  // ★ 설계특성 (역전개)
    ...
    fcId?: string;
    fcText?: string;
    fcWorkFunction?: string;  // ★ 부품(컴포넌트)기능 (역전개)
    fcProcessChar?: string;  // ★ 설계파라미터 (역전개)
...
export function migrateToAtomicDB(oldData: OldWorksheetData | any): FMEAWorksheetDB {
  ...
  // 3. L2 구조분석 (서브시스템) + L2 기능분석 + L3 구조/기능분석
  ...
  l2Data.forEach((proc: any, pIdx: number) => {
    ...
    failureModes.forEach((fm: any, fmLocalIdx: number) => {
      ...
      db.failureModes.push({
        ...
        specialChar: fm.sc,
        // ★★★ 모자관계 + 병합그룹 ★★★
        parentId: relatedL2Func.id,  // 부모: L2Function (설계특성)
        mergeGroupId,                 // 병합 그룹
        rowSpan: 1,                   // 기본값 (렌더링 시 계산)
      });
    });
    
    // L3 구조분석 (부품(컴포넌트)) + L3 기능분석
    ...
    procFailureCauses.forEach((fc: any, fcLocalIdx: number) => {
      ...
      db.failureCauses.push({
        ...
        // ★★★ 모자관계 + 병합그룹 ★★★
        parentId: relatedL3Func.id, // 부모: L3Function (설계파라미터)
...
export function convertToLegacyFormat(db: FMEAWorksheetDB): OldWorksheetData {
  ...
  db.failureLinks.forEach((link: any) => {
    ...
    result.failureLinks!.push({
      ...
      fmProcessFunction: analysis?.l2FuncName || '',  // ★ 설계기능 (역전개)
      fmProductChar: analysis?.l2ProductChar || '',    // ★ 설계특성 (역전개)
      ...
      fcWorkFunction: analysis?.l3FuncName || '',      // ★ 부품(컴포넌트)기능 (역전개)
      fcProcessChar: analysis?.l3ProcessChar || '',    // ★ 설계파라미터 (역전개)
