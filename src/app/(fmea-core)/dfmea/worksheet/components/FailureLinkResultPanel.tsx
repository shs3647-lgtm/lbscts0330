/**
 * @file FailureLinkResultPanel.tsx
 * @description 고장연결 결과 트리 패널 (page.tsx에서 분리)
 */
...
export default function FailureLinkResultPanel({ 
  state, 
  setState, 
  setDirty, 
  saveToLocalStorage 
    ...
    <>
      ...
      <div className="flex-1 overflow-auto p-1 bg-indigo-50">
        <table className="w-full border-collapse text-[8px]">
          <thead>
            <tr>
              <th colSpan={3} className="bg-blue-200 py-0.5 text-center font-bold border border-gray-300" title="Failure Effect">1. 고장영향(FE)</th>
              <th className="bg-amber-100 py-0.5 text-center font-bold border border-gray-300" title="Failure Mode">2. FM</th>
              <th colSpan={3} className="bg-green-200 py-0.5 text-center font-bold border border-gray-300" title="Failure Cause">3. 고장원인(FC)</th>
            </tr>
            <tr>
              <th className="bg-blue-100 py-0.5 text-center font-semibold border border-gray-300" title="Scope">구분(Scope)</th>
              <th className="bg-blue-100 py-0.5 text-center font-semibold border border-gray-300" title="Failure Effect">FE</th>
              <th className="bg-blue-100 py-0.5 text-center font-semibold border border-gray-300" title="Severity">S</th>
              <th className="bg-amber-100 py-0.5 text-center font-semibold border border-gray-300" title="Failure Mode">FM</th>
              <th className="bg-green-100 py-0.5 text-center font-semibold border border-gray-300">4M</th>
              <th className="bg-green-100 py-0.5 text-center font-semibold border border-gray-300" title="Work Element">작업요소(WE)</th>
              <th className="bg-green-100 py-0.5 text-center font-semibold border border-gray-300" title="Failure Cause">FC</th>
            </tr>
          </thead>
          <tbody>
            {resultLinks.length === 0 ? (
              ...
              return (
                <tr key={idx}>
                  ...
                  <td className="py-0.5 px-1 border border-gray-300 text-center" style={{ background: m4Bg }}>
                    {link.fcM4}
                  </td>
