/**
 * @file DfmeaContextMenu.tsx
 * @description DFMEA 워크시트 컨텍스트 메뉴 (CP 벤치마킹 - 구조분석~3L원인 전탭 지원)
 * @created 2026-01-27
...
export interface DfmeaContextMenuState {
    visible: boolean;
    x: number;
    y: number;
    rowIdx: number;
...
interface DfmeaContextMenuProps {
    contextMenu: DfmeaContextMenuState;
    onClose: () => void;
    // 행 추가/삭제 (CP 스타일)
...
export function DfmeaContextMenu({
    contextMenu,
    onClose,
    onInsertAbove,
    onInsertBelow,
    ...
    const getColumnLabel = () => {
        switch (columnType) {
            case 'l1': return 'L1 공정단계(Process Step)';
            case 'l2': return 'L2 작업요소(Work Element)';
            case 'l3': return 'L3 작업단계(Work Step)';
            case 'fm': return '고장형태(FM, Failure Mode)';
            case 'fc': return '고장원인(FC, Failure Cause)';
            case 'fe': return '고장영향(FE, Failure Effect)';
            case 'sod': return 'SOD 입력(SOD Input)';
            case 'ap': return 'AP 관리(AP Management)';
            default: return '일반(General)';
        }
    };
...
// 컨텍스트 메뉴 초기 상태
export const initialDfmeaContextMenu: DfmeaContextMenuState = {
    visible: false,
    x: 0,
    y: 0,
    rowIdx: -1,
