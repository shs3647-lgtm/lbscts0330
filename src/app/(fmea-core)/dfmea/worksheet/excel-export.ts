...
/**
 * 2L 서브시스템 기능분석 Excel 내보내기 (화면과 1:1 일치)
...
export async function exportFunctionL2(state: WorksheetState, fmeaName: string, includeFailure: boolean = false) {
  const ExcelJS = await loadExcelJS();
  const workbook = new ExcelJS.Workbook();
  const sheetName = includeFailure ? '2L 고장형태' : '2L 서브시스템기능';
  const worksheet = workbook.addWorksheet(sheetName, {
    properties: { tabColor: { argb: includeFailure ? 'C62828' : '1B5E20' } },
    views: [{ state: 'frozen', xSplit: 0, ySplit: 1 }]
  });
  ...
  const baseColumns = [
    ...
    { header: '서브시스템기능', key: 'function', width: 35 },
    { header: '설계특성', key: 'productChar', width: 25 },
  ...
  flatData.forEach((data) => {
    ...
    // 병합 추적 - 설계특성 + 특별특성 (col 3, 4) — 2L형태에서만 병합
    if (includeFailure) {
  ...
  const buffer = await workbook.xlsx.writeBuffer();
  saveExcelFile(buffer, `${fmeaName}_2L_${includeFailure ? '고장형태' : '서브시스템기능'}`);
}

/**
 * 3L 부품(컴포넌트) 기능분석 Excel 내보내기 (화면과 1:1 일치)
...
export async function exportFunctionL3(state: WorksheetState, fmeaName: string, includeFailure: boolean = false) {
  const ExcelJS = await loadExcelJS();
  const workbook = new ExcelJS.Workbook();
  const sheetName = includeFailure ? '3L 고장원인' : '3L 부품(컴포넌트)기능';
  const worksheet = workbook.addWorksheet(sheetName, {
    properties: { tabColor: { argb: includeFailure ? 'C62828' : '1B5E20' } },
    views: [{ state: 'frozen', xSplit: 0, ySplit: 1 }]
  });
  ...
  const baseColumns = [
    ...
    { header: '부품(컴포넌트)', key: 'workElem', width: 20 },
    { header: '부품(컴포넌트)기능', key: 'function', width: 30 },
    { header: '설계파라미터', key: 'processChar', width: 20 },
    { header: '특별특성', key: 'specialChar', width: 10 },
  ];
  ...
  const buffer = await workbook.xlsx.writeBuffer();
  const fileName = includeFailure ? `${fmeaName}_3L_고장원인` : `${fmeaName}_3L_부품(컴포넌트)기능`;
  saveExcelFile(buffer, fileName);
}
...
const ALLVIEW_COLUMNS = [
  ...
  { id: 'workElem', label: '부품(컴포넌트)', width: 16 },
  // 기능분석 (7)
  { id: 'l1Type', label: '구분', width: 6 },
  { id: 'l1Function', label: '완제품기능', width: 22 },
  { id: 'l1Requirement', label: '요구사항', width: 18 },
  { id: 'l2Function', label: '설계기능', width: 18 },
  { id: 'productChar', label: '설계특성', width: 14 },
  { id: 'l3Function', label: '부품(컴포넌트)기능', width: 16 },
  { id: 'processChar', label: '설계파라미터', width: 14 },
  // 고장분석 (4)
  ...
  // 리스크분석 (7)
  { id: 'prevention', label: '설계검증 예방', width: 14 },
  { id: 'occurrence', label: 'O', width: 4 },
  { id: 'detection', label: '설계검증 검출', width: 14 },
...
export async function exportAllViewExcel(state: WorksheetState, fmeaName: string) {
  ...
  // 2. FC → L3 기능분석 역전개 맵 (부품(컴포넌트)기능, 설계파라미터)
...
export async function exportStructureAnalysis(state: WorksheetState, fmeaName: string) {
  ...
  const columns = [
    ...
    { header: '부품(컴포넌트)', key: 'l3Name', width: 25 },
  ];
  worksheet.columns = columns;
  const headerRow = worksheet.getRow(1);
...
export async function downloadStructureTemplate() {
  ...
  const columns = ['완제품공정명', '공정번호', '공정명', '4M', '부품(컴포넌트)'];
