...
import { buildFailureChainsFromFlat } from '@/app/(fmea-core)/dfmea/import/types/masterFailureChain';
import {
...
function FMEAWorksheetPageContent() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      scrollSaveTimerRef.current = setTimeout(() => {
        try {
          localStorage.setItem(`dfmea_scrollLeft_${fmeaId}`, String(container.scrollLeft));
        } catch { /* ignore */ }
      }, 300);
    };
  ...
  useEffect(() => {
    ...
    try {
      const v = localStorage.getItem(`dfmea_scrollLeft_${fmeaId}`);
  ...
  const handleContextMenu = useCallback((
    ...
    fmId?: string,
    fcId?: string,
    columnType?: 'l1' | 'l2' | 'l3' | 'fm' | 'fc' | 'fe' | 'sod' | 'ap' | 'other',
    colKey?: string
  ) => {
    // 수동모드에서만 컨텍스트 메뉴 표시
    ...
    <>
      ...
      <div className="h-full flex flex-col font-[Segoe_UI,Malgun_Gothic,Arial,sans-serif]" style={{ background: COLORS.bg, color: COLORS.text, marginLeft: 53 }}>
              ...
              <div
                ...
                onClick={() => {
                  ...
                  try {
                    ...
                    localStorage.setItem(`dfmea_tab_${stateId}`, 'all');
        ...
        */}
        ...
        {/* ★★★ 2026-03-08: placeholder 컨텍스트 메뉴 제거
         * 각 탭(StructureTab, FunctionL1~L3, FailureL1~L3)이 자체 DfmeaContextMenu를 렌더링하므로
