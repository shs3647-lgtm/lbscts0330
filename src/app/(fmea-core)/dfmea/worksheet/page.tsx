...
import { DEFAULT_COMPARE_MASTER_FMEA_ID, normalizeCompareTab } from '../../pfmea/compare/constants';
...
function FMEAWorksheetPageContent() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // ★ 공통부품(컴포넌트)(00) 배분 (StructureTab.loadFromMaster와 동일 로직)
  ...
  // 부품(컴포넌트) 모달 연속입력 핸들러 (수동입력 후 즉시 워크시트 반영)
  ...
  // 부품(컴포넌트) 모달 삭제 핸들러 (2개 이상이면 행 삭제, 1개면 내용만 삭제)
      ...
      const newL2 = prev.l2.map(proc => {
        ...
        if (currentCount > 1) {
          ...
          if (remainingL3.length === 0) {
            remainingL3.push({ id: uid(), m4: '', name: '(클릭하여 부품(컴포넌트) 추가)', order: 10, functions: [], processChars: [] });
          }
          ...
          const updatedL3 = proc.l3.map(w => {
            ...
            if (isMatch) {
              return { ...w, name: '(클릭하여 부품(컴포넌트) 추가)', m4: '' };
            }
            return w;
          });
      ...
      >
        ...
        <TopMenuBar
          ...
          onCompareClick={() => {
            ...
            router.push(
              `/dfmea/compare?left=${encodeURIComponent(DEFAULT_COMPARE_MASTER_FMEA_ID)}&right=${encodeURIComponent(selectedFmeaId!)}&tab=${encodeURIComponent(tab)}`,
            );
          }}
                ...
                {currentFmea && !currentFmea.fmeaInfo?.subject && (
                  <div className="bg-blue-50 border border-blue-200 rounded p-3 m-3 text-sm text-blue-800">
                    ...
                    {!compareEmbed && (() => {
                      // 자동 리다이렉트 (1초 후)
                      setTimeout(() => {
                        router.push(`/dfmea/register?id=${currentFmea.id}`);
                      }, 1000);
                      return null;
                    })()}
                  </div>
