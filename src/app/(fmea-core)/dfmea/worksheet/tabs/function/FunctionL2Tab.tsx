 ...
 * @description 서브시스템(L2) 기능 분석 - 3행 헤더 구조 (L1과 동일한 패턴)
 *
...
// ┌──────────────────────────────────────────────────────────────────┐
// │ 2L기능 컨텍스트 메뉴 — 조작 대상: 공정(읽기전용) / 기능 / 설계특성 │
// │                                                                  │
// │ 공정(L2):        구조분석에서 연동, 여기서 추가/삭제 불가          │
// │ 기능(Function):  공정별로 자유롭게 추가/삭제                      │
// │ 설계특성(Char):  기능 안에서 자유롭게 추가/삭제                    │
// │                                                                  │
...
export default function FunctionL2Tab({ state, setState, setStateSynced, setDirty, saveToLocalStorage, saveAtomicDB, fmeaId, customerName, importCounts }: FunctionTabProps) {
  ...
  // ★★★ 2026-03-12 FIX: (기능 미입력)/(설계특성 미입력) placeholder도 누락으로 카운트 ★★★
  const missingCount = useMemo(() => {
    ...
    meaningfulProcs.forEach((proc: any) => {
      ...
      // ★★★ 2026-02-16: 상위레벨(기능) 입력 시 하위레벨(설계특성) 최소 1개 필수 ★★★
      funcs.forEach((f: any) => {
        if (f.name && f.name.trim()) {
          // ★ FIX: filterMeaningfulProductChars 사용하여 (설계특성 미입력) placeholder 제외
  ...
  // ★★★ 2026-03-06: 열 단위 분기 — 위로 새 행 추가 (process→공정은 구조분석, function→기능, productChar→설계특성) ★★★
  ...
  // ★★★ 2026-03-06: 열 단위 분기 — 아래로 새 행 추가 (function→기능, productChar→설계특성) ★★★
  ...
  // ★★★ 병합 위로 추가 - L1 벤치마킹 (rowType별 분기: 공정=새공정, 기능=새기능, 설계특성=새설계특성) ★★★
  const handleAddMergedAbove = useCallback(() => {
    ...
    const updateFn = (prev: WorksheetState) => {
      ...
      // ★ procId 미매칭 시 폴백: 기능/설계특성이면 첫 번째 공정(0)에 추가
      ...
      if (rowType === 'process') {
        ...
        // 설계특성 셀 우클릭 → 같은 기능 안에 새 설계특성 추가 (위로)
        const proc = newState.l2[procIdx];
        const funcIdx = proc.functions?.findIndex((f: any) => f.id === funcId) ?? -1;
        if (funcIdx < 0) {
          return prev;
  ...
  // ★★★ 병합 아래로 추가 - L1 벤치마킹 (rowType별 분기: 공정=새공정, 기능=새기능, 설계특성=새설계특성) ★★★
  const handleAddMergedBelow = useCallback(() => {
    ...
    const updateFn = (prev: WorksheetState) => {
      ...
      if (rowType === 'process') {
        ...
        // 설계특성 셀 우클릭 → 같은 기능 안에 새 설계특성 추가 (아래로)
        const proc = newState.l2[procIdx];
        const funcIdx = proc.functions?.findIndex((f: any) => f.id === funcId) ?? -1;
        if (funcIdx < 0) {
          return prev;
  ...
  const handleDeleteRow = useCallback(() => {
    ...
    if (rowType === 'productChar') {
      ...
      if (!charName) {
        ...
        if (!window.confirm(`설계특성 "${charName}"을(를) 삭제하시겠습니까?`)) return;
        const updateFn = (prev: WorksheetState) => {
          const newState = JSON.parse(JSON.stringify(prev));
          const pIdx = newState.l2.findIndex((p: any) => p.id === procId);
          if (pIdx >= 0) {
  ...
  return (
    <tr>
      ...
      <td className={cellP0} style={{ background: zebra.function }}>
        <SelectableCell value="" placeholder="설계기능 선택" bgColor={zebra.function} onClick={() => { }} />
      </td>
      <td className={cellP0} style={{ background: zebra.function }}>
        <SelectableCell value="" placeholder="설계특성 선택" bgColor={zebra.function} textColor={'#1b5e20'} onClick={() => { }} />
      </td>
          ...
          return (
            <tr key={proc.id} onContextMenu={(e) => handleContextMenu(e, 'function', proc.id, firstFuncId)}>
              ...
              <td className={cellP0} style={{ background: zebra.function }} onContextMenu={(e) => handleContextMenu(e, 'function', proc.id, firstFuncId)}>
                <SelectableCell value="" placeholder="설계기능 선택" bgColor={zebra.function} onClick={() => handleCellClick({ type: 'l2Function', procId: proc.id, title: '서브시스템 기능 선택', itemCode: 'A3' })} />
              </td>
              <td className={cellP0} style={{ background: zebra.function }}>
                <SelectableCell value="" placeholder="설계특성 선택" bgColor={zebra.function} textColor={'#1b5e20'} onClick={() => { }} />
              </td>
            ...
            return (
              <tr key={f.id} onContextMenu={(e) => handleContextMenu(e, 'function', proc.id, f.id)}>
                ...
                <td rowSpan={funcRowSpan} className="border border-[#ccc] p-0 align-middle" style={{ background: zebra.function }} onContextMenu={(e) => handleContextMenu(e, 'function', proc.id, f.id)}>
                  <SelectableCell value={f.name} placeholder="설계기능" bgColor={zebra.function} isRevised={f.isRevised} onClick={() => handleCellClick({ type: 'l2Function', procId: proc.id, funcId: f.id, title: '서브시스템 기능 선택', itemCode: 'A3' })} onDoubleClickEdit={(newValue) => handleInlineEditFunction(proc.id, f.id, newValue)} />
                </td>
                <td className={cellP0} style={{ background: zebra.function }} onContextMenu={(e) => handleContextMenu(e, 'productChar', proc.id, f.id, '')}>
                  <SelectableCell value="" placeholder="설계특성 선택" bgColor={zebra.function} textColor={'#1b5e20'} onClick={() => handleCellClick({ type: 'l2ProductChar', procId: proc.id, funcId: f.id, title: '설계특성 선택', itemCode: 'A4' })} />
                </td>
                <td className="border border-[#ccc] p-1 text-center text-[#999] text-xs align-middle" style={{ background: zebra.function }}>-</td>
              </tr>
            );
              ...
              <tr key={c.id} onContextMenu={(e) => handleContextMenu(e, 'productChar', proc.id, f.id, c.id)}>
                ...
                {cIdx === 0 && (
                  <td rowSpan={funcRowSpan} className="border border-[#ccc] p-0 align-middle" style={{ background: firstRowZebra.function }} onContextMenu={(e) => handleContextMenu(e, 'function', proc.id, f.id)}>
                    <SelectableCell value={f.name} placeholder="설계기능" bgColor={firstRowZebra.function} isRevised={f.isRevised} onClick={() => handleCellClick({ type: 'l2Function', procId: proc.id, funcId: f.id, title: '서브시스템 기능 선택', itemCode: 'A3' })} onDoubleClickEdit={(newValue) => handleInlineEditFunction(proc.id, f.id, newValue)} />
                  </td>
                )}
                <td className="border border-[#ccc] border-r-[2px] border-r-orange-500 p-0 align-middle" style={{ background: zebra.failure }} onContextMenu={(e) => handleContextMenu(e, 'productChar', proc.id, f.id, c.id)}>
                  <SelectableCell value={c.name} placeholder="설계특성" bgColor={zebra.failure} textColor={'#e65100'} isRevised={c.isRevised} onClick={() => handleCellClick({ type: 'l2ProductChar', procId: proc.id, funcId: f.id, charId: c.id, title: '설계특성 선택', itemCode: 'A4' })} onDoubleClickEdit={(newValue) => handleInlineEditProductChar(proc.id, f.id, c.id, newValue)} />
                </td>
