...
export function useFunctionL2Handlers({
  ...
  const handleConfirm = useCallback(() => {
    ...
    if (missing > 0) {
      _alert(`누락 ${missing}건이 있습니다. 모든 설계기능에 설계특성을 입력해주세요.`);
      return;
    }
      ...
      if (proc) {
        ...
        if (funcsWithChildren.length > 0) {
          const childCounts = funcsWithChildren.map((f: any) =>
            `• ${f.name}: 설계특성 ${(f.productChars || []).length}개`
          ).join('\n');

          const confirmed = confirm(
            `⚠️ 해제한 기능에 하위 데이터가 있습니다.\n\n${childCounts}\n\n적용하면 하위 데이터(설계특성)도 함께 삭제됩니다.\n정말 적용하시겠습니까?`
          );
          ...
          for (let i = startIdx; i < selectedValues.length; i++) {
            ...
            if (!existingNames.has(val)) {
              ...
              if (autoLinkedChars.length > 0) {
                const message = getAutoLinkMessage(autoLinkedChars.map(c => c.name), '설계특성');
              }
            }
          }
        ...
        newState.l2 = newState.l2.map((proc: any) => {
          ...
          return {
            ...
            functions: (proc.functions || []).map((f: any) => {
              ...
              // ★ FIX: 의미 있는 설계특성이 있으면 빈 placeholder 제거
