/**
 * @file FunctionColgroup.tsx
 * @description 기능분석 탭 컬럼 정의 (구조분석 열 포함)
 * @version 2.0.0 - 인라인 스타일 제거, Tailwind CSS 적용
 * @note ⚠️ 1L 기능분석 탭은 이 컴포넌트를 사용하지 않음!
 *       FunctionL1Tab.tsx 자체 colgroup 사용 (line ~464)
 *       L2/L3 탭에서만 사용됨
 */

'use client';

import React from 'react';

export default function FunctionColgroup() {
  // 구조분석(4열): 완제품명, 공정명, 4M, 부품(컴포넌트)
  // 기능분석(7열): L1구분, 완제품기능, 요구사항, 설계기능, 설계특성, 부품(컴포넌트)기능, 설계파라미터
  return (
    <colgroup>
      <col className="w-[100px]" />
      <col className="w-[120px]" />
      <col className="w-[80px]" />
      <col className="w-[100px]" />
      <col className="w-[35px]" />
      <col className="w-[225px]" />
      <col className="w-[150px]" />
      <col className="w-[150px]" />
      <col className="w-[120px]" />
      <col className="w-[150px]" />
      <col className="w-[120px]" />
    </colgroup>
  );
}
