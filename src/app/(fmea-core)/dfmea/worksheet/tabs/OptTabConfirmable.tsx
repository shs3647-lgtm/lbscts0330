  ...
  const handleConfirm = useCallback(() => {
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        // ✅ 버전 백업 API 호출
        if (fmeaId) {
          const atomicKey = `dfmea_atomic_${fmeaId.toLowerCase()}`;
    ...
    setTimeout(() => {
      ...
      if (confirm('🎉 FMEA 작성이 완료되었습니다!\n\nFMEA를 승인하시겠습니까?\n\n[확인] → 개정관리 화면으로 이동\n[취소] → 현재 화면 유지')) {
        router.push(`/dfmea/revision?id=${fmeaIdLocal}`);
      }
    }, 200);
  ...
  return (
    <>
      <thead className={tw.thead}>
        ...
        <tr>
          <th colSpan={14} className={tw.mainHeader}>
            <div className="flex items-center justify-between">
              <span className="flex-1 text-center"><BiHeader ko="D-FMEA 최적화" en="Optimization (6ST)" /></span>
        ...
        {/* 2행: 서브그룹 */}
        <tr>
          <th colSpan={4} className={`${tw.subHeader} ${tw.planHeader}`}><BiHeader ko="계획" en="Plan" /></th>
          <th colSpan={3} className={`${tw.subHeader} ${tw.monitorHeader}`}><BiHeader ko="결과 모니터링" en="Result Monitoring" /></th>
          <th colSpan={7} className={`${tw.subHeader} ${tw.evalHeader}`}><BiHeader ko="효과 평가" en="Effect Evaluation" /></th>
        </tr>

        {/* 3행: 컬럼명 */}
        <tr>
          <th className={`${tw.colHeader} ${tw.planCell}`}><BiHeader ko="조치" en="Action" /></th>
          <th className={`${tw.colHeader} ${tw.planCell}`}><BiHeader ko="책임자" en="Person" /></th>
          <th className={`${tw.colHeader} ${tw.planCell}`}><BiHeader ko="목표완료일" en="Target Date" /></th>
          <th className={`${tw.colHeader} ${tw.planCell}`}><BiHeader ko="조치완료일" en="Completion" /></th>
          ...
          <th className={`${tw.colHeader} ${tw.evalCell}`}><BiHeader ko="특별특성" en="SC" /></th>
          <th className={`${tw.colHeader} ${tw.evalCell}`}><BiHeader ko="심각도" en="Severity" /></th>
          <th className={`${tw.colHeader} ${tw.evalCell}`}><BiHeader ko="습득교훈" en="LLD" /></th>
          <th className={`${tw.colHeader} ${tw.evalCell}`}><BiHeader ko="비고" en="Remark" /></th>
          <th className={`${tw.colHeader} ${tw.evalCell}`}><BiHeader ko="평가일" en="Eval. Date" /></th>
        </tr>
      </thead>
