  ...
  return (
    <>
      ...
      <tr>
        ...
        <th className="bg-[#ff9800] text-white border border-[#ccc] p-1 h-5.5 font-bold text-xs">부품(컴포넌트)</th>
...
export function FailureRow({
  row, idx, l1Spans, l2Spans, state, setState, setStateSynced, setDirty, handleInputBlur, handleInputKeyDown, saveToLocalStorage,
    ...
    <>
      {l1Spans[idx] > 0 && (
        ...
        >
          <EditableCell
            ...
            onChange={(val) => { updateState(prev => ({ ...prev, l1: { ...prev.l1, failureEffect: val } })); setDirty(true); }}
      ...
      {l1Spans[idx] > 0 && (
        <td rowSpan={l1Spans[idx]} className={`border border-[#ccc] p-0 ${severityBg} align-middle text-center`}>
          <SeverityCell value={row.l1Severity ? Number(row.l1Severity) : undefined} onChange={(val) => { updateState(prev => ({ ...prev, l1: { ...prev.l1, severity: val } })); setDirty(true); }} saveToLocalStorage={saveToLocalStorage} />
        </td>
      )}
      {l2Spans[idx] > 0 && (
        <td rowSpan={l2Spans[idx]} className="border border-[#ccc] p-0.5 px-1 bg-orange-50 align-middle break-words">
          <EditableCell
            ...
            onChange={(val) => { updateState(prev => ({ ...prev, l2: prev.l2.map(p => p.id === row.l2Id ? { ...p, failureMode: val } : p) })); setDirty(true); }}
            onBlur={handleInputBlur} onKeyDown={handleInputKeyDown}
          />
        </td>
      )}
      ...
      <td className="border border-[#ccc] p-0.5 px-1 bg-orange-50 break-words">
        <EditableCell
          ...
          onChange={(val) => { updateState(prev => ({ ...prev, l2: prev.l2.map(p => ({ ...p, l3: p.l3.map(w => w.id === row.l3Id ? { ...w, failureCause: val } : w) })) })); setDirty(true); }}
          onBlur={handleInputBlur} onKeyDown={handleInputKeyDown}
        />
      </td>
    </>
