 ...
 * @description D-FMEA 고장영향(FE) 텍스트 → 심각도(S) 키워드 기반 매핑
 *
 * AIAG VDA FMEA 1st Edition (2차 정오표) 기준
 * D-FMEA 심각도: 3열 구조 (자사공장/고객사/최종사용자)
 *
 * 매칭 우선순위:
...
// =====================================================
// D-FMEA 심각도 키워드 규칙 (AIAG VDA 1st Edition)
// =====================================================
...
/**
 * AIAG VDA D-FMEA 심각도 등급명 (1st Edition, 2차 정오표)
 *
 ...
 */
export const DFMEA_SEVERITY_RULES: SeverityRule[] = [
  // ══════════════════════════════════════════
...
export function matchFESeverity(feText: string): SeverityMatch[] {
  ...
  if (prefixS !== null) {
    const rule = DFMEA_SEVERITY_RULES.find(r => r.rating === prefixS);
    return [{
      rating: prefixS,
      level: rule?.level || `S=${prefixS}`,
      score: 2.0,  // 접두사 매칭 = 최고 점수
      matchedKeywords: [`S${prefixS}: prefix`],
      ruleIndex: rule ? DFMEA_SEVERITY_RULES.indexOf(rule) : -1,
    }];
  }
  ...
  DFMEA_SEVERITY_RULES.forEach((rule, ruleIdx) => {
