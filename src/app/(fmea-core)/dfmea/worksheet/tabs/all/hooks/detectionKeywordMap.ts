 ...
 * @description FM(고장형태) → DC(검출관리) 키워드 기반 매칭 테이블 — v2.1
 *
 * ★ v2.1 변경: 모든 보조검출방법을 D≤4 장비로 교체.
 *   "육안검사"(D=8), "한도견본 비교"(D=7), "체크리스트"(D=7) 등은
 *   추천 대상에서 제외. Import 원본에 존재하면 유지.
 *
 * @created 2026-02-23
 * @updated 2026-02-26 v2.1 보조검출 D≤4 전면 교체
 */
...
export const FM_TO_DC_RULES: DetectionRule[] = [
  // 1. 사양/품번/규격 관련
  {
    fmKeywords: ['사양', '품명', '품번', '규격', '버전', '모델', '식별', '라벨', '표시', '이종'],
  ...
  // 5. 미작동/오작동/기능/NVH
  {
    ...
    primaryMethods: ['기능검사기', 'EOL 검사기'],
    secondaryMethods: ['통전검사'],
    priority: 5,
  },
  // 6. 도포/그리스/접착
  {
  ...
  // 9. 균일도/얼룩/색상/변색
  {
    fmKeywords: ['균일도', '얼룩', '색상', '색차', '변색', '무라'],
    primaryMethods: ['비전검사', 'EOL 검사기'],
  ...
  // 13. 용접/너겟/스패터/인장
  {
    fmKeywords: ['용접', '너겟', '스패터', '비드', '용접강도', '접합', '인장'],
    primaryMethods: ['인장시험기', '비파괴검사'],
    secondaryMethods: ['X-ray'],
    priority: 13,
  },
  // 14. 누설/기밀/리크/누출
  {
    fmKeywords: ['누설', '누유', '기밀', '리크', '누출'],
    primaryMethods: ['기밀시험기', '리크테스트'],
  ...
  // 22. 도장/코팅/도막/박리
  {
    fmKeywords: ['도장', '코팅', '도막', '박리', '기포', '흘러내림', '오렌지필'],
    primaryMethods: ['도막두께 측정기', '부착력 테스트'],
    secondaryMethods: ['광택도 측정'],
    priority: 22,
  },
  // 23. 열처리/담금질/침탄/질화
  {
