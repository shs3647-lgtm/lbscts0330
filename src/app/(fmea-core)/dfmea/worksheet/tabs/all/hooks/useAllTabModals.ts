...
export interface ControlModalState {
  ...
  fcId?: string;    // ★ 고유 키용
  fcText?: string;
  fmText?: string;       // ★ 2026-02-28: 검출관리 2섹션 모달용 (FM 검출 1순위)
...
export interface LLDModalState {
  ...
  pcText?: string;       // ★ 2026-03-02: 현재 행 예방관리
  dcText?: string;       // ★ 2026-03-02: 현재 행 검출관리
}
...
export function useAllTabModals(
    ...
    setState((prevState: WorksheetState) => {
      ...
      if (sodModal.category === 'D' && sodModal.targetType === 'risk' && uniqueKey) {
        ...
        if (currentDC) {
          ...
          // ★ 동일 검출관리 → 전체 FM에 검출도 자동적용 (공정별 제한 제거)
  ...
  /** ★ LLD 선택 완료 핸들러 — 예방관리개선 또는 검출관리개선에 선택 반영 */
  const handleLldSelect = async (lldNo: string, fmeaId?: string, detail?: { lldNo: string; applyTarget?: 'prevention' | 'detection'; improvement: string; failureMode: string; cause: string }) => {
