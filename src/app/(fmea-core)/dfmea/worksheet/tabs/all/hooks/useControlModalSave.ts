...
export function useControlModalSave({
  ...
  const handleSave = useCallback((selectedValues: string[]) => {
    ...
    // ★ 설계검증 예방/설계검증 검출만 P:/D: 접두어 추가 (특별특성 등은 제외)
    ...
    setState((prev: WorksheetState) => {
      ...
      // 설계검증 검출 선택 시 검출도 자동 적용 (현재 행 + 동일 FM 내)
      ...
      // 같은 공정 + 동일원인 → 설계검증 예방/설계검증 검출 자동연결 + O/D 점수 동반 복사
      ...
      // 설계검증 검출 - 같은 공정 + 동일 FM텍스트 → 빈 셀에 DC+D 자동연결
      ...
      // 자동연결 2: 동일 설계검증 예방에 동일 발생도 자동연결
      ...
      // 자동연결 3: 동일 고장형태(FM 텍스트) 기준 동일 설계검증 검출에 동일 검출도
    ...
    // ─── 알림 ───
    if (isAutoLinkType && autoLinkedCount > 0 && currentFcText) {
      const typeName = controlModal.type === 'prevention' ? '설계검증 예방' : '설계검증 검출';
  ...
  const modalTitle = useMemo(() => {
    switch (controlModal.type) {
      case 'prevention': return '설계검증 예방 선택';
      case 'prevention-opt': return '설계검증 예방개선 선택';
      case 'detection': return '설계검증 검출 선택';
      case 'detection-opt': return '설계검증 검출개선 선택';
  ...
  const currentValues = useMemo((): string[] => {
    ...
    const isDetection = controlModal.type === 'detection' || controlModal.type === 'detection-opt';
    // ★ 2026-02-28: 설계검증 검출 → [FM]/[FC] 마커 보존 (DetectionSectionModal에 전달)
  ...
  // ─── 검출/예방 모드 전환 제거 ───
  // ★ 설계검증 검출 모달은 설계검증 검출만, 설계검증 예방 모달은 설계검증 예방만 표시
