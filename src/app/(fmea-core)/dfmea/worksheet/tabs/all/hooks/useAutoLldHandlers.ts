 ...
 * - 설계검증 예방개선/설계검증 검출개선이 입력된 항목에 대해 LLD DB에서 best match 자동 매칭
  ...
  const handleAutoLld = useCallback(async () => {
    ...
    processedFMGroups.forEach(fmGroup => {
      fmGroup.rows.forEach(row => {
        ...
        if (needPrev) {
          const prev = findBestLld(lldItems, fmKeywords, fcKeywords, '설계검증 예방');
        ...
        if (needDet) {
          const det = findBestLld(lldItems, fmKeywords, fcKeywords, '설계검증 검출');
