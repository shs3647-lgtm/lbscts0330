 ...
 * @description 설계검증 예방(PC) 2섹션 선택 모달 — AIAG-VDA 발생도 평가 기준
 * - 1순위: 설계검증 예방 효과성 (B5 마스터) → O등급 표시
 * - 2순위: 추가 설계검증 예방 추천 (산업DB) → B/T/BP 분류 + O→O 개선 표시
...
export default function PreventionSectionModal({
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const imported = json.map((row, idx) => ({
        ...
        detection: null,
        improvement: String(row['설계검증 예방내용'] || '').trim(),
  ...
  // ── LLD Export ──
  const handleExport = () => {
    const HEADERS = ['공정번호','고장원인','설계검증 예방내용','O값','비고'];
    ...
    <div style={{
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <span style={{ fontSize: '13px', fontWeight: 700 }}>설계검증 예방 집중 매핑 (3-Panel)</span>
          {sodInfo && (
  ...
  return (
    <div style={{ padding: '6px 10px', display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '4px', flexWrap: 'wrap' }}>
        ...
        <span style={{ fontSize: '11px', fontWeight: 700, color: '#374151' }}>2순위: 추가 설계검증 예방 추천</span>
        <span style={{
