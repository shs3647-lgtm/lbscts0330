...
const HEADER_SHORT_MAP: Record<string, { ko: string; en: string }> = {
  ...
  '설계검증 예방개선': { ko: '예방개선', en: 'Prev. Improve' },
  '설계검증 검출개선': { ko: '검출개선', en: 'Det. Improve' },
            ...
            {span.step === '리스크분석' ? (
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '3px', flexWrap: 'wrap' }}>
                {handleManualPCDCFill && (
                  <button
                    ...
                    title="Import B5(설계검증 예방)/A6(설계검증 검출) 데이터로 빈 PC/DC 셀 전체 매칭"
                  >
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '3px', flexWrap: 'wrap' }}>
                <button
                  ...
                  title="PC/DC 매칭 + 산업DB 기반 개선추천 통합 (설계검증 예방개선/설계검증 검출개선에 적용)"
                >
                  개선추천(Improve Rec.)
                </button>
                {handleAutoLldFilter && (
