...
const PLAIN_PREFIXES = ['prevention-', 'detection-', 'prevention-opt-', 'detection-opt-', 'targetDate-opt-', 'completionDate-opt-', 'specialChar-', 'specialChar-opt-', 'status-', 'status-opt-', 'user-', 'lesson-', 'lesson-target-', 'lesson-opt-', 'result-', 'result-opt-', 'note-', 'note-opt-', 'person-', 'person-opt-', 'S-fe-', 'imported-O-', 'imported-D-', 'imported-prevention-', 'imported-detection-'];
...
export const RiskOptCellRenderer = React.memo(function RiskOptCellRendererInner({
  ...
  if (_isApL && optIdx === 0 && (col.step === '최적화' || col.name === 'LLD')) {
  ...
  // 발생도/검출도 (리스크분석 5단계)
  // ═══════════════════════════════════════════════════
  if ((col.name === '발생도' || col.name === '검출도') && col.step === '리스크분석') {
  ...
  if (col.name === 'LLD') {
    ...
    const fcKey = `filterCode-${uniqueKey}`;
    const fcValue = (state?.riskData?.[fcKey] as string) || '';
    ...
    const targetBadgeBg = lessonTarget === 'detection' ? '#fef3c7' : '#dbeafe';
    const targetBadgeColor = lessonTarget === 'detection' ? '#b45309' : '#1d4ed8';
    const FC_COLORS: Record<string, string> = { 'RMA': '#dc2626', 'ABN': '#d97706', 'CIP': '#2563eb', '변경점': '#7c3aed', 'ECN': '#7c3aed', 'FieldIssue': '#059669', 'DevIssue': '#0891b2' };
    const FC_OPTIONS = ['', 'RMA', 'ABN', 'CIP', 'ECN', 'FieldIssue', 'DevIssue'] as const;
    const optKey = lessonTarget === 'detection' ? `detection-opt-${uniqueKey}` : `prevention-opt-${uniqueKey}`;
    const optText = isLldNo ? (state?.riskData?.[optKey] as string) || '' : '';
    const tooltipText = isLldNo
      ? `${lldValue}${targetLabel ? ` → ${targetLabel === '예방' ? '예방관리개선' : '검출관리개선'}` : ''}${fcValue ? `\n분류: ${fcValue}` : ''}\n${optText ? `개선: ${optText}` : '클릭하여 LLD 상세 보기'}`
      : '클릭하여 LLD(필터코드) 선택';
    return (
      <td key={colIdx} rowSpan={fcRowSpan}
      ...
      >
        <div
          onClick={() => {
            if (isLldNo) { window.open('/dfmea/lld', '_blank'); return; }
          ...
          style={{ color: isLldNo ? '#00587a' : '#999', fontWeight: isLldNo ? 600 : 400, textDecoration: isLldNo ? 'underline' : 'none', fontSize: '8px', lineHeight: 1.1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}
        >
          {lldValue || '\u00A0'}
          {isLldNo && targetLabel && (
            <span style={{ marginLeft: 1, padding: '0 2px', borderRadius: 2, fontSize: '7px', fontWeight: 700, background: targetBadgeBg, color: targetBadgeColor, textDecoration: 'none', verticalAlign: 'middle' }}>
              {targetLabel}
            </span>
          )}
        </div>
        <select
          ...
          onChange={(e) => {
            ...
            setState?.((prev: WorksheetState) => ({ ...prev, riskData: { ...(prev.riskData || {}), [fcKey]: newVal } }));
            setDirty?.(true);
          }}
          style={{
            width: '100%', border: 'none', background: 'transparent', fontSize: '7px', textAlign: 'left', cursor: 'pointer',
            color: FC_COLORS[fcValue] || '#888', fontWeight: fcValue ? 700 : 400, outline: 'none', padding: 0, marginTop: '1px',
          }}
          title="분류 선택"
        >
          {FC_OPTIONS.map(opt => (
            <option key={opt} value={opt} style={{ color: FC_COLORS[opt] || '#333' }}>
              {opt || '-'}
            </option>
          ))}
        </select>
        {lldValue && (
          <span
            onClick={(e) => {
              e.stopPropagation();
              setState?.((prev: WorksheetState) => ({ ...prev, riskData: { ...(prev.riskData || {}), [lldKey]: '', [`lesson-target-${uniqueKey}`]: '' } }));
              setDirty?.(true);
            }}
