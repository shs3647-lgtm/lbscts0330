...
export const RiskOptCellRenderer = React.memo(function RiskOptCellRendererInner({
  ...
  // ═══════════════════════════════════════════════════
  // 설계검증 예방/설계검증 검출/설계검증 예방개선/설계검증 검출개선
  // ═══════════════════════════════════════════════════
  if (CONTROL_TYPES[col.name]) {
    ...
    // ★ Import 자동보완 하이라이트 (설계검증 예방/설계검증 검출)
  ...
  if (col.name === 'LLD') {
    ...
    // tooltip — LLD는 5단계 리스크분석 설계검증 예방/설계검증 검출에 적용
    ...
    const tooltipText = isLldNo
      ? `${lldValue}${clsValue ? ` [${clsValue}]` : ''}${targetLabel ? ` → ${targetLabel === '예방' ? '설계검증 예방' : '설계검증 검출'}` : ''}\n${ctrlText ? `관리: ${ctrlText}` : '클릭하여 LLD 변경'}`
      : '클릭하여 LLD 선택';
    return (
      <td key={colIdx} rowSpan={fcRowSpan}
