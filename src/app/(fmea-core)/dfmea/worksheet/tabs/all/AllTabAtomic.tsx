...
export default function AllTabAtomic({ fmeaId, visibleSteps = [2, 3, 4, 5, 6], setState, onNoData }: AllTabAtomicProps) {
  ...
  const getOptimizedWidth = useMemo(() => {
    ...
    const widthMap: Record<string, number> = {
      ...
      '5': 600,           // 5ST만: 리스크만
      '6': 600,           // 6ST만: 최적화만
      '2,3': 1380,        // 2ST+3ST: 구조+기능
      '3,4': 1800,        // 3ST+4ST: 기능+고장
      '4,5': 1500,        // 4ST+5ST: 고장+리스크
      '2,3,4': 1900,      // 2ST+3ST+4ST
      '2,3,4,5': 2500,    // 2ST+3ST+4ST+5ST
        ...
        <thead>
          ...
          <tr>
            {visibleSteps.includes(2) && (
              ...
              >
                D-FMEA 구조분석(Structure Analysis, 2단계)
              </th>
            )}
            {visibleSteps.includes(3) && (
              <th
              ...
              >
                D-FMEA 기능분석(Function Analysis, 3단계)
              </th>
            )}
            {visibleSteps.includes(4) && (
              <th
              ...
              >
                D-FMEA 고장분석(Failure Analysis, 4단계)
              </th>
            )}
            {visibleSteps.includes(5) && (
              <th
              ...
              >
                D-FMEA 리스크분석(Risk Analysis, 5단계)
              </th>
            )}
            {visibleSteps.includes(6) && (
              <th
          ...
          <tr>
            {visibleSteps.includes(2) && (
              <>
                <th style={stickyHeaderCellStyle(COLORS.structure.main, HEADER_ROW_H, '#fff', 59, '9px', 600)} title="Product Line">1.완제품<br />공정명</th>
                <th style={stickyHeaderCellStyle(COLORS.structure.main, HEADER_ROW_H, '#fff', 59, '9px', 600)} title="Process Step">2.메인<br />공정명</th>
                <th colSpan={2} style={stickyHeaderCellStyle(COLORS.structure.main, HEADER_ROW_H, '#fff', 59, '9px', 600)} title="Work Element">3.작업<br />요소명</th>
              </>
            )}
            {visibleSteps.includes(3) && (
              <>
                <th colSpan={3} style={stickyHeaderCellStyle(COLORS.function.main, HEADER_ROW_H, '#fff', 59, '9px', 600)} title="Product Function / Requirement">1.완제품공정기능<br />/요구사항</th>
                <th colSpan={2} style={stickyHeaderCellStyle(COLORS.function.main, HEADER_ROW_H, '#fff', 59, '9px', 600)} title="Process Function / Product Characteristic">2.메인공정기능<br />및 제품특성</th>
                <th colSpan={3} style={stickyHeaderCellStyle(COLORS.function.main, HEADER_ROW_H, '#fff', 59, '9px', 600)} title="Work Element Function / Process Characteristic">3.작업요소기능<br />및 공정특성</th>
              </>
            )}
            {visibleSteps.includes(4) && (
              <>
                <th colSpan={3} style={stickyHeaderCellStyle('#f9a825', HEADER_ROW_H, '#333', 59, '9px', 600)} title="Failure Effect">1.자사/고객/사용자<br />고장영향(FE)</th>
                <th colSpan={2} style={stickyHeaderCellStyle('#7e57c2', HEADER_ROW_H, '#fff', 59, '9px', 600)} title="Failure Mode">2.메인공정<br />고장형태(FM)</th>
                <th colSpan={2} style={stickyHeaderCellStyle('#66bb6a', HEADER_ROW_H, '#fff', 59, '9px', 600)} title="Failure Cause">3.작업요소<br />고장원인(FC)</th>
              </>
            )}
            {visibleSteps.includes(5) && (
              <>
                <th colSpan={2} style={stickyHeaderCellStyle(COLORS.risk.main, HEADER_ROW_H, '#fff', 59, '9px', 600)} title="Current Prevention Control">현재<br />예방관리(PC)</th>
                <th colSpan={2} style={stickyHeaderCellStyle(COLORS.risk.main, HEADER_ROW_H, '#fff', 59, '9px', 600)} title="Current Detection Control">현재<br />검출관리(DC)</th>
                <th colSpan={4} style={stickyHeaderCellStyle(COLORS.risk.main, HEADER_ROW_H, '#fff', 59, '9px', 600)} title="Risk Evaluation">리스크<br />평가</th>
              </>
            )}
            {visibleSteps.includes(6) && (
              <>
                <th colSpan={4} style={stickyHeaderCellStyle(COLORS.opt.main, HEADER_ROW_H, '#fff', 59, '9px', 600)} title="Plan">계획<br />(Plan)</th>
                <th colSpan={3} style={stickyHeaderCellStyle(COLORS.opt.main, HEADER_ROW_H, '#fff', 59, '9px', 600)} title="Result Monitoring">결과<br />모니터링</th>
                <th colSpan={8} style={stickyHeaderCellStyle(COLORS.opt.main, HEADER_ROW_H, '#fff', 59, '9px', 600)} title="Effectiveness Evaluation">효과<br />평가</th>
              </>
            )}
          </tr>
          ...
          <tr>
            {visibleSteps.includes(2) && (
              <>
                <th style={stickyHeaderCellStyle(COLORS.structure.header, HEADER_ROW_H * 2, '#000', 58, '9px', 600)} title="Product Line">완제품<br />공정명</th>
                <th style={stickyHeaderCellStyle(COLORS.structure.header, HEADER_ROW_H * 2, '#000', 58, '9px', 600)} title="Process Step">NO+<br />공정명</th>
                <th style={stickyHeaderCellStyle(COLORS.special.m4.h3, HEADER_ROW_H * 2, '#000', 58, '8px', 700)} title="Man/Machine/Material/Environment">4M</th>
                <th style={stickyHeaderCellStyle(COLORS.structure.header, HEADER_ROW_H * 2, '#000', 58, '9px', 600)} title="Work Element">작업<br />요소</th>
              </>
            )}
            {visibleSteps.includes(3) && (
              <>
                <th style={stickyHeaderCellStyle(COLORS.special.scope.h3, HEADER_ROW_H * 2, '#000', 58, '8px', 700)} title="Category">구분</th>
                <th style={stickyHeaderCellStyle(COLORS.function.header, HEADER_ROW_H * 2, '#000', 58, '9px', 600)} title="Product Function">완제품<br />기능</th>
                <th style={stickyHeaderCellStyle(COLORS.function.header, HEADER_ROW_H * 2, '#000', 58, '9px', 600)} title="Requirement">요구<br />사항</th>
                <th style={stickyHeaderCellStyle(COLORS.function.header, HEADER_ROW_H * 2, '#000', 58, '9px', 600)} title="Process Function">공정<br />기능</th>
                <th style={stickyHeaderCellStyle(COLORS.function.header, HEADER_ROW_H * 2, '#000', 58, '9px', 600)} title="Product Characteristic">제품<br />특성</th>
                <th style={stickyHeaderCellStyle(COLORS.function.header, HEADER_ROW_H * 2, '#000', 58, '9px', 600)} title="Work Element">작업<br />요소</th>
                <th style={stickyHeaderCellStyle(COLORS.function.header, HEADER_ROW_H * 2, '#000', 58, '9px', 600)} title="Work Element Function">작업요소<br />기능</th>
                <th style={stickyHeaderCellStyle(COLORS.function.header, HEADER_ROW_H * 2, '#000', 58, '9px', 600)} title="Process Characteristic">공정<br />특성</th>
              </>
            )}
            {visibleSteps.includes(4) && (
              <>
                <th style={stickyHeaderCellStyle('#fff8e1', HEADER_ROW_H * 2, '#000', 58, '9px', 600)} title="Category">구분</th>
                <th style={stickyHeaderCellStyle('#fff8e1', HEADER_ROW_H * 2, '#000', 58, '9px', 600)} title="Failure Effect">고장영향<br />(FE)</th>
                <th style={stickyHeaderCellStyle('#fff8e1', HEADER_ROW_H * 2, '#000', 58, '9px', 600)} title="Severity">심각도(S)</th>
                <th style={stickyHeaderCellStyle('#ede7f6', HEADER_ROW_H * 2, '#000', 58, '9px', 600)}></th>
                <th style={stickyHeaderCellStyle('#ede7f6', HEADER_ROW_H * 2, '#000', 58, '9px', 600)} title="Failure Mode">고장형태<br />(FM)</th>
                <th style={stickyHeaderCellStyle('#e8f5e9', HEADER_ROW_H * 2, '#000', 58, '9px', 600)} title="Failure Cause">고장원인<br />(FC)</th>
                <th style={stickyHeaderCellStyle('#e8f5e9', HEADER_ROW_H * 2, '#000', 58, '9px', 600)} title="Work Element">작업<br />요소</th>
              </>
            )}
            {visibleSteps.includes(5) && (
              <>
                <th style={stickyHeaderCellStyle(COLORS.risk.prevention.header, HEADER_ROW_H * 2, '#000', 58, '9px', 600)} title="Prevention Control">예방관리<br />(PC)</th>
                <th style={stickyHeaderCellStyle(COLORS.indicator.occurrence.bg, HEADER_ROW_H * 2, COLORS.indicator.occurrence.text, 58, '8px', 700)} title="Occurrence">발생도<br />(O)</th>
                <th style={stickyHeaderCellStyle(COLORS.risk.detection.header, HEADER_ROW_H * 2, '#000', 58, '9px', 600)} title="Detection Control">검출관리<br />(DC)</th>
                <th style={stickyHeaderCellStyle(COLORS.indicator.detection.bg, HEADER_ROW_H * 2, COLORS.indicator.detection.text, 58, '8px', 700)} title="Detection">검출도<br />(D)</th>
                <th style={stickyHeaderCellStyle(COLORS.indicator.ap.bg, HEADER_ROW_H * 2, COLORS.indicator.ap.text, 58, '8px', 700)} title="Action Priority">AP</th>
                <th style={stickyHeaderCellStyle(COLORS.risk.evaluation.header, HEADER_ROW_H * 2, '#000', 58, '8px', 700)} title="Risk Priority Number">RPN</th>
                <th style={stickyHeaderCellStyle(COLORS.risk.evaluation.header, HEADER_ROW_H * 2, '#000', 58, '9px', 600)} title="Special Characteristic">특별<br />특성(SC)</th>
                <th style={stickyHeaderCellStyle('#f97316', HEADER_ROW_H * 2, '#fff', 58, '9px', 600)} title="Lessons Learned">습득<br />교훈(LLD)</th>
              </>
            )}
            {visibleSteps.includes(6) && (
              <>
                <th style={stickyHeaderCellStyle('#bbdefb', HEADER_ROW_H * 2, '#000', 58, '9px', 600)} title="Prevention Control Improvement">예방관리<br />개선(PC Opt)</th>
                <th style={stickyHeaderCellStyle('#bbdefb', HEADER_ROW_H * 2, '#000', 58, '9px', 600)} title="Detection Control Improvement">검출관리<br />개선(DC Opt)</th>
                <th style={stickyHeaderCellStyle('#bbdefb', HEADER_ROW_H * 2, '#000', 58, '9px', 600)} title="Responsible Person">책임자<br />성명</th>
                <th style={stickyHeaderCellStyle('#bbdefb', HEADER_ROW_H * 2, '#000', 58, '9px', 600)} title="Target Date">목표완료<br />일자</th>
                <th style={stickyHeaderCellStyle('#ffccbc', HEADER_ROW_H * 2, '#000', 58, '8px', 700)} title="Status">상태</th>
                <th style={stickyHeaderCellStyle('#ffccbc', HEADER_ROW_H * 2, '#000', 58, '9px', 600)} title="Evidence of Improvement">개선결과<br />근거</th>
                <th style={stickyHeaderCellStyle('#ffccbc', HEADER_ROW_H * 2, '#000', 58, '9px', 600)} title="Completion Date">완료<br />일자</th>
                <th style={stickyHeaderCellStyle('#c8e6c9', HEADER_ROW_H * 2, '#000', 58, '8px', 700)} title="Severity">심각도<br />(S)</th>
                <th style={stickyHeaderCellStyle('#c8e6c9', HEADER_ROW_H * 2, '#000', 58, '8px', 700)} title="Occurrence">발생도<br />(O)</th>
                <th style={stickyHeaderCellStyle('#c8e6c9', HEADER_ROW_H * 2, '#000', 58, '8px', 700)} title="Detection">검출도<br />(D)</th>
                <th style={stickyHeaderCellStyle('#c8e6c9', HEADER_ROW_H * 2, '#000', 58, '9px', 600)} title="Special Characteristic">특별<br />특성(SC)</th>
                <th style={stickyHeaderCellStyle('#c8e6c9', HEADER_ROW_H * 2, '#000', 58, '8px', 700)} title="Action Priority">AP</th>
                <th style={stickyHeaderCellStyle('#c8e6c9', HEADER_ROW_H * 2, '#000', 58, '8px', 700)} title="Risk Priority Number">RPN</th>
                <th style={stickyHeaderCellStyle('#c8e6c9', HEADER_ROW_H * 2, '#000', 58, '8px', 700)} title="Remarks">비고</th>
