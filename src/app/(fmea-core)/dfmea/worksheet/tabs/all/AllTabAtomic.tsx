...
// FC 부품(컴포넌트) (fontSize: '11px' 명시) 캐시 - getFailureCellStyle과 동일하므로 재사용
      ...
      <table style={{ borderCollapse: 'collapse', width: '100%', minWidth: getOptimizedWidth }}>
        ...
        <thead>
          ...
          <tr>
            ...
            {visibleSteps.includes(3) && (
              <>
                <th colSpan={3} style={stickyHeaderCellStyle(COLORS.function.main, HEADER_ROW_H, '#fff', 59, '9px', 600)}>1.완제품설계기능<br />/요구사항</th>
                <th colSpan={2} style={stickyHeaderCellStyle(COLORS.function.main, HEADER_ROW_H, '#fff', 59, '9px', 600)}>2.서브시스템기능<br />및 설계특성</th>
                <th colSpan={3} style={stickyHeaderCellStyle(COLORS.function.main, HEADER_ROW_H, '#fff', 59, '9px', 600)}>3.부품(컴포넌트)기능<br />및 설계파라미터</th>
              </>
            )}
            {visibleSteps.includes(4) && (
              <>
                <th colSpan={3} style={stickyHeaderCellStyle('#f9a825', HEADER_ROW_H, '#333', 59, '9px', 600)}>1.자사/고객/사용자<br />고장영향(FE)</th>
                <th colSpan={2} style={stickyHeaderCellStyle('#7e57c2', HEADER_ROW_H, '#fff', 59, '9px', 600)}>2.서브시스템<br />고장형태(FM)</th>
                <th colSpan={2} style={stickyHeaderCellStyle('#66bb6a', HEADER_ROW_H, '#fff', 59, '9px', 600)}>3.부품(컴포넌트)<br />고장원인(FC)</th>
              </>
            )}
            {visibleSteps.includes(5) && (
              <>
                <th colSpan={2} style={stickyHeaderCellStyle(COLORS.risk.main, HEADER_ROW_H, '#fff', 59, '9px', 600)}>현재<br />설계검증 예방</th>
                <th colSpan={2} style={stickyHeaderCellStyle(COLORS.risk.main, HEADER_ROW_H, '#fff', 59, '9px', 600)}>현재<br />설계검증 검출</th>
                <th colSpan={5} style={stickyHeaderCellStyle(COLORS.risk.main, HEADER_ROW_H, '#fff', 59, '9px', 600)}>리스크<br />평가</th>
              </>
            )}
            {visibleSteps.includes(6) && (
              <>
          ...
          <tr>
            ...
            {visibleSteps.includes(3) && (
              <>
                ...
                <th style={stickyHeaderCellStyle(COLORS.function.header, HEADER_ROW_H * 2, '#000', 58, '9px', 600)}>부품(컴포넌트)<br />기능</th>
            ...
            {visibleSteps.includes(5) && (
              <>
                <th style={stickyHeaderCellStyle(COLORS.risk.prevention.header, HEADER_ROW_H * 2, '#000', 58, '9px', 600)}>설계검증 예방<br />(PC)</th>
                ...
                <th style={stickyHeaderCellStyle(COLORS.risk.detection.header, HEADER_ROW_H * 2, '#000', 58, '9px', 600)}>설계검증 검출<br />(DC)</th>
                ...
                <th style={stickyHeaderCellStyle(COLORS.risk.evaluation.header, HEADER_ROW_H * 2, '#000', 58, '8px', 700)}>Filter<br />Code</th>
              </>
            )}
            {visibleSteps.includes(6) && (
              <>
                <th style={stickyHeaderCellStyle('#bbdefb', HEADER_ROW_H * 2, '#000', 58, '9px', 600)}>설계검증 예방<br />개선</th>
                <th style={stickyHeaderCellStyle('#bbdefb', HEADER_ROW_H * 2, '#000', 58, '9px', 600)}>설계검증 검출<br />개선</th>
