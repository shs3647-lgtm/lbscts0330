 ...
 * @description D-FMEA ALL 화면 (구조분석 CSS 벤치마킹)
 *
...
export default function AllTabEmpty({
  ...
  const handleNavigateToFailureLink = React.useCallback((fmId: string) => {
    ...
    try {
      localStorage.setItem(`dfmea_tab_${fmeaIdRef.current || 'default'}`, 'failure-link');
  ...
  const handleNavigateToAnalysisTab = React.useCallback((level: 'FE' | 'FM' | 'FC', itemId: string) => {
    ...
    try {
      localStorage.setItem(`dfmea_tab_${fmeaIdRef.current || 'default'}`, targetTab);
    } catch (e) { console.error('[AllTab] localStorage 저장 실패:', e); }
  }, [setState, saveAtomicDB]);
  ...
  const columns = useMemo(() => {
    ...
    return visibleSteps && visibleSteps.length > 0
      ? allCols.filter(col => visibleSteps.includes(col.step))
      : allCols;
  }, [showRPN, visibleSteps]);
    ...
    <div className="relative bg-white" style={{ display: 'inline-block', minWidth: '100%' }}>
      ...
      <table
        ...
        style={{
          ...
          width: (visibleSteps && visibleSteps.length <= 3) ? '100%' : `${totalWidth}px`,
          minWidth: (visibleSteps && visibleSteps.length <= 3) ? undefined : `${totalWidth}px`,
      ...
      >
        ...
        <colgroup>
          {columns.map((col, idx) => {
            const usePct = visibleSteps && visibleSteps.length <= 3;
        ...
        <AllTabHeader
          ...
          handleOMatch={handleOMatch}
          handleDMatch={handleDMatch}
          highlightMissingO={highlightMissingO}
          highlightMissingD={highlightMissingD}
          toggleHighlightO={toggleHighlightO}
          toggleHighlightD={toggleHighlightD}
          runAutoFix={runAutoFix}
          isAutoFixRunning={isAutoFixRunning}
        />
