 ...
 * | 서브시스템기능   | ✅ 연동        | ✅ 연동        | L2.functions            |
 * | 설계특성       | ✅ 연동        | ✅ 연동        | L2.functions.productChars|
 * | 특별특성       | ✅ 연동        | ✅ 연동        | specialChar             |
 * -----------------------------------------------------------------------------
 *
 * ★★★ 3L 연동 규칙 ★★★
 * -----------------------------------------------------------------------------
 * - 부품(컴포넌트): PFMEA L3 [4M + 부품(컴포넌트)명] ↔ CP workElement ↔ 양방향
 * - 설계파라미터: PFMEA L3.functions.processChars ↔ CP processChar ↔ 양방향
 ...
 * - 같은 행에 설계특성(J열) + 설계파라미터(K열) 동시 배치 절대 금지
 * - 설계특성 행과 설계파라미터 행은 별도 행으로 분리
 * - 표시 순서: 같은 공정 내에서 설계특성 행 → 설계파라미터 행
 ...
 *
 * Phase A: 설계특성 행 (L2.functions[].productChars) — 공정 수준, L3 없음
 * Phase B: 설계파라미터 행 (L3.functions[].processChars) — L3 부품(컴포넌트) 수준
 * Phase C: 구조 행 (L3에 설계파라미터 없는 경우) — 설비/구조만
...
export function syncPfmeaToCP(state: WorksheetState): { cpRows: CPRow[]; result: SyncResult } {
  ...
  (((state as unknown as Record<string, unknown>).cpRows as CPRow[]) || []).forEach((row: CPRow) => {
    // 설계특성 행: processNo + productChar
    if (row.productChar) {
      existingCpRows.set(`product|${row.processNo}|${row.productChar}`, row);
    }
    // 설계파라미터 행: processNo + workElement + processChar
    if (row.processChar) {
  ...
  ((state.l2 || []) as unknown as Record<string, unknown>[]).forEach((proc: Record<string, unknown>) => {
    ...
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    // Phase A: 설계특성 행 (L2.functions[].productChars)
    // ★ 분리배치: 설계특성만 담은 별도 행
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    ...
    for (const func of rawFunctions) {
      ...
      for (const pc of productChars) {
        ...
        cpRow.workElement = '';             // 설계특성은 L2 수준
        cpRow.productChar = pcName;
        cpRow.processChar = '';             // ★ 분리배치: 설계파라미터 없음
    ...
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    // Phase B: 설계파라미터 행 (L3 부품(컴포넌트)별)
    // ★ MN(사람) 제외, 분리배치: 설계파라미터만 담은 별도 행
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
      ...
      if (l3ProcessChars.length > 0) {
        for (const procChar of l3ProcessChars) {
          ...
          if (l3RiskRef) {
            ...
            // ★ PFMEA→CP: B5 설계검증 예방 → CP controlMethod, A6 설계검증 검출 → CP measureMethod
...
/**
 * 동기화 상태 확인
 * ★ 분리배치 적용: PFMEA 행 수 = 설계특성 수 + 설계파라미터 수 + 구조 수
 */
