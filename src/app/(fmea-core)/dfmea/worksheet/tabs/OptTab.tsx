...
const PLAN_COLS: { ko: string; en: string }[] = [
  { ko: '설계검증 예방개선', en: 'Prev. Control Improve' },
  { ko: '설계검증 검출개선', en: 'Det. Control Improve' },
  { ko: '책임자성명', en: 'Person' },
    ...
    <>
      ...
      <thead className="sticky top-0 z-20 bg-white border-b-2 border-black">
        ...
        <tr>
          ...
          <th colSpan={3} style={subH(COLORS.function.header)}><BiHeader ko="3. 부품(컴포넌트)기능 및 설계파라미터" en="Function & Process Char." /></th>
        ...
        <tr>
          ...
          <th style={colH(COLORS.structure.cell)}><BiHeader ko="부품(컴포넌트)" en="Work Element" /></th>
          {[{ko:'구분',en:'Type'},{ko:'완제품기능',en:'Product Func.'},{ko:'요구사항',en:'Requirements'},{ko:'설계기능',en:'Process Func.'},{ko:'설계특성',en:'Product Char.'},{ko:'부품(컴포넌트)',en:'Work Element'},{ko:'부품(컴포넌트)기능',en:'WE Function'},{ko:'설계파라미터',en:'Process Char.'}].map(c => <th key={c.ko} style={colH(COLORS.function.cell)}><BiHeader ko={c.ko} en={c.en} /></th>)}
          {[{ko:'구분',en:'Type'},{ko:'고장영향',en:'FE'},{ko:'심각도',en:'S'},{ko:'고장형태',en:'FM'},{ko:'부품(컴포넌트)',en:'WE'},{ko:'고장원인',en:'FC'}].map(c => <th key={c.ko} style={colH(COLORS.failure.cell)}><BiHeader ko={c.ko} en={c.en} /></th>)}
