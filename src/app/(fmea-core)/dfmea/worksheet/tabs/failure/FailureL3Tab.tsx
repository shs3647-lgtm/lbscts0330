 ...
 * -----------------------------------------------------------------------------
 * 1) 부품(컴포넌트)(WE)가 다르거나, 같은 기능 아래라도 B3(설계파라미터) id가 다르면 **동일 텍스트라도 별개 행**이다.
 ...
 * 3) missingCount / flatRows / totalCauseCount: 연결·건수는 **processCharId·FC id 기준**. 설계파라미터명·FC명만으로 그룹핑 금지.
...
// │ 2. charId 있음  → 해당 설계파라미터 + 연결된 FC 일괄 삭제            │
// │ 3. weId 있음    → 해당 부품(컴포넌트) + 연결된 FC 일괄 삭제            │
// │ 4. procId만     → 공정 전체 삭제                                 │
// │                                                                  │
...
export default function FailureL3Tab({ state, setState, setStateSynced, setDirty, saveToLocalStorage, saveAtomicDB, fmeaId, customerName, importCounts }: FailureTabProps) {
  ...
  const [modal, setModal] = useState<{
    ...
    processId: string;
    weId?: string;
    processCharId?: string;  // ✅ 설계파라미터 ID 추가 (CASCADE 연결)
  ...
  const [specialCharModal, setSpecialCharModal] = useState<{
    ...
    processCharIds: string[];  // 동일 이름 설계파라미터 전체 ID
  } | null>(null);
  ...
  // ★★★ 2026-02-05 FIX: missingCount — flatRows와 동일 isMeaningful + 설계파라미터 행별 processCharId ★★★
  // (useFailureL3Handlers 훅 호출 전에 선언 필요 → flatRows에서 직접 파생 불가, 동일 로직으로 일관성 보장)
  // ⚠️ AI주의: 설계파라미터 **이름**으로 행을 합치거나 FC를 묶지 말 것. 각 B3 행은 `pc.id`로만 FC(`processCharId`) 매칭.
  const missingCount = useMemo(() => {
    ...
    procs.forEach(proc => {
      ...
      workElements.forEach((we: any) => {
        ...
        // ★ FIX: 의미있는 함수가 있지만 의미있는 설계파라미터이 하나도 없으면 → 누락 1건
        ...
        functions.forEach((f: any) => {
          ...
          (f.processChars || []).forEach((pc: any) => {
            ...
            // ★ 동일 설계파라미터명 복수 행 — 행별 processCharId로만 FC 연결 판정 (이름 그룹 합치기 금지)
            const linked = pcId
  ...
  // ⚠️ AI주의: 여기서 설계파라미터 **이름→대표 id** 병합 또는 `(processCharId + FC명)` 중복 제거를 **재도입하지 말 것**.
  //    서로 다른 부품(컴포넌트)·다른 B3 id인데 문구만 같으면 정상 데이터다. 과거 그 로직이 데이터를 합쳐 버렸음(2026-03-23 제거).
  ...
  useEffect(() => {
    ...
    // 중복 고장원인 검사 및 정리
    // ★ 2026-03-23: 부품(컴포넌트)·B3가 다르면 동일 설계파라미터명·동일 FC명도 별개 — 이름으로 processCharId 병합·(pc+이름) dedup 금지
  ...
  // ★★★ 행 삭제 — 계층별 캐스케이드 (고장원인 → 설계파라미터 → 부품(컴포넌트) → 공정) ★★★
  const handleDeleteRowCtx = useCallback(() => {
    ...
    // ===== 2. 설계파라미터 삭제 (charId 있음, causeId 없음) =====
    if (charId) {
      ...
      const label = charName ? `설계파라미터 "${charName}"` : '이 설계파라미터';
    ...
    // ===== 3. 부품(컴포넌트) 삭제 (weId 있음, charId/causeId 없음) =====
    if (weId) {
      ...
      const label = weName ? `부품(컴포넌트) "${weName}"` : '이 부품(컴포넌트)';
      const msg = linkedCount > 0 ? `${label} 및 연결된 고장원인 ${linkedCount}건을 삭제하시겠습니까?` : `${label}을(를) 삭제하시겠습니까?`;
      if (!window.confirm(msg)) return;
   ...
   * 공정(proc) → 부품(컴포넌트)(we) → 기능(func) → 설계파라미터(char) → 고장원인(cause)
   * 설계파라미터 기준으로 행 분리, 각 고장원인에 processCharId 연결
   *
   * ⚠️ AI주의: 기능(function) 안에서 설계파라미터 **이름**이 같아도 **행을 스킵하지 말 것** — 행마다 `c.id`가 다를 수 있음.
  ...
  const flatRows = useMemo(() => {
    ...
    // ✅ 2026-01-19: 공정 레벨 중복 추적 제거 (부품(컴포넌트)별로 설계파라미터 표시)
    ...
    processes.forEach(proc => {
      ...
      workElements.forEach((we: any, weIdx: number) => {
        ...
        functions.forEach((f: any) => {
          ...
          // ★ 2026-03-23: 동일 기능 내 동일 설계파라미터명 복수 행 허용 — 행마다 별도 id, FC는 processCharId로만 연결
  ...
  // FC 건수 — 부품(컴포넌트)가 달라도 동일 FC명 허용: FC id(없으면 proc+pcId+name)로 집계
    ...
    <div className="p-0 overflow-auto h-full" style={{ paddingBottom: '50px' }} onKeyDown={handleEnterBlur}>
            ...
            (() => {
              ...
              return (
                <tr>
                  ...
                  <td className="border border-[#ccc] p-2.5 text-center font-semibold align-middle" style={{ background: zebra.structure }}>
                    {!isUpstreamConfirmed ? '하위 단계는 상위 단계 확정 후 활성화됩니다.' : '(부품(컴포넌트) 입력)'}
                  </td>
            ...
            return flatRows.map((row, idx) => {
              ...
              return (
                <tr key={`row-${idx}`} data-cause-id={row.cause?.id} data-missing-cause={row.processChar && (!row.cause || isMissing(row.cause?.name)) ? "true" : undefined} onContextMenu={(e) => handleContextMenuEvt(e, 'failureCause', row.proc?.id, row.we?.id, row.processChar?.id, row.cause?.id)}>
                  ...
                  {/* 부품(컴포넌트) 셀: showWe && weRowSpan > 0 (파란색) ★ 읽기전용: 컨텍스트 메뉴 차단 */}
                  ...
                  {/* 설계파라미터 셀: showChar && charRowSpan > 0 (녹색) ★ 읽기전용: 컨텍스트 메뉴 차단 */}
      ...
      {modal && (
        <DataSelectModal
          ...
          workElementName={modal.processCharName || ''}  // ✅ 설계파라미터명 표시
