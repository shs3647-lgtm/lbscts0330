...
export default function FailureL2Tab({ state, setState, setStateSynced, setDirty, saveToLocalStorage, saveAtomicDB, fmeaId, customerName, importCounts }: FailureTabProps) {
  ...
  const loadFromMaster = useCallback(async () => {
    ...
    try {
      ...
      const masterUrl = fmeaId
        ? `/api/dfmea/master?fmeaId=${encodeURIComponent(fmeaId)}&includeItems=true`
        : '/api/dfmea/master?includeItems=true';
  ...
  const handleDeleteRowCtx = useCallback(() => {
    const { procId, modeId } = menuExtra;
    if (!procId || !modeId) {
      showAlert('삭제할 고장형태가 없습니다.');
      return;
    }
    ...
    setTimeout(() => { saveToLocalStorage?.(); saveAtomicDB?.(true); }, 100);
    closeContextMenu();
  }, [menuExtra, state.l2, setState, setStateSynced, setDirty, saveToLocalStorage, saveAtomicDB, closeContextMenu]);
            ...
            return buildFlatRows.map((row, idx) => {
              ...
              return (
                <tr key={`row-${idx}`} data-mode-id={row.modeId} onContextMenu={(e) => handleContextMenu(e, 'failureMode', row.procId, row.charId, row.modeId)}>
                  {/* 공정명 - rowSpan (파란색) */}
                  {row.showProc && (
                    <td rowSpan={row.procRowSpan} className="border border-[#ccc] px-0.5 py-0.5 text-center font-semibold text-[10px] align-middle" style={{ background: getZebra('structure', procStripeIdx) }}>
                  ...
                  {/* 제품특성 - rowSpan (녹색 줄무늬) ★ 기능분석 데이터 */}
                  {row.showChar && (
                    <td rowSpan={row.charRowSpan} className="border border-[#ccc] border-r-[2px] border-r-green-500 p-2 text-center text-xs align-middle" style={{ background: getZebra('function', charStripeIdx) }}>
                      {row.charName || ''}
                    </td>
                  )}
