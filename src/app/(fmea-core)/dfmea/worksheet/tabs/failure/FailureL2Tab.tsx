...
// │ 설계특성(Char):  2L기능에서 연동, 여기서 추가/삭제 불가            │
// │ 고장형태(FM):    설계특성별로 자유롭게 추가/삭제                   │
// │                                                                  │
...
export default function FailureL2Tab({ state, setState, setStateSynced, setDirty, saveToLocalStorage, saveAtomicDB, fmeaId, customerName, importCounts }: FailureTabProps) {
  ...
  const handleDeleteRowCtx = useCallback(() => {
    ...
    // ★★★ 2026-03-07 FIX: modeId 빈값 → 설계특성 행 삭제 폴백 (Function탭과 동일 패턴) ★★★
    ...
    if (!modeId) {
      ...
      if (hasContent) {
        if (!window.confirm(`이 설계특성의 고장형태 ${modes.length}건을 삭제하시겠습니까?`)) return;
      }
      const updateFn = (prev: WorksheetState) => {
        const newState = JSON.parse(JSON.stringify(prev));
  ...
  const buildFlatRows = useMemo(() => {
    ...
    processes.forEach(proc => {
      ...
      // ★★★ 2026-02-05 FIX: L3Tab 패턴 — 공정 내 동일 이름 설계특성 ID 그룹화 ★★★
      ...
      if (functions.length === 0) {
        rows.push({
          ...
          charId: '', charName: '(설계특성 미입력)', specialChar: '', charRowSpan: 1, showChar: true,
          modeId: '', modeName: ''
        });
        return;
      } else {
  ...
  const handleAutoFillMissing = useCallback(() => {
    ...
    const msg = `누락된 고장형태 ${missingRows.length}건을 자동 생성합니다.\n\n` +
      `설계특성명 기반으로 "[설계특성]_부적합" 패턴으로 생성됩니다.\n계속하시겠습니까?`;
      ...
      <table className="w-full border-collapse table-fixed" style={{ marginBottom: '50px' }}>
        ...
        <tbody>
          {buildFlatRows.length === 0 ? (
              ...
              return (
                <tr>
                  ...
                  <td className="border border-[#ccc] p-2.5 text-center align-middle" style={{ background: zebra.function }}>
                    {!isUpstreamConfirmed ? '하위 단계는 상위 단계 확정 후 활성화됩니다.' : '(기능분석에서 설계기능 입력)'}
                  </td>
                  <td className="border border-[#ccc] border-r-[2px] border-r-green-500 p-2.5 text-center align-middle" style={{ background: zebra.function }}>
                    {!isUpstreamConfirmed ? '-' : '(기능분석에서 설계특성 입력)'}
                  </td>
            ...
            const charIdxMap = new Map<string, number>();  // ★ 설계특성 인덱스 맵 추가
            let procIdx = 0;
            let funcIdx = 0;
            let charIdx = 0;  // ★ 설계특성 인덱스 카운터
            ...
            return buildFlatRows.map((row, idx) => {
              ...
              const charStripeIdx = charIdxMap.get(`${row.procId}:${row.charId || ''}`) ?? 0;  // ★ 설계특성 줄무늬 인덱스
              ...
              return (
                <tr key={`row-${idx}`} data-mode-id={row.modeId} onContextMenu={(e) => handleContextMenu(e, 'failureMode', row.procId, row.charId, row.modeId)}>
                  ...
                  {/* 설계특성 - rowSpan (녹색 줄무늬) ★ 읽기전용: 컨텍스트 메뉴 차단 */}
                  {row.showChar && (
                    ...
                    <SelectableCell
                      ...
                      onClick={() => {
                        if (!row.charId || !row.charName) {
                          showAlert('상위 항목(설계특성)이 없습니다.\n\n고장형태를 추가하려면 먼저 기능분석에서 설계특성을 입력해주세요.\n\n[기능분석 2L(서브시스템) → 설계특성 입력]');
                          return;
                        }
                        handleCellClick({
                          type: 'l2FailureMode',
