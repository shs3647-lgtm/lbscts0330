...
export default function FailureLinkResult({ savedLinks, fmData, isFullscreen, onToggleFullscreen, onExcelExport }: FailureLinkResultProps) {
      ...
      <table className="w-full border-collapse" style={{ fontSize: fs }}>
        <thead className="sticky top-0 z-10">
          <tr>
            ...
            <th className={`w-[12%] bg-[#1a3520] text-white ${thPad} border border-[#2a5530] font-semibold sticky top-0`} title="Work Element: 부품(컴포넌트)">WE</th>
  ...
  return (
    <div style={resultTableContainer}>
      {(onToggleFullscreen || onExcelExport) && (
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 4, padding: '2px 4px', flexShrink: 0 }}>
          {onExcelExport && (
            <button
