...
// DFMEA Interface Type 옵션 (PC/ME/ET/DE/HMI)
export const M4_OPTIONS = [
  { value: 'PC', label: 'PC', color: '#e3f2fd' },   // Physical Connection
  { value: 'ME', label: 'ME', color: '#e8f5e9' },   // Material Exchange
  { value: 'ET', label: 'ET', color: '#fff3e0' },   // Energy Transfer
  { value: 'DE', label: 'DE', color: '#fce4ec' },   // Data Exchange
  { value: 'HMI', label: 'HMI', color: '#fff0f0' }, // Human-Machine Interface
];
...
export function EditableL3Cell({
  ...
  return (
    <td
      ...
      onClick={handleClick}
      onDoubleClick={handleDoubleClick}
      title="클릭: 부품(컴포넌트) 선택 모달 | 더블클릭: 직접 입력"
    >
      {isPlaceholder || !value || value.trim() === '' ? (
        <span className="text-[#e65100] font-semibold italic text-[11px]">🔍 클릭하여 부품(컴포넌트) 추가</span>
      ) : (
...
export function StructureColgroup() {
  // 완제품 공정(22%) / 서브시스템(22%) / 4M(55px) / 부품(컴포넌트)(나머지)
  return (
    <colgroup>
      <col className="w-[22%]" />
      <col className="w-[22%]" />
