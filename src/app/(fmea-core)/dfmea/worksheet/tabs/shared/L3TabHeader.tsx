    ...
    <thead className="sticky top-0 z-20 bg-white border-b-2 border-black">
            ...
            <div className="flex gap-1 items-center shrink-0 flex-wrap">
              {onToggleMode && (
                <button type="button" onClick={() => onToggleMode()}
                  ...
                  {isLoadingMaster ? '로딩...' : isAutoMode ? '자동' : '수동'}
                </button>
              )}
              {!isUpstreamConfirmed && tabType === 'failure' ? (
                <span className="bg-yellow-500 text-white px-2 py-0 rounded text-[10px] whitespace-nowrap">미확정</span>
              ) : isConfirmed ? (
                <span className={badgeConfirmed}>확정됨({
                ...
                <button type="button" onClick={onConfirm} className={btnConfirm}>미확정</button>
              )}
              {missingCount > 0 && (
                <button type="button" onClick={onMissingClick} className={`${badgeMissing} cursor-pointer hover:opacity-80`}>누락 {missingCount}건</button>
              )}
      ...
      <tr>
        ...
        <th colSpan={showSecondary ? 3 : 2} className="text-white border border-[#ccc] p-1 text-[11px] font-bold text-center" style={{ background: stepBgColor }}>
          {isFunction ? '3. 작업요소 기능/공정특성/특별특성' : '3. 공정특성/고장원인/발생도'}
          {missingCount > 0 && (
      ...
      <tr style={{ background: stepBgColorLight }}>
        ...
        {showSecondary && (
          <>
            ...
            <th className="bg-green-600 text-white border border-[#ccc] border-l-0 p-1 text-[11px] font-bold text-center whitespace-nowrap" style={{ boxShadow: 'inset 0 -2px 0 #2196f3' }}>
              특별특성
            </th>
          </>
        )}
