  ...
  return (
    <div className="flex flex-col h-full" style={{ width: RIGHT_PANEL_WIDTH }}>
      ...
      <div className="bg-[#1e3a5f] text-white py-2 px-3 text-xs font-bold flex justify-between items-center">
        <span>{t('5AP 리스크분석(Risk Analysis)')}</span>
      ...
      {selectedCell && selectedItems.length > 0 && (
        <div className="bg-orange-50 border-b border-orange-200 p-1">
          <div className="flex justify-between items-center mb-1">
            <span className="text-[10px] font-bold text-orange-700">{t('H/M→L 개선 방향')} ({selectedItems.length}{t('건')})</span>
          ...
          <div className="max-h-[200px] overflow-auto scrollbar-hide">
            {selectedItems.map(item => {
              const paths = getAllPaths(item.s, item.o, item.d);
              const dirs: { key: ImproveDirection; label: string; path: ImprovePath; desc: string }[] = [
                { key: 'O_ONLY', label: t('① O개선'), path: paths.oOnly,
                  desc: `O:${item.o}→${paths.oOnly.targetO}` },
                { key: 'D_ONLY', label: t('② D개선'), path: paths.dOnly,
                  desc: `D:${item.d}→${paths.dOnly.targetD}` },
                { key: 'BOTH', label: t('③ O+D'), path: paths.both,
                    ...
                    {dirs.filter(dir => dir.path.feasible && paths.recommended === dir.key).map(dir => {
                      ...
                      return (
                        <div key={dir.key} className="flex items-center gap-1">
                          ...
                          >
                            {done ? t('✓ 완료') : `${dir.desc} (Δ${dir.path.totalChange}) ${t('★추천')}`}
                          </button>
                        </div>
                      );
                    })}
                  ...
                  {row.d.map((ap, dIdx) => {
                    ...
                    return (
                      <td
                        ...
                        title={`${ap}: ${count}건${(ap === 'H' || ap === 'M') && count > 0 ? ' - 클릭하여 개선 제안 보기' : ''}${isTarget ? ` | 개선 목표 ${targetCells[cellKey]}건` : ''}`}
                        onClick={() => {
                          if (count > 0 && (ap === 'H' || ap === 'M')) {
          ...
          <div className="p-3 overflow-y-auto text-xs">
            ...
            {(direction === 'O_ONLY' || direction === 'BOTH') && (
              <>
                ...
                <label className="text-[10px] font-semibold text-green-700 mb-0.5 block">{t('예방관리 개선안')}</label>
            ...
            <div className="flex justify-end gap-2 mt-2">
              <button onClick={() => setInputModal(null)} className="px-3 py-1 text-xs text-gray-600 hover:bg-gray-100 rounded">취소</button>
              <button onClick={handleSaveImprovement} disabled={!canSave}
                className={`px-3 py-1 text-xs text-white rounded ${
                  ...
                  : 'bg-green-500 hover:bg-green-600'} disabled:opacity-50`}>저장</button>
            </div>
          </div>
