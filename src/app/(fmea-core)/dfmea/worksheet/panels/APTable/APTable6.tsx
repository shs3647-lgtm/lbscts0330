  ...
  return (
    <div className="flex flex-col h-full" style={{ width: RIGHT_PANEL_WIDTH }}>
      ...
      <div className="bg-[#2e7d32] text-white py-2 px-3 text-xs font-bold flex justify-between items-center">
        <span>6AP 최적화(Optimization)</span>
        <span className="text-[10px] font-normal" style={{ marginRight: 24 }}>
      ...
      {/* 선택된 H 셀의 개선 제안 - 컴팩트 테이블 */}
      {selectedCell && selectedItems.length > 0 && (
        <div className="bg-green-50 border-b border-green-200 p-1">
          <div className="flex justify-between items-center mb-1">
            <span className="text-[10px] font-bold text-green-700">🔧 H→L 최적화 제안 ({selectedItems.length}건)</span>
          ...
          <table className="w-full border-collapse text-[9px] table-fixed">
            <thead className="bg-green-100">
              <tr>
                <th className="border border-green-200 p-0.5" style={{ width: '35%' }}>항목</th>
                <th className="border border-green-200 p-0.5" style={{ width: '12%' }}>S:O:D</th>
                <th className="border border-green-200 p-0.5" style={{ width: '26.5%' }}>예방관리</th>
                <th className="border border-green-200 p-0.5" style={{ width: '26.5%' }}>검출관리</th>
              </tr>
            </thead>
          </table>
          {/* 테이블 본문 (스크롤) */}
            ...
            <table className="w-full border-collapse text-[9px] table-fixed">
              <tbody>
                {selectedItems.map(item => {
                  ...
                  const oNeeded = path.type === 'O_ONLY' || path.type === 'BOTH';
                  const dNeeded = path.type === 'D_ONLY' || path.type === 'BOTH';
                  return (
                    <tr key={item.uniqueKey} className="bg-white hover:bg-green-50">
                      <td className="border border-green-200 p-0.5 text-[9px] truncate" style={{ width: '35%' }} title={item.failureMode || `#${item.idx + 1}`}>{item.failureMode || `#${item.idx + 1}`}</td>
                      <td className="border border-green-200 p-0.5 text-center text-gray-500" style={{ width: '12%' }}>{item.s}:{item.o}:{item.d}</td>
                      <td className="border border-green-200 p-0.5" style={{ width: '26.5%' }}>
                        {oNeeded ? (
                          <button
