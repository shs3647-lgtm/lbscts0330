 ...
 * @description 부품(컴포넌트) 선택 모달 - 상태/로직 hook
...
export const M4_OPTIONS = [
  { id: 'PC', label: 'PC', bg: '#e3f2fd', text: '#1565c0' },   // Physical Connection
  { id: 'ME', label: 'ME', bg: '#e8f5e9', text: '#2e7d32' },   // Material Exchange
  { id: 'ET', label: 'ET', bg: '#fff3e0', text: '#e65100' },   // Energy Transfer
  { id: 'DE', label: 'DE', bg: '#fce4ec', text: '#c2185b' },   // Data Exchange
  { id: 'HMI', label: 'HMI', bg: '#fff0f0', text: '#881a1a' }, // Human-Machine Interface
];
...
// ★★★ 2026-02-03: DB 기반 부품(컴포넌트) 로드 API 호출 ★★★
...
export function useWorkElementSelect({
        ...
        if (editValue.trim() !== rawOldName) {
          ...
          try {
            ...
            if (data.success) {
              ...
              console.error('부품(컴포넌트)명 저장 실패:', data.error);
            }
          } catch (e) {
            console.error('부품(컴포넌트)명 DB 저장 오류:', e);
          }
  ...
  const handleDelete = useCallback(() => {
    ...
    if (!window.confirm(`선택된 ${selectedElements.length}개 부품(컴포넌트)를 워크시트에서 제거하시겠습니까?\n\n❌ 제거: ${deleteNames}\n※ 마스터 기초정보는 유지됩니다.`)) return;
