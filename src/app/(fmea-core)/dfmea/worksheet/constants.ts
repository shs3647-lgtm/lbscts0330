/**
 * @file constants.ts
 * @description DFMEA 워크시트 상수 정의 (PFMEA 클론 + 설계 FMEA 용어 치환)
 * @version 0.1.0
 * @created 2026-03-25
 */
...
export interface L2FailureMode extends AtomicUnit {
  sc?: boolean; // 특별특성
  productCharId?: string; // [원자성] 상위 설계특성 FK - 고장형태는 특정 설계특성에 연결됨
}
...
export interface L3FailureCauseExtended extends L3FailureCause {
  processCharId?: string; // [원자성] 상위 설계파라미터 FK
}
...
export interface WorksheetState {
  ...
  // 리스크 분석 데이터 (SOD 점수 + 예방/설계검증 검출 저장)
...
export interface FlatRow {
  ...
  l3IsRevised?: boolean; // 엑셀 적색 표기 (부품(컴포넌트)명)
}
...
// ============ DFMEA 인터페이스 타입 정렬 순서 (PC→ME→ET→DE→HMI) ============
/** DFMEA 렌더링/정렬 순서: PC(물리적연결) → ME(재료교환) → ET(에너지전달) → DE(데이터교환) → HMI(휴먼머신) */
export const M4_SORT_ORDER: Record<string, number> = { PC: 0, ME: 1, ET: 2, DE: 3, HMI: 4 };

/** 인터페이스 타입 값에 대한 정렬 우선순위 반환 (알 수 없는 값은 99) */
export function m4SortValue(m4: string | undefined): number {
  return M4_SORT_ORDER[(m4 || '').toUpperCase()] ?? 99;
}

/** Component 배열을 인터페이스 타입 순서(PC→ME→ET→DE→HMI)로 정렬 (원본 불변, 새 배열 반환) */
...
export const COLORS = {
  ...
  // DFMEA 인터페이스 타입 색상
  pc:  { bg: '#eef7ff', border: '#cfe0f4', color: '#1f4f86' },  // Physical Connection (파랑)
  me:  { bg: '#f0fff2', border: '#bdeac5', color: '#1b6b2a' },  // Material Exchange (초록)
  et:  { bg: '#fff3e6', border: '#ffd2a6', color: '#8a4f00' },  // Energy Transfer (주황)
  de:  { bg: '#fef0ff', border: '#f0bdf5', color: '#7a1a88' },  // Data Exchange (보라)
  hmi: { bg: '#fff0f0', border: '#f5bdbd', color: '#881a1a' },  // Human-Machine (빨강)
  // PFMEA 4M 호환 (레거시 데이터용)
 ...
 *   
 *   L3S: M001-L3S-P01W1-001            공정1.부품(컴포넌트)1
 *   L3F: M001-L3F-P01W1F1C1-001        공정1.부품(컴포넌트)1.기능1.특성1
 *   FC:  M001-FC-P01W1C1-001           공정1.부품(컴포넌트)1.특성1의 고장원인
 *   
...
export const createInitialState = (): WorksheetState => ({
  ...
  l2: [{
    id: uid(), no: '', name: '(클릭하여 서브시스템 선택)', order: 10,
    functions: [], productChars: [], failureModes: [],
    l3: [{ id: uid(), m4: '', name: '(서브시스템 선택 후 부품 추가)', order: 10, functions: [], processChars: [], failureCauses: [] }]
  }],
...
export const get4MBadgeStyle = (m4: string) => {
  const colorMap: any = {
    // DFMEA 인터페이스 타입
    PC: COLORS.pc, ME: COLORS.me, ET: COLORS.et, DE: COLORS.de, HMI: COLORS.hmi,
    // PFMEA 4M 호환
