  ...
  return typeof document === 'undefined' ? null : createPortal(
    <div className="fixed z-[9999] bg-white rounded-lg shadow-2xl flex flex-col select-none border border-gray-300"
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-[#00587a] text-white rounded-t-lg cursor-move" onMouseDown={onDragStart}>
        <h3 className="text-sm font-bold">FMEA명 목록(FMEA Name List) (중복 방지)</h3>
        <button onClick={handleClose} className="text-white hover:text-gray-200">✕</button>
      </div>
      {/* 새 FMEA명 추가 영역 */}
      <div className="px-4 py-2 bg-green-50 border-b border-green-200">
        <div className="flex items-center gap-2">
          <span className="text-xs font-semibold text-green-700">새 FMEA명:</span>
          ...
          }} className="px-3 py-1.5 bg-green-500 text-white text-xs font-semibold rounded hover:bg-green-600">✓ 적용</button>
        </div>
      ...
      <div className="px-4 py-1 bg-gray-100 border-b">
        <span className="text-[10px] text-gray-600">기존 FMEA 목록 (중복 확인용)</span>
      </div>
          ...
          <table className="w-full text-xs border-collapse">
            <thead className="sticky top-0 bg-gray-100">
              <tr>
                ...
                <th className="border px-2 py-1 text-center w-12">유형</th>
                <th className="border px-2 py-1 text-left">FMEA명</th>
                <th className="border px-2 py-1 text-center w-16">선택</th>
              </tr>
            </thead>
            <tbody>
              {filteredList.map((f, idx) => (
                <tr key={f.id} className={`hover:bg-blue-50 ${f.id === currentFmeaId ? 'bg-yellow-100' : idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                  ...
                  <td className="border px-2 py-1 text-center">
                    {f.id === currentFmeaId ? (
                      <span className="text-yellow-600 font-semibold">현재</span>
                    ) : (
                      <button onClick={() => { onLoadFmea(f.id); handleClose(); }}
                        className="px-2 py-0.5 bg-blue-500 text-white rounded text-[10px] hover:bg-blue-600">불러오기</button>
                    )}
                  </td>
                </tr>
              ))}
      ...
      <div className="px-4 py-2 border-t bg-gray-50 flex justify-between items-center">
        ...
        <button onClick={handleClose} className="px-3 py-1 text-xs bg-gray-200 rounded hover:bg-gray-300">닫기</button>
      </div>
