 ...
 * @description DFMEA 등록 페이지 - 상태 선언 + useEffect (데이터 로드, 접속 로그, 라이프사이클)
...
export function useRegisterPageCore() {
    ...
    const logTimeout = setTimeout(() => {
      fetch('/api/auth/access-log', {
        ...
        body: JSON.stringify({
          ...
          module: 'DFMEA', action: '조회', itemType: '등록화면',
          description: `${user.name}님이 DFMEA 등록화면에 접속 (${targetId})`,
        }),
  ...
  useEffect(() => {
    ...
    const updateLogout = () => {
      const blob = new Blob([JSON.stringify({
        userId: user.id, projectId: targetId, module: 'DFMEA', _method: 'PUT',
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      const loadProjectData = async (project: any) => {
        ...
        const projectType = (project.fmeaType || 'D') as FMEAType;
        ...
        localStorage.setItem('dfmea-last-viewed', projectId);
      ...
      if (!isEditMode && !editId) {
        ...
        if (urlParams.get('mode') === 'new') {
          localStorage.removeItem('dfmea-last-viewed');
        ...
        const lastViewedId = localStorage.getItem('dfmea-last-viewed');
        ...
        try {
          ...
          if (data.success && data.projects?.length > 0) {
            ...
            if (targetProject) {
              ...
              router.replace(`/dfmea/register?id=${targetId}`);
              return;
            }
          }
        } catch (e) {
      ...
      if (!project) {
        localStorage.removeItem('dfmea-last-edited');
        localStorage.removeItem('dfmea-last-viewed');
        const localProjects = localStorage.getItem('dfmea-projects');
        if (localProjects) {
          try {
            const projects = JSON.parse(localProjects);
            const filtered = projects.filter((p: any) => p.id?.toLowerCase() !== targetId.toLowerCase());
            localStorage.setItem('dfmea-projects', JSON.stringify(filtered));
      ...
      if (project) {
        ...
        if (!isEditMode) router.replace(`/dfmea/register?id=${project.id.toLowerCase()}`);
      }
    };
    loadData();
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      localStorage.setItem('dfmea-temp-data', JSON.stringify(tempData));
    }, 1000);
    return () => clearTimeout(timer);
  ...
  useEffect(() => {
    if (!isEditMode && !editId) {
      localStorage.removeItem('dfmea-temp-data');
    }
  }, [isEditMode, editId]);
  ...
  useEffect(() => {
    if (selectedBaseFmea?.includes('-m')) {
      fetch('/api/dfmea/master?includeItems=false')
        .then(res => res.json())
