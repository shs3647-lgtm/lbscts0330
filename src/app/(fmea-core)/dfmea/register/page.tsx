/**
 * @file page.tsx
 * @description DFMEA 등록 페이지 (리팩토링 버전)
 * @version 11.0.0
...
import DfmeaBasicInfoTable from './components/DfmeaBasicInfoTable';
...
import { useTemplateGenerator } from '@/app/(fmea-core)/dfmea/import/hooks/useTemplateGenerator';
import type { ImportedFlatData } from '@/app/(fmea-core)/dfmea/import/types';
import type { BdStatusItem, FMEAProject as ImportFMEAProject } from '@/app/(fmea-core)/dfmea/import/components/ImportPageTypes';

// ★ Heavy import modules → dynamic/lazy loading (컴파일 성능 최적화)
const TemplateGeneratorPanel = dynamic(
  () => import('@/app/(fmea-core)/dfmea/import/components').then(mod => ({ default: mod.TemplateGeneratorPanel })),
  { ssr: false, loading: () => <div className="p-4 text-xs text-gray-400">기초정보 템플릿 로딩 중...</div> }
);
const BdStatusTable = dynamic(
  () => import('@/app/(fmea-core)/dfmea/import/components/BdStatusTable').then(mod => ({ default: mod.BdStatusTable })),
...
const getMasterApi = () => import('@/app/(fmea-core)/dfmea/import/utils/master-api');
const getExcelTemplate = () => import('@/app/(fmea-core)/dfmea/import/excel-template');
const getMasterDataHandlers = () => import('@/app/(fmea-core)/dfmea/import/hooks/useMasterDataHandlers');
...
function DFMEARegisterPageContent() {
  ...
  const templateGen = useTemplateGenerator({ setFlatData, setPreviewColumn: setBdPreviewCol, setDirty: setBdDirty, setIsSaved: setBdIsSaved, existingFlatData: flatData });
  ...
  const handleBdDeleteDatasets = async (fmeaIds: string[]) => {
    try {
      const res = await fetch('/api/dfmea/master', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleBdFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setBdIsSaving(true);
    try {
      const { parseMultiSheetExcel } = await import('@/app/(fmea-core)/dfmea/import/excel-parser');
  ...
  return (
    <FixedLayout topNav={<DFMEATopNav selectedFmeaId={fmeaId} />} topNavHeight={48} showSidebar={true} contentPadding="pl-[5px] pr-2 py-2">
      <div className="font-[Malgun_Gothic]">
        {/* 헤더 */}
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <span className="text-lg">{isEditMode ? '✏️' : '📝'}</span>
            <h1 className="text-sm font-bold text-gray-800">DFMEA {isEditMode ? '수정(Edit)' : '등록(Register)'}</h1>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => {
              ...
              else if (fmeaId) { router.push(`/dfmea/register?id=${fmeaId}`); }
        ...
        <div className="bg-white rounded border border-gray-300 mb-3">
          <div className="bg-[#e3f2fd] px-3 py-1.5 border-b border-gray-300 flex items-center justify-between">
            <h2 className="text-xs font-bold text-gray-700">기획 및 준비(Planning and Preparation, 1단계)</h2>
          ...
          <form autoComplete="off" onSubmit={e => e.preventDefault()}>
            <table className="w-full border-collapse text-xs table-fixed">
              ...
              <tbody>
                ...
                <tr className="h-8">
                  <td className={headerCell}>FMEA 유형(Type)</td>
                  ...
                  <td className={headerCell}>FMEA명(Name)</td>
                  ...
                  <td className={`${headerCell} opacity-40`}>상위 APQP(Parent)</td>
                ...
                <tr className="h-8">
                  <td className={headerCell}>공정 책임(Design Resp.)</td>
                  <td className={inputCell}>
                  ...
                  <td className={headerCell}>FMEA 담당자(Responsible)</td>
                  <td className={inputCell}>
                    <div className="flex items-center gap-0.5 min-w-0">
                  ...
                  <td className={headerCell}>시작 일자(Start Date)</td>
                  ...
                  <td className={`${headerCell} bg-yellow-600`}>상위 FMEA(Parent)</td>
                  <td className={inputCell}>
                ...
                <tr className="h-8">
                  <td className={headerCell}>고객 명(Customer)</td>
                  <td className={inputCell}>
                    <div className="flex items-center gap-0.5 min-w-0">
                  ...
                  <td className={headerCell}>엔지니어링 위치(Eng. Location)</td>
                  ...
                  <td className={headerCell}>목표완료일(Target Date)</td>
                  ...
                  <td className={`${headerCell} bg-teal-600`}>연동 CP(Linked CP){linkedCpList.length > 0 && ` (${linkedCpList.length})`}</td>
                  <td className={inputCell}>
                    <div className="flex items-center gap-1 px-1 min-w-0 flex-wrap">
                      {linkedCpList.length > 0 ? linkedCpList.map((cp) => (
                        <span key={cp.id} className="inline-flex items-center gap-0.5 cursor-pointer" onClick={() => router.push(`/control-plan/list`)} title="CP 리스트로 이동">
                          <span className="px-1 py-0.5 rounded text-[8px] font-bold text-white bg-teal-500 hover:bg-teal-600 shrink-0">CP</span>
                          <span className="text-[10px] font-semibold text-teal-600 hover:underline break-all">{cp.id}</span>
                        </span>
                ...
                <tr className="h-8">
                  <td className={headerCell}>회사 명(Company)</td>
                  <td className={inputCell}>
                  ...
                  <td className={headerCell}>모델 연식(Model Year)</td>
                  ...
                  <td className={headerCell}>품명(Part Name)</td>
                  <td className={inputCell}>
                  ...
                  <td className={`${headerCell} bg-violet-600`}>연동 PFD(Linked PFD){linkedPfdList.length > 0 && ` (${linkedPfdList.length})`}</td>
                  <td className={inputCell}>
                    <div className="flex items-center gap-1 px-1 min-w-0 flex-wrap">
                      {linkedPfdList.length > 0 ? linkedPfdList.map((pfd) => (
                        <span key={pfd.id} className="inline-flex items-center gap-0.5 cursor-pointer" onClick={() => router.push(`/pfd/list`)} title="PFD 리스트로 이동">
                          <span className="px-1 py-0.5 rounded text-[8px] font-bold text-white bg-violet-500 hover:bg-violet-600 shrink-0">PFD</span>
                          <span className="text-[10px] font-semibold text-violet-600 hover:underline break-all">{pfd.id}</span>
                        </span>
                      )) : <span className="text-[10px] text-gray-400">-</span>}
                    </div>
                  </td>
                ...
                <tr className="h-8">
                  <td className={headerCell}>품번(Part No.)</td>
                  ...
                  <td className={headerCell}>상호기능팀(CFT)</td>
                  ...
                  <td className={headerCell}>기밀수준(Confidentiality)</td>
                  <td className={inputCell}>
                  ...
                  <td className={`${headerCell} bg-cyan-600`}>연동 DFMEA(Linked)</td>
                  <td className={inputCell}>
            ...
            <tbody>
              <tr className="h-8">
                <td className="w-[15%] bg-[#00587a] text-white px-1 py-1.5 border border-gray-400 font-bold text-center whitespace-nowrap">
                  <span className="text-[11px]">FMEA 기초 정보등록(BD Registration)</span>
        ...
        <div className="mt-4">
          <TemplateGeneratorPanel
            ...
            fmeaInfo={{
              ...
              fmeaType: core.fmeaInfo.fmeaType || 'D',
            ...
            onWorksheetSaved={() => {
              if (fmeaId) window.location.href = `/dfmea/worksheet?id=${fmeaId}&tab=structure`;
            }}
        ...
        <div id="cft-section" className="mt-6 scroll-mt-20">
          <CFTRegistrationTable title="CFT 리스트(CFT List)" members={cftMembers}
            ...
            onNavigateWorksheet={fmeaId ? () => router.push(`/dfmea/worksheet?id=${fmeaId}`) : undefined}
        ...
        <div className="flex items-center gap-2 mt-6 mb-2">
          <span>📊</span><h2 className="text-sm font-bold text-gray-700">CFT 접속 로그(CFT Access Log)</h2>
        </div>
        ...
        <div className="mt-3 px-4 py-2 bg-white rounded border border-gray-300 flex justify-between text-xs text-gray-500">
          ...
          <span>버전: D-FMEA Suite v3.0 | 사용자: {user?.name || 'FMEA Lead'}</span>
        </div>
        ...
        <FmeaSelectModal isOpen={fmeaSelectModalOpen} onClose={() => setFmeaSelectModalOpen(false)} onSelect={handleFmeaSelect} fmeas={availableFmeas} selectType={fmeaSelectType} currentFmeaId={fmeaId} onExcelImport={() => window.location.href = `/dfmea/import?id=${fmeaId}&mode=excel&type=${fmeaSelectType}`} />
                ...
                <table className="w-full border-collapse text-[11px]">
                  <thead>
                    <tr className="bg-gray-100 sticky top-0">
                      <th className="border border-gray-300 px-2 py-1 text-left">FMEA명</th>
                      <th className="border border-gray-300 px-2 py-1 text-center w-16">공정</th>
                      ...
                      <th className="border border-gray-300 px-2 py-1 text-center w-16">데이터</th>
                      <th className="border border-gray-300 px-2 py-1 text-center w-14">판정</th>
                      ...
                      <tr key={bd.fmeaId} className="hover:bg-blue-50 cursor-pointer" onClick={() => {
                        ...
                        <td className="border border-gray-200 px-2 py-1 text-center">
                          <span className={`text-[10px] font-bold px-1 py-0.5 rounded ${
                            ...
                            {(bd.processCount ?? 0) > 0 && (bd.fmCount ?? 0) > 0 && (bd.fcCount ?? 0) > 0 ? '적합' : '누락'}
                          </span>
                        </td>
                        <td className="border border-gray-200 px-2 py-1 text-center">
        ...
        <FmeaNameModal isOpen={fmeaNameModalOpen} onClose={() => setFmeaNameModalOpen(false)}
          ...
          onLoadFmea={(id) => { router.push(`/dfmea/register?id=${id}`); window.location.reload(); }} />
