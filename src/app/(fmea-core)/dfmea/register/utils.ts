/**
 * @file utils.ts
 * @description DFMEA 등록 페이지 유틸리티 함수
...
export async function generateFMEAIdFromDB(fmeaType: FMEAType = 'P', linkGroupNo: number = 0): Promise<string> {
  try {
    const res = await fetch(`/api/fmea/next-id?type=${fmeaType}&module=dfmea&linkGroup=${linkGroupNo}`);
...
export function mapDbToFmeaInfo(project: any): FMEAInfo {
  ...
  return {
    ...
    fmeaId: project.id || '',
    fmeaType: (info.fmeaType || project.fmeaType || 'D') as FMEAType,
