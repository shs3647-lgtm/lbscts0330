/**
 * @file DfmeaDashboardCharts.tsx
 * @description recharts 기반 DFMEA 대시보드 차트 (dynamic import 최적화용)
...
interface DfmeaDashboardChartsProps {
  statusPieData: StatusPieItem[];
  stepProgressData: StepProgressItem[];
}

export default function DfmeaDashboardCharts({ statusPieData, stepProgressData }: DfmeaDashboardChartsProps) {
  return (
    <>
      {/* 상태별 현황 도넛 차트 */}
      <div
      ...
      >
        <h2 className="text-xs font-semibold text-white text-center mb-2">📊 상태별 현황(Status Overview)</h2>
        {/* 범례 */}
        <div className="flex justify-center gap-3 mb-2">
          <div className="flex items-center">
            <div className="w-3 h-3 rounded mr-1" style={{ backgroundColor: '#22c55e' }}></div>
            <span className="text-[10px] text-gray-300">완료(Done)</span>
          </div>
          <div className="flex items-center">
            <div className="w-3 h-3 rounded mr-1" style={{ backgroundColor: '#3b82f6' }}></div>
            <span className="text-[10px] text-gray-300">진행중(In Progress)</span>
          </div>
          <div className="flex items-center">
            <div className="w-3 h-3 rounded mr-1" style={{ backgroundColor: '#eab308' }}></div>
            <span className="text-[10px] text-gray-300">지연(Delayed)</span>
          </div>
        </div>
        {/* 도넛 차트 */}
      ...
      >
        <h2 className="text-sm font-semibold text-white mb-2">📊 단계별 진행 현황(Step Progress)</h2>
