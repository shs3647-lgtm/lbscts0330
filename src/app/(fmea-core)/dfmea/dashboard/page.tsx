/**
 * @file page.tsx
 * @description DFMEA 대시보드 페이지 - 실제 데이터 연동
 * @version 2.0.0
 * @created 2026-02-02
...
const DfmeaDashboardCharts = dynamic(
    () => import('./DfmeaDashboardCharts'),
    {
        ssr: false,
        loading: () => (
            <>
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            if (result.success && result.projects) {
                ...
                setProjects(dfmeaProjects);
            } else {
                // ★ 온프레미스: fake 데이터 주입 금지 → 빈 목록 유지
                setProjects([]);
            }
        } catch (error) {
            console.error('[DFMEA Dashboard] 데이터 로드 실패:', error);
                ...
                <div className="grid grid-cols-2 gap-3">
                    ...
                    >
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            {[
                                { label: t('프로젝트 등록'), href: '/dfmea/register', icon: '➕', color: '#3b82f6' },
                                { label: t('DFMEA 리스트'), href: '/dfmea/list', icon: '📋', color: '#10b981' },
                                { label: t('워크시트 작성'), href: '/dfmea/worksheet', icon: '📝', color: '#8b5cf6' },
                                { label: t('AP 개선관리'), href: '/dfmea/ap-improvement', icon: '🔧', color: '#f59e0b' },
                                { label: t('LLD(필터코드)'), href: '/dfmea/lld', icon: '📋', color: '#ef4444' },
                                { label: t('개정관리'), href: '/dfmea/revision', icon: '📑', color: '#06b6d4' },
                            ].map((item) => (
                                <a
                                    key={item.label}
                                    href={item.href}
                            ...
                            <tbody>
                                {projects.slice(0, 5).map((project) => (
                                    <tr key={project.id} className="border-b border-gray-800 hover:bg-gray-800/50">
                                        ...
                                        <td className="p-3 text-center">
                                            <a
                                                href={`/dfmea/worksheet?id=${project.id}`}
                                ...
                                {projects.length === 0 && (
                                    <tr>
                                        <td colSpan={7} className="p-8 text-center text-gray-500">
                                            {t('등록된 DFMEA 프로젝트가 없습니다.')}
                                            <a href="/dfmea/register" className="text-blue-400 ml-2 hover:underline">{t('새 프로젝트 등록')} →</a>
                                        </td>
                                    </tr>
                                )}
                            </tbody>
