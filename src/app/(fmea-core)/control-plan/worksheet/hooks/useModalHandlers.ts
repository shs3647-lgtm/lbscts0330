...
export function useModalHandlers({
  ...
  const handleAutoModeClick = useCallback((rowIdx: number, type: ContextMenuType, colKey?: string) => {
    ...
    if (colKey === 'workElement' || type === 'work') {
      if (item.processNo && item.processName) {
        setEquipmentModal({ 
          visible: true, 
          rowIdx,
    ...
    if (selectedEquips.length > 0) {
      setState((prev: any) => {
        ...
        for (let i = 1; i < selectedEquips.length; i++) {
          const newItem = createEmptyItem(prev.cpNo || '', currentItem.processNo || '', currentItem.processName || '');
    ...
    if (Array.isArray(values) && values.length > 0) {
      setState((prev: any) => {
        ...
        for (let i = 1; i < values.length; i++) {
          ...
          newItem.detectorEp = currentItem.detectorEp || false;  // EP
          newItem.detectorAuto = currentItem.detectorAuto || false;  // 자동검사
          newItem.charNo = currentItem.charNo || '';
          // 현재 컬럼과 상위 특성 컬럼 복사
    ...
    setState((prev: any) => {
      ...
      sortedProcesses.forEach((process, idx) => {
        ...
        if (idx === 0 && targetRowIdx >= 0 && targetRowIdx < newItems.length) {
          ...
          // ★ 나머지 공정: 새 행 추가 (A, B열만)
          const newItem = createEmptyItem(prev.cpNo || '', processNo, processName);
