...
/**
 * ★★★ 부품명(partName) rowSpan 계산 ★★★
 * 연속된 같은 부품명 값만으로 그룹핑 (공정정보 제외)
 */
export function usePartNameRowSpan(items: CPItem[]): SpanInfo[] {
  return useMemo(() => {
    const result: SpanInfo[] = [];
    let i = 0;
...
export function useWorkRowSpan(items: CPItem[]): SpanInfo[] {
  return useMemo(() => {
    ...
    while (i < items.length) {
      const currentItem = items[i];
      // ★ 설비명(equipment) 값만으로 그룹핑 (연속된 같은 값이면 병합)
