...
describe('assignParentsByRowSpan — 같은 공정 내 excelRow+rowSpan 기반 매핑', () => {
  ...
  it('rowSpan 범위 밖 자식 → proximity fallback으로 가장 가까운 선행 부모에 배정', () => {
    const parents = [
      { id: 'p0', processNo: '10', excelRow: 5, rowSpan: 2 },
    ];
    const children = [
      { id: 'c0', processNo: '10', excelRow: 5 },    // rowSpan 범위 내 매핑
      { id: 'c-orphan', processNo: '10', excelRow: 99 }, // 범위 밖 → proximity fallback
    ];

    const result = assignParentsByRowSpan(parents, children);

    expect(result.get('c0')).toBe('p0');
    // proximity fallback: p0(row 5) <= c-orphan(row 99) → 가장 가까운 선행 부모 = p0
    expect(result.get('c-orphan')).toBe('p0');
  });
