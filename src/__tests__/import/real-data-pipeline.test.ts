 ...
 *
 * 핵심 검증: 고장연결 누락 0건
 *   - A5(고장형태/FM) 107건 → process.failureModes 107건
 *   - B4(고장원인/FC) 251건 → process.failureCauses 251건
 *   - C4(고장영향/FE) 22건  → l1.failureScopes ≥22건
 *   - SA 확정 → buildWorksheetState 성공
 */
...
const EXPECTED = {
  ...
  a5FM: 107,      // 고장형태 (Failure Mode) — 2026-03-10: 불필요 placeholder 제거 (원본=워크시트)
  b4FC: 251,      // 고장원인 (Failure Cause)
...
describe('실제 엑셀 데이터(1515건) Import→SA→고장연결 파이프라인', () => {
  ...
  describe('2. buildWorksheetState 고장연결 완전성', () => {
    ...
    it('2-4. ★ B4(고장원인/FC) 251건 누락 없음', () => {
      let totalFC = 0;
      for (const proc of result.state.l2) {
        totalFC += (proc.failureCauses || []).length;
      }
      expect(totalFC).toBe(EXPECTED.b4FC);
      expect(result.diagnostics.fcCount).toBe(EXPECTED.b4FC);
    });
    ...
    it('2-7. 공정별 FC 건수 일치', () => {
      const expectedByProcess: Record<string, number> = {
        '10': 6, '20': 19, '30': 18, '40': 23, '50': 15,
        '60': 17, '70': 16, '80': 22, '90': 21, '100': 21,
        '110': 18, '120': 12, '130': 11, '140': 9, '150': 8,
        '160': 7, '170': 8,
      };
      ...
      for (const proc of result.state.l2) {
        ...
        expect(actual, `공정 ${proc.no} FC: expected=${expected}, actual=${actual}`).toBe(expected);
      }
    });
  ...
  describe('2.5. 단계별 누락 0건 검증', () => {
    ...
    // ── 3L 기능 ──
    it('2.5-7. [3L기능] B2(요소기능) 128건 누락 없음', () => {
      const b2Count = realData.filter(d => d.itemCode === 'B2').length;
      let totalL3Func = 0;
      for (const proc of result.state.l2) {
      ...
      expect(totalL3Func).toBeGreaterThanOrEqual(b2Count);
    });

    it('2.5-8. [3L기능] B3(공정특성) 129건 누락 없음', () => {
      const b3Count = realData.filter(d => d.itemCode === 'B3').length;
      ...
      expect(totalProcessChar).toBeGreaterThanOrEqual(b3Count);
    });

    // ── 고장원인 ──
    it('2.5-9. [고장원인] B4(고장원인/FC) 251건 누락 없음', () => {
      let totalFC = 0;
      for (const proc of result.state.l2) {
        totalFC += (proc.failureCauses || []).length;
      }
  ...
  describe('3. SA→FC→FA 순차 확정 흐름', () => {
    ...
    it('3-3. 전체 흐름: SA→FC→FA 한 번에', () => {
      ...
      expect(buildResult.diagnostics.fcCount).toBe(EXPECTED.b4FC);
  ...
  describe('4. FM↔FC 연결 검증 (고장형태↔고장원인)', () => {
    ...
    it('4-1. 모든 공정에 FM이 있으면 FC도 있어야 함', () => {
      for (const proc of result.state.l2) {
        const fmCount = (proc.failureModes || []).length;
        const fcCount = (proc.failureCauses || []).length;
        // FM이 있는 공정은 반드시 FC도 있어야 함
        if (fmCount > 0) {
          expect(fcCount, `공정 ${proc.no}: FM=${fmCount}인데 FC=${fcCount}`).toBeGreaterThan(0);
        }
      }
    });
    ...
    it('4-3. 모든 FC에 m4 정보 보존', () => {
      ...
      // B4 원본에 m4가 있는 건은 FC에도 m4가 보존되어야 함
      const b4WithM4 = realData.filter(d => d.itemCode === 'B4' && d.m4).length;
      expect(fcWithM4).toBe(b4WithM4);
    });
  });
  ...
  describe('5.5. 특별특성(specialChar) 100% 전파', () => {
    ...
    it('5.5-2. B3 특별특성 3건 → processChars에 100% 전파', () => {
      ...
      expect(scCount).toBeGreaterThanOrEqual(3);
    });
  ...
  describe('5.6. FC 고장사슬 완전성', () => {
    ...
    it('5.6-1. FM 107건 × FC 조합 → 고장사슬 구성 가능', () => {
      // FM 107건과 FC 251건이 모두 빌드되어야 고장사슬 구성 가능
      let fmTotal = 0, fcTotal = 0;
      for (const proc of result.state.l2) {
        fmTotal += (proc.failureModes || []).length;
        fcTotal += (proc.failureCauses || []).length;
      }
      expect(fmTotal).toBe(107);  // FM 누락 0건 (원본 그대로)
      expect(fcTotal).toBe(251);  // FC 누락 0건
    });

    it('5.6-2. 모든 공정에 FM+FC 쌍이 존재 (고장사슬 구성 전제)', () => {
      for (const proc of result.state.l2) {
        const fmCount = (proc.failureModes || []).length;
        const fcCount = (proc.failureCauses || []).length;
        // 모든 공정에 FM과 FC 모두 존재해야 고장사슬 구성 가능
        expect(fmCount, `공정 ${proc.no}: FM 없음`).toBeGreaterThan(0);
        expect(fcCount, `공정 ${proc.no}: FC 없음`).toBeGreaterThan(0);
      }
    });

    it('5.6-3. FE(22건) + FM(107건) + FC(251건) = 고장 3요소 완전', () => {
      const feCount = (result.state.l1.failureScopes || []).length;
      expect(feCount).toBeGreaterThanOrEqual(22);
      // 고장사슬 = FE × FM × FC 조합, 모든 요소가 빌드에 포함되어야 함
      expect(result.diagnostics.fmCount + result.diagnostics.fcCount + result.diagnostics.feCount)
        .toBeGreaterThanOrEqual(107 + 251 + 22);
    });
