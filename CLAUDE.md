...
### 🔴 Rule 0: DB 원자성 원칙 — 최우선 아키텍처 규칙 (2026-03-14)
...
| 구간 | 원칙 | 금지 패턴 |
|------|------|----------|
| **BD Import 파싱** | 파싱 결과는 DB 트랜잭션 내에서 원자적 저장 | 부분 저장, 트랜잭션 미래핑 |
...
| **렌더링** | DB에서 로드한 데이터만 렌더링 (메모리 조작 금지) | 로드 경로에 필터/변환 삽입 |
| **FC 고장사슬** | Map.get() 결정론적 매칭 (유사도 매칭 금지) | `distribute()` 순서 배분, 임계값 매칭 |
| **CP 연동** | 동일 productCharId FK로 FMEA↔CP 참조 | 텍스트 기반 재매칭, ID 재생성 |
| **PFD 연동** | 동일 processId FK로 FMEA↔PFD 참조 | 순서 기반 매핑, 이름 기반 추측 |

#### 카테시안 복제 절대 금지 (CRITICAL)
...
1. **공유 엔티티 단일 생성**: A4(제품특성)는 공정 단위 1회 생성, FK로만 참조
2. **트랜잭션 래핑 필수**: Import/저장/연동 모든 DB 작업은 `$transaction` 내에서 실행
3. **결정론적 매칭**: Map.get() 조회만 허용, 유사도/임계값/순서 기반 매칭 금지
