...
| 2026-03-22 | 고장매칭(누락 FM·FC 자동연결) 후 DB/마스터 미반영 | `handleAutoMatchMissing`이 `saveTemp`만 호출하고 `saveAtomicDB` 미호출 + 신규 링크 `id` 누락; `upsertActiveMasterFromWorksheetTx`가 flat 0건이면 `failureChains` 동기화 스킵 | `useLinkHandlers.ts`: 매칭 후 `saveToLocalStorage(true)`+`saveAtomicDB(true)`, 신규 링크에 `uid()`; `sync.ts`: flat 0건이어도 `failureLinks` 있으면 체인 동기화 | ✅ tsc |
...
- **이 룰은 영구 CODEFREEZE** — 수정 시 반드시: "Rule 3.1 FK 필드 보호를 수정해도 될까요? 수정 사유: ___"

### 🔴 Rule 3.2: isPositionBasedFormat 라우팅 보호 — "통합" 키워드 차단 재삽입 절대 금지 (2026-03-23) — 영구 CODEFREEZE
...
- **이 룰은 영구 CODEFREEZE** — 수정 시 반드시: "Rule 3.2 isPositionBasedFormat 라우팅 보호를 수정해도 될까요? 수정 사유: ___"
