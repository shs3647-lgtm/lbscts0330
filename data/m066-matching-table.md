# pfm26-m066 구조분석→최적화 UUID/PG/PK/SW 매칭표
> 생성: 2026-03-18T23:46:03.090Z | Schema: `pfmea_pfm26_m066`
...
| L2 | ProcessProductChar | **129** | process_product_chars |
| L2 | FailureMode(FM) | **26** | failure_modes |
| L3 | L3Structure | **91** | l3_structures |
| L3 | L3Function | **103** | l3_functions |
| L3 | FailureCause(FC) | **103** | failure_causes |
| FK | FailureLink | **104** | failure_links |
| 분석 | FailureAnalysis | **104** | failure_analyses |
| 위험 | RiskAnalysis | **104** | risk_analyses |
| 최적화 | Optimization | **104** | optimizations |
...
|--------|------|------|----------|---------|----------|---|
| `PF-L1-SP` | SP | 고객 기능 안정성을 위한 Au 순도 및 IMC | IMC 두께 고객 규격(IMC Thi | `PF-L1-SP` | IMC 두께 Spec Out으로 인한 접합부  | 7 |
| `PF-L1-SP` | SP | 고객 납품 기준(높이·외관·포장)을 충족하는  | 고객 납품 파티클 기준(Custome | `PF-L1-SP` | 라벨 불일치로 인한 Lot 혼입 불량 유출 | 5 |
| `PF-L1-SP` | SP | 고객 납품 기준(높이·외관·포장)을 충족하는  | Au Bump 외관 고객 기준(Cus | `PF-L1-SP` | Particle 오염 불량 유출로 인한 고객  | 5 |
| `PF-L1-SP` | SP | 고객 납품 기준(높이·외관·포장)을 충족하는  | Au Bump 높이 고객 출하 규격( | `PF-L1-SP` | IMC 과성장에 의한 접합부 열화 | 5 |
| `PF-L1-SP` | SP | 고객 기능 안정성을 위한 Au 순도 및 IMC | Au 순도 고객 규격(Au Purit | `PF-L1-SP` | 고객 Packing 기준 부적합으로 인한 납품 | 5 |
| `PF-L1-SP` | SP | 고객 기능 안정성을 위한 Au 순도 및 IMC | 포장 기준 적합성(Packaging  | `PF-L1-SP` | 외관 결함 불량 유출(고객 Outgoing D | 4 |
...
| `PF-L1-YP` | YP | Wafer 청정도(파티클 수)가 공정 기준을  | UBM 두께 규격(UBM Thickn | `PF-L1-YP` | 전기적 Open/Short | 5 |
| `PF-L1-YP` | YP | Au Bump 제품특성(높이·순도·외관)이 자 | 파티클 수 허용 기준(Max Part | `PF-L1-YP` | Lot 혼입으로 인한 Wafer 수율 손실 | 7 |
| `PF-L1-YP` | YP | Au Bump 제품특성(높이·순도·외관)이 자 | Au Bump 외관 결함 기준(Vis | `PF-L1-YP` | Wafer 표면 결함 미검출로 인한 후공정 유 | 5 |
| `PF-L1-YP` | YP | Au Bump 제품특성(높이·순도·외관)이 자 | Au Bump 높이 규격(Bump H | `PF-L1-YP` | Particle 오염으로 인한 제품 특성 이상 | 5 |
| `PF-L1-YP` | YP | UBM·PR·Etch 공정특성이 규격을 충족하 | Seed 잔류물 허용 기준(Seed  | `PF-L1-YP` | Bump Height Spec Out으로 인한 | 8 |
| ↑|↑|↑|↑ | `PF-L1-YP` | CD Spec Out으로 인한 Bump 형성  | 5 |
| ↑|↑|↑|↑ | `PF-L1-YP` | Plating 균일도 Spec Out으로 인한 | 4 |
| ↑|↑|↑|↑ | `PF-L1-YP` | PR Scum 잔류로 인한 Au Bump 형성 | 5 |
| `PF-L1-YP` | YP | UBM·PR·Etch 공정특성이 규격을 충족하 | PR 잔사 허용 기준(PR Resid | `PF-L1-YP` | Bump간 Bridge | 5 |
| `PF-L1-YP` | YP | Wafer 청정도(파티클 수)가 공정 기준을  | PR 두께 규격(PR Thicknes | `PF-L1-YP` | PR 두께 Spec Out으로 인한 패턴 불량 | 5 |
| `PF-L1-YP` | YP | Wafer 청정도(파티클 수)가 공정 기준을  | CD 규격(Critical Dimen | `PF-L1-YP` | Bump Lift-off | 7 |

## 3. L2 공정
| No | L2 UUID | 공정명 | L2Func | 제품특성(PPC) | FM UUID | 고장형태 |
|----|---------|--------|--------|---------------|---------|----------|
| 01 | `PF-L2-00` | 작업환경 | 클린룸 환경을 온습도·청정도 | 청정도 등급(ISO Clas | `PF-L2-00` | 파티클 초과 |
| ↑|↑|↑ |  | 작업 숙련도 | | |
| ↑|↑|↑ |  | 풍속(Air Velocity | | |
| ↑|↑|↑ |  | 자재 규격 기준(Materi | | |
| ↑|↑|↑ |  | 파티클 수 | | |
...
| ↑|↑|↑ | IQA(입고검사)를 통해 입 | Wafer TTV(Total | `PF-L2-01` | TTV 규격 초과 |
| ↑|↑|↑ |  | 검사 숙련도 | | |
| ↑|↑|↑ |  | 측정 정밀도(Accuracy | | |
| ↑|↑|↑ |  | 광학 해상도(Optical  | | |
| ↑|↑|↑ |  | 표면 결함 밀도(Defect | | |
| ↑|↑|↑ |  | 청정도 등급(ISO Clas | | |
| 20 | `PF-L2-02` | Sorter | Sorter 장비를 이용하여 | 청정도 등급(ISO Clas | `PF-L2-02` | OCR ID 오인식 또는 Lot 혼입 |
| ↑|↑|↑ | Sorter 장비를 이용하여 | 자재 규격 기준(Materi | `PF-L2-02` | Wafer 정렬 불량 |
| ↑|↑|↑ |  | 인식 정확도(Recognit | | |
| ↑|↑|↑ |  | 정렬 반복 정밀도(Repea | | |
| ↑|↑|↑ |  | 작업 숙련도 | | |
| ↑|↑|↑ |  | Wafer 정렬 정확도 | | |
| ↑|↑|↑ |  | OCR ID 인식 정확도 | | |
| 30 | `PF-L2-03` | Scrubber | Scrubber 장비로 Wa | 비저항(Resistivity | `PF-L2-03` | Wafer 표면 이물 잔류(Parti |
| ↑|↑|↑ |  | 분사 압력(Spray Pre | | |
| ↑|↑|↑ |  | 작업 숙련도 | | |
| ↑|↑|↑ |  | 파티클 잔류수(Particl | | |
| ↑|↑|↑ |  | 청정도 등급(ISO Clas | | |
| 40 | `PF-L2-04` | UBM Sputter | Sputter 장비로 Waf | Au Thickness(Å) ◇ | `PF-L2-04` | UBM 두께 부족(Thickness  |
| ↑|↑|↑ | Sputter 장비로 Waf | 증착 속도(Depositio | `PF-L2-04` | 막질 불균일(Film Non-unif |
| ↑|↑|↑ |  | UBM 두께(UBM Thic | | |
| ↑|↑|↑ |  | 막질 균일도(Film Uni | | |
| ↑|↑|↑ |  | 진공도(Base Pressu | | |
| ↑|↑|↑ |  | Target 두께 잔량(Re | | |
...
| ↑|↑|↑ |  | 작업 숙련도 | | |
| ↑|↑|↑ |  | RF Etch Amount( ◇ | | |
| 50 | `PF-L2-05` | Scrubber2 | UBM Sputter 후 W | 비저항(Resistivity | `PF-L2-05` | Wafer 표면 이물 잔류(Parti |
| ↑|↑|↑ |  | 청정도 등급(ISO Clas | | |
| ↑|↑|↑ |  | 파티클 잔류수(Particl | | |
| ↑|↑|↑ |  | 작업 숙련도 | | |
| ↑|↑|↑ |  | 분사 압력(Spray Pre | | |
| 60 | `PF-L2-06` | PR Coating | Coater 장비로 Wafe | 청정도 등급(ISO Clas | `PF-L2-06` | PR 두께 불균일(PR Thickne |
| ↑|↑|↑ |  | PR 두께(PR Thickn | | |
| ↑|↑|↑ |  | 작업 숙련도 | | |
| ↑|↑|↑ |  | 회전 속도(RPM) | | |
| ↑|↑|↑ |  | 점도(Viscosity, c | | |
...
| ↑|↑|↑ |  | CD 정밀도(Reticle  | | |
| ↑|↑|↑ |  | 청정도 등급(ISO Clas | | |
| 80 | `PF-L2-08` | Develop | Developer로 노광된  | 작업 숙련도 | `PF-L2-08` | Under/Over develop(O |
| ↑|↑|↑ |  | 현상 시간(Develop T | | |
| ↑|↑|↑ |  | TMAH 농도(Concent | | |
| ↑|↑|↑ |  | Opening 정확도(Ope | | |
| ↑|↑|↑ |  | 청정도 등급(ISO Clas | | |
| 90 | `PF-L2-09` | Descum | Descum 장비로 O₂ P | O₂ Plasma Power | `PF-L2-09` | PR Scum 잔류(Descum 불완 |
| ↑|↑|↑ |  | 청정도 등급(ISO Clas | | |
| ↑|↑|↑ |  | 작업 숙련도 | | |
| ↑|↑|↑ |  | Gas 유량(Flow Rat | | |
| ↑|↑|↑ |  | PR 잔사 수(Scum Co | | |
| 100 | `PF-L2-10` | Au Plating | Au Plating Tank | 작업 숙련도 ★ | `PF-L2-10` | Au Bump 높이 편차(Height |
| ↑|↑|↑ | Au Plating Tank | 도금 시간(Plating T | `PF-L2-10` | Au 순도 저하(Purity Degr |
| ↑|↑|↑ |  | 청정도 등급(ISO Clas ★ | | |
| ↑|↑|↑ |  | Au 순도(Au Purity | | |
| ↑|↑|↑ |  | Au Bump 높이(Bump ★ | | |
| ↑|↑|↑ |  | Au 이온 농도(Au Con ★ | | |
...
| ↑|↑|↑ |  | 도금 전류밀도(Current ★ | | |
| ↑|↑|↑ |  | 도금 균일도(Plating  | | |
| 110 | `PF-L2-11` | PR Strip | Strip 장비로 도금 후  | 청정도 등급(ISO Clas | `PF-L2-11` | PR 잔사 잔류(Strip 불완전) |
| ↑|↑|↑ |  | 작업 숙련도 | | |
| ↑|↑|↑ |  | Strip 온도(Temper | | |
| ↑|↑|↑ |  | 용제 농도(Concentra | | |
| ↑|↑|↑ |  | PR 잔사(Strip Res | | |
| 120 | `PF-L2-12` | Au Etch | Au Etchant를 이용하 | H₂O₂ Etch Rate( ◇ | `PF-L2-12` | 에칭 부족/과다(Under/Over  |
| ↑|↑|↑ |  | 에칭 잔류물(Etch Res | | |
| ↑|↑|↑ |  | 작업 숙련도 | | |
| ↑|↑|↑ |  | 식각 시간(Etch Time | | |
| ↑|↑|↑ |  | Au Etch Rate(Å/ ◇ | | |
...
| ↑|↑|↑ |  | 해상도(Resolution, | | |
| ↑|↑|↑ |  | Defect 수(Defect | | |
| 200 | `PF-L2-20` | OGI/Packing | 포장 장비 및 절차에 따라  | 자재 규격 기준(Materi | `PF-L2-20` | 포장 불량(Packaging Defe |
| ↑|↑|↑ |  | 청정도 등급(ISO Clas | | |
| ↑|↑|↑ |  | 포장 상태(Packaging | | |
| ↑|↑|↑ |  | 작업 숙련도 | | |
| ↑|↑|↑ |  | 진공 압력(Vacuum Pr | | |
...
|--------|---------|----------|----|--------|----------|---------|----------|
| 01 | `PF-L3-00` | 클린룸 | EN | 클린룸이 외부 오염을 차단하 | 청정도 등급(ISO C | `PF-L3-00` | 온습도 제어 이탈 |
...
| 01 | `PF-L3-00` | 항온항습기 | MC | 항온항습기가 클린룸 온습도를 | 온도(Temperatu | `PF-L3-00` | 습도(Humidity, %RH) 부적 |
| ↑|↑|↑|↑ | 항온항습기가 클린룸 온습도를 | 습도(Humidity, | `PF-L3-00` | 설비 가동률 저하 |
...
| 10 | `PF-L3-01` | 두께 측정기 | MC | 두께 측정기가 Wafer 두 | 측정 정밀도(Accur | `PF-L3-01` | 측정 오차 |
...
| 20 | `PF-L3-02` | Sorter 장비 | MC | Sorter 장비가 Wafe | 정렬 반복 정밀도(Re | `PF-L3-02` | 정렬 센서 오작동 |
...
| 40 | `PF-L3-04` | Cu Target | IM | Cu Target이 스퍼터링 | Target 두께 잔량 | `PF-L3-04` | Target 두께 잔량(Remaini |
| 40 | `PF-L3-04` | Ti Target | IM | Ti Target이 스퍼터링 | Target 두께 잔량 | `PF-L3-04` | Target 소진 |
| 40 | `PF-L3-04` | DC Power Supply | MC | DC Power Supply | 전압 안정도(Volta | `PF-L3-04` | 전압 안정도(Voltage Stabi |
| 40 | `PF-L3-04` | Sputter 장비 | MC | Sputter 장비가 Ti/ | 증착 속도(Deposi | `PF-L3-04` | 전압 변동 |
| ↑|↑|↑|↑ | Sputter 장비가 Ti/ | DC Power(kW) | `PF-L3-04` | TiW 증착 두께 편차 |
| ↑|↑|↑|↑ | Sputter 장비가 Ti/ | RF Etch Amou ◇ | `PF-L3-04` | Au 증착 두께 편차 |
| ↑|↑|↑|↑ | Sputter 장비가 Ti/ | Au Thickness ◇ | `PF-L3-04` | RF Etch Amount 규격 이탈 |
| ↑|↑|↑|↑ | Sputter 장비가 Ti/ | TiW Thicknes ◇ | `PF-L3-04` | Power 변동 |
...
| 100 | `PF-L3-10` | Au Plating Tank | MC | Au Plating Tank | 도금 균일도(Plati | `PF-L3-10` | 전류밀도 편차 |
| ↑|↑|↑|↑ | Au Plating Tank | 도금 시간(Platin | `PF-L3-10` | 전류 변동 |
| ↑|↑|↑|↑ | Au Plating Tank | 도금 전류밀도(Curr ★ | `PF-L3-10` | 도금 균일도 편차 |
...
| 120 | `PF-L3-12` | Au Etchant | IM | Au Etchant가 Au  | Etchant 농도(C | `PF-L3-12` | H₂O₂ 농도 편차에 의한 Etch  |
| ↑|↑|↑|↑ | Au Etchant가 Au  | H₂O₂ Etch Ra ◇ | `PF-L3-12` | 자재 품질 부적합 |
| 120 | `PF-L3-12` | Au Etch 장비 | MC | Au Etch 장비가 Au  | 식각 시간(Etch T | `PF-L3-12` | 에칭 시간 편차 |
| ↑|↑|↑|↑ | Au Etch 장비가 Au  | Au Etch Rate ◇ | `PF-L3-12` | Au Etch Rate 편차 |
...
| 140 | `PF-L3-14` | Anneal 장비 | MC | Anneal 장비가 설정 온 | 열처리 시간(Annea | `PF-L3-14` | 열처리 시간(Anneal Time,  |
| ↑|↑|↑|↑ | Anneal 장비가 설정 온 | 열처리 온도(Annea | `PF-L3-14` | 온도 과다/부족 |
...
| 150 | `PF-L3-15` | AVI 장비 | MC | AVI 장비가 Wafer 전 | 해상도(Resoluti ★ | `PF-L3-15` | 해상도 저하 |
| 150 | `PF-L3-15` | 높이 측정기 | MC | 높이 측정기가 Bump 높이 | 측정 정밀도(Accur | `PF-L3-15` | 측정 오차 |
| ↑|↑|↑|↑ | 높이 측정기가 Bump 높이 | 공간적 균일도 측정 정 | `PF-L3-15` | 측정 균일도 편차 |
...
|------|-----|---------|-----|---------|-----|---------|---|
| `88399095` | `PF-L2-04` | UBM 두께 부족(Thicknes | `PF-L1-YP` | PR 두께 Spec Out으로 인 | `PF-L3-04` | Target 소진 | 5 |
...
| `PF-FC-07` | `PF-L2-07` | CD 규격 이탈(CD Out of | `PF-L1-YP` | Bump Height Spec O | `PF-L3-07` | 환경 조건 이탈 | 8 |
| `PF-FC-07` | `PF-L2-07` | CD 규격 이탈(CD Out of | `PF-L1-YP` | Bump Height Spec O | `PF-L3-07` | Mask 결함 | 8 |
| `PF-FC-07` | `PF-L2-07` | CD 규격 이탈(CD Out of | `PF-L1-YP` | Bump Height Spec O | `PF-L3-07` | 에너지 편차 | 8 |
| `PF-FC-07` | `PF-L2-07` | CD 규격 이탈(CD Out of | `PF-L1-YP` | Bump Height Spec O | `PF-L3-07` | 작업 숙련도 부족 | 8 |
...
|------|---|---|---|----|-------------|-------------|
| `88399095` | 5 | 4 | 1 | L | P:Au 증착 두께 인라인 모니터링, | D:4-Point Probe 두께 측 |
| `auto-PF-` | 5 | 4 | 1 | L | P:Au 증착 두께 인라인 모니터링, | D:4-Point Probe 두께 측 |
...
| `PF-FC-02` | 7 | 3 | 1 | L | P:Sorter 장비 정기 PM, 정 | D:OCR Reader 자동 인식 확 |
| `PF-FC-02` | 7 | 3 | 1 | L | P:OCR Reader 정기 교정,  | D:OCR Reader 자동 인식 확 |
| `PF-FC-02` | 7 | 5 | 1 | M | P:Sorter 작업 표준 교육, 인 | D:OCR Reader 자동 인식 확 |
...
| `PF-FC-04` | 5 | 4 | 1 | L | P:Sputter 장비 정기 PM,  | D:4-Point Probe 두께 측 |
| `PF-FC-04` | 5 | 4 | 1 | L | P:DC Power Supply 전압 | D:4-Point Probe 두께 측 |
| `PF-FC-04` | 5 | 4 | 1 | L | P:Au 증착 두께 인라인 모니터링, | D:4-Point Probe 두께 측 |
| `PF-FC-04` | 5 | 4 | 1 | L | P:Au 증착 두께 인라인 모니터링, | D:4-Point Probe 두께 측 |
| `PF-FC-04` | 5 | 5 | 1 | L | P:Sputter Recipe 승인제 | D:4-Point Probe 두께 측 |
| `PF-FC-04` | 5 | 2 | 1 | L | P:진공 챔버 기밀 점검, 진공도 실 | D:4-Point Probe 두께 측 |
| `PF-FC-04` | 5 | 4 | 1 | L | P:Au 증착 두께 인라인 모니터링, | D:4-Point Probe 두께 측 |
| `PF-FC-04` | 5 | 4 | 1 | L | P:Au 증착 두께 인라인 모니터링, | D:4-Point Probe 두께 측 |
| `PF-FC-04` | 5 | 4 | 1 | L | P:Au 증착 두께 인라인 모니터링, | D:4-Point Probe 두께 측 |
...
| `PF-FC-07` | 8 | 3 | 1 | L | P:Exposure 챔버 온습도·진동 | D:CD-SEM CD 측정, 매 Lo |
| `PF-FC-07` | 8 | 4 | 1 | M | P:Mask 이물·손상 입고 검사,  | D:CD-SEM CD 측정, 매 Lo |
| `PF-FC-07` | 8 | 4 | 1 | M | P:Stepper/Scanner 광원 | D:CD-SEM CD 측정, 매 Lo |
| `PF-FC-07` | 8 | 5 | 1 | M | P:Exposure Recipe 승인 | D:CD-SEM CD 측정, 매 Lo |
...
## 7. Optimization 최적화
| Risk | 추천조치 | 담당 | 신S | 신O | 신D | 신AP | 상태 |
|------|----------|------|-----|-----|-----|------|------|
| `id_00b77` | [LLD26-002] | 서고객 |  |  |  |  | open |
| `id_01181` | [LLD26-005] | 서고객 |  |  |  |  | open |
| `id_033b1` | [LLD26-005] | 서고객 |  |  |  |  | open |
| `id_03928` | P:P:검사원 인증 교육, 판정 기준 | 서고객 |  |  | 5 |  | open |
| `id_06050` | [LLD26-001] | 서고객 |  |  |  |  | open |
| `id_089a9` | P:P:Stepper/Scanner  | 서고객 |  |  |  |  | 대기 |
| `id_08bb0` | [LLD26-007] | 서고객 |  |  | 4 |  | open |
| `id_0bbf7` | P:P:Au Etch 장비 식각 시간 | 서고객 |  |  |  |  | open |
| `id_0bc87` | P:P:Exposure Recipe  | 서고객 |  |  |  |  | 대기 |
| `id_194ba` | P:P:포장 작업자 교육, 포장 체크 | 서고객 |  |  | 6 |  | 대기 |
| `id_1ae97` | P:P:클린룸 작업자 교육 및 인증, | 서고객 |  |  |  |  | open |
| `id_1af78` | [LLD26-002] | 서고객 |  |  |  |  | open |
| `id_1b9c6` | [LLD26-001] | 서고객 |  |  |  |  | 대기 |
| `id_1ca72` | P:P:포장 작업자 교육, 포장 체크 | 서고객 |  |  | 6 |  | 대기 |
| `id_1e163` | [LLD26-007] | 서고객 |  |  |  |  | 대기 |
| `id_1f052` | P:P:OCR Reader 정기 교정 | 서고객 |  |  |  |  | open |
| `id_1fa30` | [LLD26-002] | 서고객 |  |  |  |  | open |
| `id_20e18` | [LLD26-002] | 서고객 |  |  |  |  | open |
| `id_237ed` | P:P:FFU 풍속 정기 측정 및 P | 서고객 |  |  |  |  | open |
| `id_26174` | P:P:클린룸 작업자 교육 및 인증, | 서고객 |  |  |  |  | open |
| `id_2888b` | [LLD26-007] | 서고객 |  |  | 7 |  | 대기 |
| `id_2af51` | P:P:H₂O₂ 농도 정기 측정, 기 | 서고객 |  |  |  |  | open |
| `id_2ba0d` | [LLD26-003] | 서고객 |  |  |  |  | open |
| `id_3057b` | [LLD26-002] | 서고객 |  |  |  |  | open |
| `id_33521` | P:P:항온항습기 정기 PM 및 교정 | 서고객 |  |  |  |  | open |
| `id_33715` | [LLD26-003] | 서고객 |  |  |  |  | open |
| `id_352fd` | P:P:HEPA 필터 정기 교체 주기 | 서고객 | 5 | 2 | 4 | L | 대기 |
| `id_37d06` | [LLD26-003] | 서고객 |  |  |  |  | 대기 |
| `id_3b8d3` | P:P:Au Etch Recipe 승 | 서고객 |  |  |  |  | open |
| `id_3c48f` | [LLD26-007] | 서고객 |  |  | 7 |  | 대기 |
| `id_3de14` | [LLD26-007] | 서고객 |  |  |  |  | open |
| `id_45fc7` | [LLD26-007] | 서고객 |  |  |  |  | 대기 |
| `id_4b138` | [LLD26-001] | 서고객 |  |  |  |  | open |
| `id_4f79c` | [LLD26-005] | 서고객 |  |  |  |  | open |
| `id_5404a` | [LLD26-001] | 서고객 |  |  |  |  | open |
| `id_58812` | P:P:Sorter 작업 표준 교육, | 서고객 | 7 | 3 | 1 | L | 대기 |
| `id_5983d` | [LLD26-007] | 서고객 |  |  | 7 |  | 대기 |
| `id_5de8e` | [LLD26-003] | 서고객 |  |  |  |  | 대기 |
| `id_6745a` | P:P:Develop 작업영역 온도· | 서고객 |  |  |  |  | open |
| `id_68a43` | P:P:Anneal Recipe 승인 | 서고객 |  |  |  |  | open |
| `id_69067` | [LLD26-005] | 서고객 |  |  |  |  | open |
| `id_6cb03` | [LLD26-003] | 서고객 |  |  |  |  | open |
| `id_6d330` | [LLD26-002] | 서고객 |  |  |  |  | open |
| `id_7358a` | [LLD26-002] | 서고객 |  |  |  |  | open |
| `id_756a8` | [LLD26-005] | 서고객 |  |  |  |  | open |
| `id_76b70` | P:P:N₂ 공급장치 유량 모니터링, | 서고객 |  |  |  |  | open |
| `id_7c624` | [LLD26-003] | 서고객 |  |  |  |  | open |
| `id_7e8e0` | [LLD26-001] | 서고객 |  |  |  |  | 대기 |
| `id_806b0` | P:P:포장 장비 진공 압력 정기 점 | 서고객 |  |  | 6 |  | 대기 |
| `id_82be0` | [LLD26-007] | 서고객 |  |  | 4 |  | open |
| `id_82d54` | P:P:Sorter 장비 정기 PM, | 서고객 |  |  |  |  | open |
| `id_84516` | [LLD26-005] | 서고객 |  |  |  |  | open |
| `id_85a25` | P:P:항온항습기 정기 PM 및 교정 | 서고객 |  |  |  |  | open |
| `id_8881a` | P:P:Au Etch Rate 정기  | 서고객 |  |  |  |  | open |
| `id_8addc` | [LLD26-007] | 서고객 |  |  |  |  | 대기 |
| `id_8fc9d` | P:P:Anneal Recipe 승인 | 서고객 |  |  |  |  | open |
| `id_90136` | P:P:검사원 인증 교육, 판정 기준 | 서고객 |  |  | 5 |  | open |
| `id_90a0b` | [LLD26-007] | 서고객 |  |  | 4 |  | open |
| `id_90c8e` | [LLD26-007] | 서고객 |  |  | 7 |  | 대기 |
| `id_92c78` | P:P:Au Etch Recipe 승 | 서고객 |  |  |  |  | open |
| `id_96447` | P:P:Exposure 챔버 온습도· | 서고객 |  |  |  |  | open |
| `id_993af` | P:P:AVI 결과 검토 교육, 이상 | 서고객 |  |  | 7 |  | 대기 |
| `id_99a7b` | [LLD26-001] | 서고객 |  |  |  |  | open |
| `id_99d5e` | [LLD26-007] | 서고객 |  |  |  |  | 대기 |
| `id_99eb4` | [LLD26-005] | 서고객 |  |  |  |  | open |
| `id_9c7db` | [LLD26-001] | 서고객 |  |  |  |  | open |
| `id_9d39b` | [LLD26-003] | 서고객 |  |  |  |  | 대기 |
| `id_a2303` | P:P:검사원 인증 교육, 판정 기준 | 서고객 |  |  | 5 |  | open |
| `id_a2860` | [LLD26-002] | 서고객 |  |  |  |  | open |
| `id_a6c42` | [LLD26-005] | 서고객 |  |  |  |  | open |
| `id_a6c93` | P:P:AVI 장비 해상도 정기 교정 | 서고객 |  |  | 7 |  | open |
| `id_af6b8` | P:P:Exposure Recipe  | 서고객 |  |  |  |  | 대기 |
| `id_afe73` | [LLD26-007] | 서고객 |  |  | 4 |  | open |
| `id_b167e` | [LLD26-002] | 서고객 |  |  |  |  | open |
| `id_b567a` | [LLD26-007] | 서고객 |  |  | 4 |  | open |
| `id_b885b` | P:P:AVI 결과 검토 교육, 이상 | 서고객 |  |  | 7 |  | 대기 |
| `id_bcbdb` | P:P:두께 측정기 정기 교정(MSA | 서고객 |  |  | 5 |  | open |
| `id_bd122` | P:P:검사원 인증 교육, 판정 기준 | 서고객 |  |  | 5 |  | open |
| `id_bea0e` | [LLD26-005] | 서고객 |  |  |  |  | open |
| `id_ce735` | P:P:현상액 농도 정기 측정, 기준 | 서고객 |  |  |  |  | open |
| `id_d01e8` | [LLD26-003] | 서고객 |  |  |  |  | open |
| `id_d0df8` | [LLD26-002] | 서고객 |  |  |  |  | open |
| `id_d4b76` | [LLD26-002] | 서고객 |  |  |  |  | open |
| `id_d59b8` | [LLD26-001] | 서고객 |  |  |  |  | 대기 |
| `id_d8d55` | [LLD26-005] | 서고객 |  |  |  |  | open |
| `id_da6cd` | P:P:Anneal 장비 온도 교정, | 서고객 |  |  |  |  | open |
| `id_db2dc` | [LLD26-003] | 서고객 |  |  |  |  | open |
| `id_dd649` | P:P:Developer 현상 시간  | 서고객 |  |  |  |  | open |
| `id_dd8a2` | [LLD26-001] | 서고객 |  |  |  |  | open |
| `id_e2be3` | [LLD26-005] | 서고객 |  |  |  |  | open |
| `id_e5fcc` | [LLD26-007] | 서고객 |  |  | 7 |  | 대기 |
| `id_e7197` | [LLD26-007] | 서고객 |  |  | 4 |  | open |
| `id_e7db3` | [LLD26-003] | 서고객 |  |  |  |  | open |
| `id_e8ed4` | P:P:포장 작업자 교육, 포장 체크 | 서고객 |  |  | 6 |  | 대기 |
| `id_ecf61` | P:P:Anneal Recipe 승인 | 서고객 |  |  |  |  | open |
| `id_ef187` | [LLD26-001] | 서고객 |  |  |  |  | open |
| `id_f0602` | [LLD26-001] | 서고객 |  |  |  |  | 대기 |
| `id_f2f43` | [LLD26-005] | 서고객 |  |  |  |  | open |
| `id_f4b04` | [LLD26-003] | 서고객 |  |  |  |  | open |
| `id_f4ff7` | [LLD26-007] | 서고객 |  |  | 4 |  | open |
| `id_f78b9` | P:P:AVI 결과 검토 교육, 이상 | 서고객 |  |  | 7 |  | 대기 |
| `id_fe08f` | [LLD26-007] | 서고객 |  |  | 7 |  | 대기 |
| `id_fe169` | [LLD26-001] | 서고객 |  |  |  |  | open |
| `id_fe73f` | P:P:Develop Recipe 승 | 서고객 |  |  |  |  | open |
