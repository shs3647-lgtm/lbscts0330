/**
 * @file pm-complete-regression.spec.ts
 * @description PM 모듈 전체 화면 TDD 회귀 테스트 (Main + Worksheet + Modal)
 * @version 1.0.0
 * @created 2026-02-01
 */
...
const BASE_URL = `${process.env.TEST_BASE_URL ?? 'http://localhost:3000'}/pm/worksheet`;

// 가상 테스트 데이터
const MOCK_PM_MAIN_DATA = {
    pmNo: 'PM-TEST-001',
    subject: 'PM 회귀 테스트',
    partName: '테스트 부품',
    partNo: 'PN-12345',
 ...
 * PM Main 탭 데이터 입력 헬퍼
 */
async function fillPmMainData(page: Page, data: typeof MOCK_PM_MAIN_DATA) {
    // 기본 정보 입력
...
test.describe('PM 모듈 전체 화면 회귀 테스트', () => {
    ...
    test.describe('1. 페이지 로딩 및 기본 UI', () => {
        ...
        test('1.1 PM 페이지가 정상적으로 로드되어야 함', async ({ page }) => {
        ...
        test('1.3 PM TopMenuBar가 표시되어야 함', async ({ page }) => {
            // PM 선택 드롭다운 확인
            const pmSelect = page.locator('select, button:has-text("PM 선택")').first();
            await expect(pmSelect).toBeVisible({ timeout: 10000 });
        });

        test('1.4 PM TabMenu가 표시되어야 함', async ({ page }) => {
            // PM Main, PM Work Sheet 탭 확인
            const mainTab = page.locator('button:has-text("PM Main")');
            const worksheetTab = page.locator('button:has-text("PM Work Sheet")');
    ...
    // ========== 2. PM Main 탭 기능 검증 ==========
    test.describe('2. PM Main 탭 기능', () => {

        test('2.1 PM Main 탭으로 전환되어야 함', async ({ page }) => {
            await switchTab(page, 'PM Main');
        ...
        test('2.2 PM Main 탭에 입력 필드가 표시되어야 함', async ({ page }) => {
            await switchTab(page, 'PM Main');
        ...
        test('2.3 PM Main 데이터 입력 테스트', async ({ page }) => {
            await switchTab(page, 'PM Main');
        ...
        test('2.4 PM Main 저장 버튼 존재 확인', async ({ page }) => {
            await switchTab(page, 'PM Main');
    ...
    // ========== 3. PM Work Sheet 탭 기능 검증 ==========
    test.describe('3. PM Work Sheet 탭 기능', () => {

        test('3.1 PM Work Sheet 탭으로 전환되어야 함', async ({ page }) => {
            await switchTab(page, 'PM Work Sheet');
    ...
    test.describe('5. 탭 전환 순차 회귀 검증 (5회 반복)', () => {
        ...
        for (let iteration = 1; iteration <= 5; iteration++) {
            test(`5.${iteration} 탭 전환 회귀 테스트 ${iteration}회차`, async ({ page }) => {
                ...
                // PM Main 컨텐츠 로딩 확인 - 더 관대한 검증
                await page.waitForTimeout(1000);
                const mainVisible = await page.locator('div, form, input').first().isVisible().catch(() => false);
                if (mainVisible) {
                    console.log(`[${iteration}회차] PM Main 탭 전환 성공`);
                } else {
                    console.log(`[${iteration}회차] PM Main 탭 컨텐츠 로딩 지연, 계속 진행`);
                }

                // PM Work Sheet 탭으로 전환
                await switchTab(page, 'PM Work Sheet');
                const worksheetTable = page.locator('table').first();
                await expect(worksheetTable).toBeVisible({ timeout: 10000 });
                console.log(`[${iteration}회차] PM Work Sheet 탭 전환 성공`);

                // 다시 PM Main으로
                await switchTab(page, 'PM Main');
                await page.waitForTimeout(1000);
                console.log(`[${iteration}회차] PM Main 재전환 성공`);
    ...
    test.describe('7. 전체 워크플로우 통합 테스트 (5회 반복)', () => {
        ...
        for (let iteration = 1; iteration <= 5; iteration++) {
            test(`7.${iteration} 전체 워크플로우 ${iteration}회차`, async ({ page }) => {
                ...
                // 1. PM Main 탭 - 데이터 입력
                await switchTab(page, 'PM Main');
                console.log(`[${iteration}회차] Step 1: PM Main 탭 전환`);
                ...
                // 2. PM Work Sheet 탭 - 테이블 확인
                await switchTab(page, 'PM Work Sheet');
                console.log(`[${iteration}회차] Step 3: PM Work Sheet 탭 전환`);
    ...
    test.describe('9. 성능 검증', () => {
        ...
        test('9.1 탭 전환 응답 시간 측정', async ({ page }) => {
            ...
            console.log(`PM Main 탭 전환 시간: ${mainSwitchTime}ms`);

            const worksheetStartTime = Date.now();
            await switchTab(page, 'PM Work Sheet');
            const worksheetSwitchTime = Date.now() - worksheetStartTime;
            console.log(`PM Work Sheet 탭 전환 시간: ${worksheetSwitchTime}ms`);
