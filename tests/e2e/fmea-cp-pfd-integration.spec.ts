 ...
 * @description FMEA ↔ CP ↔ PFD 통합 FK 정합성 검증 E2E 테스트
 *
 * 검증 항목:
 * 1. FMEA pipeline-verify allGreen=true (기본)
 * 2. sync-cp-pfd 실행 → CP/PFD 생성
 * 3. cp-pfd-verify → 7개 FK orphan=0
 * 4. CP 워크시트 저장 후 FK 보존 확인
 * 5. PFD 워크시트 저장 후 FK 보존 확인
 * 6. 브라우저에서 CP/PFD 워크시트 렌더링 확인
 */
...
test.describe('FMEA↔CP↔PFD 통합 FK 검증', () => {
  ...
  test('Step 1: FMEA pipeline allGreen=true', async ({ request }) => {
    ...
    expect(data.steps.every((s: any) => s.status === 'ok')).toBe(true);

    console.log(`FMEA pipeline: allGreen=${data.allGreen}`);
  });

  test('Step 2: sync-cp-pfd 실행 → CP/PFD 생성', async ({ request }) => {
    ...
    console.log(`sync-cp-pfd: CP=${data.data.cpItems}, PFD=${data.data.pfdItems}`);
  });
  ...
  test('Step 3: cp-pfd-verify → 7 FK orphan=0', async ({ request }) => {
    ...
    expect(data.fmea.fl).toBe(103);
    expect(data.fmea.ra).toBe(103);
